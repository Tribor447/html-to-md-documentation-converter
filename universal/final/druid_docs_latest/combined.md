...
- [Getting started](#tutorials)
  ...
  - [Other tutorials](#tutorials-tutorial-kerberos-hadoop)
...
- [Data management](#data-management)
  ...
  - [Compaction](#data-management-compaction)
    ...
    - [Cascading reindexing](#data-management-cascading-reindexing)
...
- [Release info](#release-info-release-notes)
  - [Release notes](#release-info-release-notes)
...
<!-- source_url: https://druid.apache.org/docs/latest/design/ -->
...
[Powered by Apache Druid](https://druid.apache.org/druid-powered).
...
- You need low-latency updates of *existing* records using a primary key. Druid supports streaming inserts, but not streaming updates. You can perform updates using
...
- Try the Druid [Quickstart](#tutorials).
- Learn more about Druid components in [Design](#design-architecture).
...
<!-- source_url: https://druid.apache.org/docs/latest/tutorials/ -->
...
[Ingestion](#ingestion) page to determine which ingestion method is right for you.
...
overview](#operations-security-overview). In general, avoid running Druid as root user. Consider creating a
...
After starting the Druid services, open the [web console](#operations-web-console) at <http://localhost:8888>.

![web console](assets/images/tutorial-quickstart-01-55397fb67d2b63fa55d39770575a846f_368934db783c123c.png "web console")
...
For a complete walkthrough of the **Query** view as it relates to the multi-stage query architecture and the MSQ task engine, see [UI walkthrough](#operations-web-console).
...
2. Select the **Local disk** tile and enter the following values:
   ...
   ![Data location](assets/images/tutorial-quickstart-02-02e231b09af60b9b3e84a66bf3742b49_31f2c4b2b595ea6d.png "Data location")
...
4. On the **Parse** page, you can examine the raw data and perform the following optional actions before loading data into Druid:
   ...
   ![Data sample](assets/images/tutorial-quickstart-03-ca6142482a661095586235c8a9327067_644554f62aa9a73e.png "Data sample")
...
8. Click **Run** to execute the query. The task may take a minute or two to complete. When done, the task displays its duration and the number of rows inserted into the table. The view is set to automatically refresh, so you don't need to refresh the browser to see the status change.
   ...
   ![Run query](assets/images/tutorial-quickstart-04-10eea489c28fe821798a99cd69f18ea6_b9cee1056b541908.png "Run query")
...
![Query view](assets/images/tutorial-quickstart-05-85c807249150e21f285bbbbff7dc9a4a_872390a814201801.png "Query view")
...
- [Druid SQL overview](#querying-sql) or the [Query tutorial](#tutorials-tutorial-query) to learn about how to query the data you just ingested.
- [Ingestion overview](#ingestion) to explore options for ingesting more data.
- [Tutorial: Load files using SQL](#tutorials-tutorial-msq-extern) to learn how to generate a SQL query that loads external data into a Druid datasource.
- [Tutorial: Load data with native batch ingestion](https://druid.apache.org/docs/latest/tutorials/tutorial-batch-native) to load and query data with Druid's native batch ingestion feature.
- [Tutorial: Load stream data from Apache Kafka](#tutorials-tutorial-kafka) to load streaming data from a Kafka topic.
...
<a id="operations-single-server"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/single-server/ -->
...
<a id="tutorials-docker"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/docker/ -->
...
The default `docker-compose.yml` launches eight containers: Zookeeper, PostgreSQL, and six Druid containers based upon the [micro quickstart configuration](#operations-single-server--single-server-reference-configurations-deprecated).
...
You can modify the value of `DRUID_SINGLE_NODE_CONF` in the Docker [`environment`](#tutorials-docker--environment-file) to use different single-server mode. For example to use the nano quickstart: `DRUID_SINGLE_NODE_CONF=nano-quickstart`.
...
Once the cluster has started, you can navigate to the [web console](#operations-web-console) at <http://localhost:8888>. The [Druid router process](#design-router) serves the UI.
...
From here you can follow along with the [Quickstart](#tutorials--load-data). For production use, refine your `docker-compose.yml` file to add any additional external service dependencies as necessary.
...
<a id="tutorials-cluster"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/cluster/ -->
...
<a id="tutorials-tutorial-msq-extern"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-msq-extern/ -->

<!-- page_index: 6 -->

# Load files with SQL-based ingestion

> [!NOTE]
> **info**
> This page describes SQL-based batch ingestion using the [`druid-multi-stage-query`](#multi-stage-query)
> extension, new in Druid 24.0. Refer to the [ingestion methods](#ingestion--batch) table to determine which
> ingestion method is right for you.

---

<a id="tutorials-tutorial-kafka"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-kafka/ -->
...
<a id="tutorials-tutorial-rollup"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-rollup/ -->
...
Before proceeding, download Druid as described in [Quickstart (local)](#tutorials) and have it running on your local machine. You don't need to load any data into the Druid cluster.
...
<a id="tutorials-tutorial-ingestion-spec"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-ingestion-spec/ -->
...
the [single-machine quickstart](#tutorials) and have it running on your local machine.

It will also be helpful to have finished [Tutorial: Loading a file](https://druid.apache.org/docs/latest/tutorials/tutorial-batch), [Tutorial: Querying data](#tutorials-tutorial-query), and [Tutorial: Rollup](#tutorials-tutorial-rollup).
...
```json
"tuningConfig" : {"type" : "index_parallel","partitionsSpec": {"type": "dynamic","maxRowsPerSegment" : 5000000}}
```
...
<a id="tutorials-tutorial-transform"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-transform/ -->
...
Before proceeding, download Apache Druid® as described in [Quickstart (local)](#tutorials) and have it running on your local machine. You don't need to load any data into the Druid cluster.
...
- `animal`: prepends "super-" to the values in the `animal` column using the [`TEXTCAT`](#querying-sql-functions--textcat) function. Note that it only ingests the transformed data.
...
- [All functions](#querying-sql-functions) for a list of functions that can be used to transform data.
- [Transform spec reference](#ingestion-ingestion-spec--transformspec) to learn more about transforms in JSON-based batch ingestion.
- [WHERE clause](#querying-sql--where) to learn how to specify filters in Druid SQL.

---

<a id="tutorials-tutorial-msq-convert-spec"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-msq-convert-spec/ -->

<!-- page_index: 11 -->

# Convert an ingestion spec for SQL-based ingestion
...
<a id="tutorials-tutorial-retention"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-retention/ -->
...
It will also be helpful to have finished [Load a file](https://druid.apache.org/docs/latest/tutorials/tutorial-batch) and [Query data](#tutorials-tutorial-query) tutorials.
...
![Summary](assets/images/tutorial-retention-01-09f120b6b0b2bc901f08801e508e778c_565613a4b6d86b7f.png "Summary")
...
![Original segments](assets/images/tutorial-retention-02-c388e3c07c39dc0f5ff33b78b68db565_af42bd6d03e894b5.png "Original segments")
...
![Rule configuration](assets/images/tutorial-retention-03-4082b62d45d1ffdf102f31209ab141f2_2f5c90141016aa46.png "Rule configuration")
...
![Set rules](assets/images/tutorial-retention-04-b0841ac2bfbb857680b28192306b1931_d68fadd87d14b66c.png "Set rules")
...
![New rules](assets/images/tutorial-retention-05-54500f6be8508fb4ddf4468830bd94cd_8cbffbdfd9f433ad.png "New rules")
...
![New segments](assets/images/tutorial-retention-06-8c3d36ef058fbe029fe3ef423d2341de_7ec020a3fbf74e3a.png "New segments")
...
<a id="tutorials-tutorial-append-data"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-append-data/ -->

<!-- page_index: 13 -->

# Append data

This tutorial shows you how to use the Apache Druid SQL [INSERT](#multi-stage-query-reference--insert) function to append data to a [datasource](#design-storage) without changing the existing data.
The examples in the tutorial use the [multi-stage query (MSQ)](#multi-stage-query) task engine to executes SQL statements.

Before you follow the steps in this tutorial, download Druid as described in [Quickstart (local)](#tutorials) and have it running on your local machine. You don't need to load any data into the Druid cluster.
...
<a id="tutorials-tutorial-update-data"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-update-data/ -->

<!-- page_index: 14 -->

# Update data

Apache Druid stores data and indexes in [segment files](#design-segments) partitioned by time.
...
- [Overwrite all data](#tutorials-tutorial-update-data--overwrite-all-data)
- [Overwrite records for a specific time range](#tutorials-tutorial-update-data--overwrite-records-for-a-specific-time-range)
- [Update a row using partial segment overshadowing](#tutorials-tutorial-update-data--update-a-row-using-partial-segment-overshadowing)
...
Load a sample dataset using [REPLACE](#multi-stage-query-reference--replace) and [EXTERN](#multi-stage-query-reference--extern-function) functions.
In Druid SQL, the REPLACE function can create a new [datasource](#design-storage) or update an existing datasource.
...
This capability is called [overshadowing](#ingestion-tasks--overshadowing-between-segments).
...
- The OVERWRITE WHERE and WHERE TIME\_IN\_INTERVAL clauses specify the destination where the update occurs and the source of the update, respectively.
...
<a id="tutorials-tutorial-compaction"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-compaction/ -->
...
<a id="tutorials-tutorial-delete-data"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-delete-data/ -->
...
- A running Apache Druid instance. If you don't have Druid, see the [single-machine quickstart](#tutorials) to get started.
...
![Segments_2](assets/images/tutorial-deletion-02-178ae7a7beae527951d67c5b79412e3d_63586a3872668ef2.png "Segments 2")
...
![Segments 3](assets/images/tutorial-deletion-03-09b79439abe874b5e8d765b249b28d89_4c8ce7413ac5a676.png "Segments 3")
...
```bash
$ ls -l1 var/druid/segments/deletion-tutorial/2015-09-12T00:00:00.000Z_2015-09-12T01:00:00.000Z 2015-09-12T01:00:00.000Z_2015-09-12T02:00:00.000Z 2015-09-12T02:00:00.000Z_2015-09-12T03:00:00.000Z 2015-09-12T03:00:00.000Z_2015-09-12T04:00:00.000Z 2015-09-12T04:00:00.000Z_2015-09-12T05:00:00.000Z 2015-09-12T05:00:00.000Z_2015-09-12T06:00:00.000Z 2015-09-12T06:00:00.000Z_2015-09-12T07:00:00.000Z 2015-09-12T07:00:00.000Z_2015-09-12T08:00:00.000Z 2015-09-12T08:00:00.000Z_2015-09-12T09:00:00.000Z 2015-09-12T09:00:00.000Z_2015-09-12T10:00:00.000Z 2015-09-12T10:00:00.000Z_2015-09-12T11:00:00.000Z 2015-09-12T11:00:00.000Z_2015-09-12T12:00:00.000Z 2015-09-12T12:00:00.000Z_2015-09-12T13:00:00.000Z 2015-09-12T13:00:00.000Z_2015-09-12T14:00:00.000Z 2015-09-12T14:00:00.000Z_2015-09-12T15:00:00.000Z 2015-09-12T15:00:00.000Z_2015-09-12T16:00:00.000Z 2015-09-12T16:00:00.000Z_2015-09-12T17:00:00.000Z 2015-09-12T17:00:00.000Z_2015-09-12T18:00:00.000Z 2015-09-12T20:00:00.000Z_2015-09-12T21:00:00.000Z 2015-09-12T21:00:00.000Z_2015-09-12T22:00:00.000Z
```

---

<a id="tutorials-tutorial-latest-by"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-latest-by/ -->

<!-- page_index: 17 -->

# Query for latest values

This tutorial describes strategies in Apache Druid for use cases that might be handled by UPSERT in other databases. You can use the LATEST\_BY aggregation at query time or "deltas" for numeric dimensions at insert time.

The [Update data](#tutorials-tutorial-update-data) tutorial demonstrates how to use batch operations to update data according to the timestamp, including UPSERT cases. However, with streaming data, you can potentially use LATEST\_BY or deltas to satisfy requirements otherwise handled with updates.

Before you follow the steps in this tutorial, download Druid as described in the [Local quickstart](#tutorials) and have it running on your local machine. You don't need to load any data into the Druid cluster.

You should be familiar with data querying in Druid. If you haven't already, go through the [Query data](#tutorials-tutorial-query) tutorial first.

Sometimes, you want to read the latest value of one dimension or measure in relation to another dimension. In a transactional database, you might maintain dimensions or measures using UPSERT, but in Druid you can append all updates or changes during ingestion. The LATEST\_BY function lets you get the most recent value for the dimension with the following type of query:
...
Alternatively, you can perform ingestion-time aggregation using LATEST\_BY and append updates with streaming ingestion into a rolled up datasource. Appending into a time chunk adds new segments and does not perfectly roll up data, so rows may be partial rather than complete rollups, and you may have multiple partially rolled up rows. In this case, you still need to use the GROUP BY query for correct querying of the rolled up data source. You can tune automatic compaction to significantly reduce the number of stale rows and improve your performance.
...
The following query returns the same points per hour as the second LATEST\_BY example:
...
- [Update data](#tutorials-tutorial-update-data) for a tutorial on updating data in Druid.
- [Data updates](#data-management-update) for an overview of updating data in Druid.

---

<a id="tutorials-tutorial-query"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-query/ -->
...
<a id="tutorials-tutorial-sql-query-view"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-sql-query-view/ -->
...
Query view lets you run [Druid SQL queries](#querying-sql) and [native (JSON-based) queries](#querying) against ingested data.

You can use Query view to test and tune queries before you use them in API requests—for example, to perform [SQL-based ingestion](#api-reference-sql-ingestion-api). You can also ingest data directly in Query view.
...
Before you follow the steps in this tutorial, download Druid as described in the [quickstart](#tutorials) and have it running on your local machine. You don't need to have loaded any data.
...
In this section you load the demo queries and run a SQL task to ingest sample data into a [table datasource](#querying-datasource--table).
...
2. Click the ellipsis at the bottom of the query window and select **Load demo queries**. Note that loading the demo queries replaces all of your current query tabs. The demo queries load in several tabs:
   ...
   ![demo queries](assets/images/tutorial-sql-demo-queries-18b129cf96aca4bf1f61903039a0d462_a3f602ca104a3cf2.png)
3. Click the **Demo 1** tab. This query ingests sample data into a datasource called **kttm\_simple**. Click the **Demo 1** tab heading again and note the options—you can rename, copy, and duplicate tabs.
4. Click **Run** to ingest the data.
5. When ingestion is complete, Druid displays the time it took to complete the insert query, and the new datasource **kttm\_simple** displays in the left pane.
...
2. Click the name of the datasource **kttm\_simple** in the left pane to display some automatically generated queries:

   ![auto queries](assets/images/tutorial-sql-auto-queries-eb62d0f4f2bb563fddf353a7afbd18a2_1199d48f7c3c2cce.png)
3. Click **SELECT \* FROM kttm\_simple** and run the query.
...
[Aggregate functions](#querying-sql-aggregations) allow you to perform a calculation on a set of values and return a single value.
...
2. Click **kttm\_simple** in the left pane to display the generated queries.
3. Click **SELECT COUNT(\*) AS "Count" FROM kttm\_simple** and run the query.
4. After you run a query that contains an aggregate function, additional Query view options become available.

   Click the arrow to the left of the **kttm\_simple** datasource to display the columns, then click the **country** column. Several options appear to apply country-based filters and aggregate functions to the query:

   ![count distinct](assets/images/tutorial-sql-count-distinct-d6020a15c19fff26ed0570fb2a7c7a64_9f0afa43abfa4a7f.png)
...
6. Run the updated query:
   ...
   ![aggregate-query](assets/images/tutorial-sql-aggregate-query-cda2de57ff7e679c2ebb5d1373b3ff62_ab14c37353fe2c55.png)
...
8. From the engine menu you can also edit the query context and turn off some query defaults.
   ...
   Deselect **Use approximate COUNT(DISTINCT)** and rerun the query. The country count in the results decreases because the computation has become more exact. See [SQL aggregation functions](#querying-sql-aggregations) for more information.
9. Query view can provide information about a function, in case you aren't sure exactly what it does.
   ...
   ![count distinct help](assets/images/tutorial-sql-count-distinct-help-4c7ab10ef4a68ccd07adc09287bbe3e5_f868626842310d67.png)

   Click outside the help window to close it.
10. You can perform actions on calculated columns in the results pane.

    Click the results column heading **dist\_country COUNT(DISTINCT "country")** to see the available options:

    ![result columns actions](assets/images/tutorial-sql-result-column-actions-f40697022055f14315241ca973c0847d_2da085e3cd17f641.png)
...
Druid optimizes queries of certain [types](#querying-sql-translation--query-types)—see [SQL query translation](#querying-sql-translation) for information on how to interpret an explain plan and use the details to improve query performance.
...
4. Click the ellipsis at the bottom of the query window and select **Explain SQL query**. The query plan opens in a new window:
   ...
   ![query plan](assets/images/tutorial-sql-query-plan-f9aae6d8f66389766c16b59aadd1a2f0_8a4490c1c1e7de5e.png)
...
Queries without a FROM clause run in calculator mode—this can be useful to help you understand how functions work. See the [Druid SQL functions](#querying-sql-functions) reference for more information.
...
- [Quickstart](#tutorials) for information on getting started with Druid.
- [Tutorial: Querying data](#tutorials-tutorial-query) for example queries to run on Druid data.
- [Ingestion](#ingestion) for an overview of ingestion and the ingestion methods available in Druid.
- [SQL-based ingestion](#multi-stage-query) for an overview of SQL-based ingestion.
- [SQL-based ingestion query examples](#multi-stage-query-examples) for examples of SQL-based ingestion for various use cases.
- [Introduction to Druid SQL](https://github.com/implydata/learn-druid/tree/main/notebooks) to learn more about Druid SQL.

---

<a id="tutorials-tutorial-unnest-arrays"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-unnest-arrays/ -->
...
> [!NOTE]
> **info**
> If you're looking for information about how to unnest `COMPLEX<json>` columns, see [Nested columns](#querying-nested-columns).

---

<a id="tutorials-tutorial-query-deep-storage"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-query-deep-storage/ -->
...
For more general information, see [Query from deep storage](#querying-query-deep-storage).

If you are trying this feature on an existing cluster, make sure query from deep storage [prerequisites](#querying-query-deep-storage--prerequisites) are met.
...
```json
[{"interval": "2016-06-27T00:00:00.000Z/2016-06-27T02:59:00.000Z","tieredReplicants": {},"useDefaultTierForNull": false,"type": "loadByInterval"}]
```
...
![](assets/images/tutorial-query-deepstorage-retention-rule-ed99c67c161810ee03d3301b530d0ede_fea97414cccd000f.png)
...
> **Show the response**
> ```json
> {"queryId": "query-6888b6f6-e597-456c-9004-222b05b97051","state": "ACCEPTED","createdAt": "2023-07-28T21:59:02.334Z","schema": [{"name": "page","type": "VARCHAR","nativeType": "STRING"} ],"durationMs": -1}
> ```
>
...
> **Show the response**
> ```json
> [{"page": "Salo Toraut" },{"page": "利用者:ワーナー成増/放送ウーマン賞" },{"page": "Bailando 2015" },.........]
> ```

- [Query from deep storage](#querying-query-deep-storage)
- [Query from deep storage API reference](#api-reference-sql-api--query-from-deep-storage)

---

<a id="tutorials-tutorial-extern"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-extern/ -->

<!-- page_index: 22 -->

# Export query results

This tutorial demonstrates how to use the Apache Druid® SQL [EXTERN](#multi-stage-query-reference--extern-function) function to export data.

Before you follow the steps in this tutorial, download Druid as described in the [Local quickstart](#tutorials).
Don't start Druid, you'll do that as part of the tutorial.

You should be familiar with ingesting and querying data in Druid.
If you haven't already, go through the [Query data](#tutorials-tutorial-query) tutorial first.
...
2. After Druid starts, open <http://localhost:8888/> in your browser to access the Web Console.
3. From the [Query view](http://localhost:8888/unified-console.html#workbench), run the following command to load the Wikipedia example data set:
...
1. Enable the extension for your cloud storage destination. See [Loading core extensions](#configuration-extensions--loading-core-extensions).

   - **Amazon S3**: `druid-s3-extensions`
   - **GCS**: `google-extensions`
     See [Loading core extensions](#configuration-extensions--loading-core-extensions) for more information.
2. Configure the additional properties for your cloud storage destination. Replace `{CLOUD}` with `s3` or `google` accordingly:
...
3. Verify the instance role has the correct permissions to the bucket and folders: read, write, create, and delete. See [Permissions for durable storage](#multi-stage-query-security--permissions-for-durable-storage).
4. Use the query syntax for your cloud storage type. For example:


<div class="tabs-container tabList__CuJ"><ul><li>S3</li><li>GCS</li></ul><div><div><div><div><pre><code><span><span>INSERT</span><span> </span><span>INTO</span><span></span> </span><span><span>EXTERN</span><span>(</span><span></span> </span><span><span>  s3</span><span>(</span><span>bucket </span><span>=</span><span>&gt;</span><span> </span><span>'your_bucket'</span><span>,</span><span> prefix </span><span>=</span><span>&gt;</span><span> </span><span>'prefix/to/files'</span><span>)</span><span>)</span><span></span> </span><span><span></span><span>AS</span><span> CSV</span> </span><span><span></span><span>SELECT</span><span> </span><span>"channel"</span><span>,</span><span></span> </span><span><span></span><span>SUM</span><span>(</span><span>"delta"</span><span>)</span><span> </span><span>AS</span><span> </span><span>"changes"</span><span></span> </span><span><span></span><span>FROM</span><span> </span><span>"wikipedia"</span><span></span> </span><span><span></span><span>GROUP</span><span> </span><span>BY</span><span> </span><span>1</span><span></span> </span><span><span></span><span>LIMIT</span><span> </span><span>10</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>INSERT</span><span> </span><span>INTO</span><span></span> </span><span><span>EXTERN</span> </span><span><span> google</span><span>(</span><span>bucket </span><span>=</span><span>&gt;</span><span> </span><span>'your_bucket'</span><span>,</span><span> prefix </span><span>=</span><span>&gt;</span><span> </span><span>'prefix/to/files'</span><span>)</span><span></span> </span><span><span></span><span>AS</span><span> CSV</span> </span><span><span></span><span>SELECT</span><span> </span><span>"channel"</span><span>,</span><span></span> </span><span><span></span><span>SUM</span><span>(</span><span>"delta"</span><span>)</span><span> </span><span>AS</span><span> </span><span>"changes"</span><span></span> </span><span><span></span><span>FROM</span><span> </span><span>"wikipedia"</span><span></span> </span><span><span></span><span>GROUP</span><span> </span><span>BY</span><span> </span><span>1</span><span></span> </span><span><span></span><span>LIMIT</span><span> </span><span>10</span> </span></code></pre><div></div></div></div></div></div></div>

5. When querying, use the `rowsPerPage` query context parameter to restrict the output file size. While it's possible to add a very large LIMIT at the end of your query to force Druid to create a single file, we don't recommend this technique.

See the following topics for more information:

- [Export to a destination](#multi-stage-query-reference--extern-to-export-to-a-destination) for a reference of the EXTERN.
- [SQL-based ingestion security](#multi-stage-query-security--permissions-for-durable-storage) for cloud permission requirements for MSQ.

---

<a id="tutorials-tutorial-sketches-theta"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-sketches-theta/ -->
...
<a id="tutorials-tutorial-jdbc"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-jdbc/ -->

<!-- page_index: 24 -->

# Use the JDBC driver to query Druid

Redirecting you to the JDBC driver API...

---

<a id="tutorials-tutorial-kerberos-hadoop"></a>

<!-- source_url: https://druid.apache.org/docs/latest/tutorials/tutorial-kerberos-hadoop/ -->
...
3. Give druid processes appropriate permissions for the druid processes to access this folder. This would ensure that druid is able to create necessary folders like data and indexing\_log in HDFS.
...
<!-- source_url: https://druid.apache.org/docs/latest/design/architecture/ -->
...
![Druid architecture](assets/images/druid-architecture-a2d14e8be60d586632471e97383e406c_08e7cb24c479f94a.svg)

The following sections describe the components of this architecture.

Druid has several types of services:

- [Coordinator](#design-coordinator) manages data availability on the cluster.
- [Overlord](#design-overlord) controls the assignment of data ingestion workloads.
- [Broker](#design-broker) handles queries from external clients.
- [Router](#design-router) routes requests to Brokers, Coordinators, and Overlords.
- [Historical](#design-historical) stores queryable data.
- [Middle Manager](#design-middlemanager) and [Peon](#design-peons) ingest data.
- [Indexer](#design-indexer) serves as an alternative to the Middle Manager + Peon task execution system.

You can view services in the **Services** tab in the web console:

![Druid services](assets/images/services-overview-626a864a11edcf4945591a35d4a3814c_25dcf3ecd2c954e4.png "Services in the web console")

You can deploy Druid services according to your preferences. For ease of deployment, we recommend organizing them into three server types: [Master](#design-architecture--master-server), [Query](#design-architecture--query-server), and [Data](#design-architecture--data-server).

A Master server manages data ingestion and availability. It is responsible for starting new ingestion jobs and coordinating availability of data on the [Data server](#design-architecture--data-server).

Master servers divide operations between Coordinator and Overlord services.

[Coordinator](#design-coordinator) services watch over the Historical services on the Data servers. They are responsible for assigning segments to specific servers, and for ensuring segments are well-balanced across Historicals.

[Overlord](#design-overlord) services watch over the Middle Manager services on the Data servers and are the controllers of data ingestion into Druid. They are responsible for assigning ingestion tasks to Middle Managers and for coordinating segment publishing.
...
[**Router**](#design-router) services provide a unified API gateway in front of Brokers, Overlords, and Coordinators.

The Router service also runs the [web console](#operations-web-console), a UI for loading data, managing datasources and tasks, and viewing server status and segment information.
...
[**Historical**](#design-historical) services handle storage and querying on historical data, including any streaming data that has been in the system long enough to be committed. Historical services download segments from deep storage and respond to queries about these segments. They don't accept writes.

[**Middle Manager**](#design-middlemanager) services handle ingestion of new data into the cluster. They are responsible
for reading from external data sources and publishing new Druid segments.

[**Peon**](#design-peons) services are task execution engines spawned by Middle Managers. Each Peon runs a separate JVM and is responsible for executing a single task. Peons always run on the same host as the Middle Manager that spawned them.

[**Indexer**](#design-indexer) services are an alternative to Middle Managers and Peons. Instead of
forking separate JVM processes per-task, the Indexer runs tasks as individual threads within a single JVM process.

The Indexer is designed to be easier to configure and deploy compared to the MiddleManager + Peon system and to better enable resource sharing across tasks, which can help streaming ingestion. The Indexer is currently designated [experimental](#development-experimental).

Typically, you would deploy one of the following: MiddleManagers, [MiddleManager-less ingestion using Kubernetes](https://druid.apache.org/docs/latest/development/extensions-core/k8s-jobs), or Indexers. You wouldn't deploy more than one of these options.
...
For more information, see [Coordinator Operation](#configuration--coordinator-operation).
...
- To store all the data you ingest. Segments that get loaded onto Historical services for low latency queries are also kept in deep storage for backup purposes. Additionally, segments that are only in deep storage can be used for [queries from deep storage](#querying-query-deep-storage).
...
For more details, please see the [Deep storage](#design-deep-storage) page.
...
For more details, please see the [Metadata storage](#design-metadata-storage) page.

Used for internal service discovery, coordination, and leader election.

For more details, please see the [ZooKeeper](#design-zookeeper) page.

See the following topics for more information:

- [Storage components](#design-storage) to learn about data storage in Druid.
- [Segments](#design-segments) to learn about segment files.
- [Query processing](#querying-query-processing) for a high-level overview of how Druid processes queries.

---

<a id="design-coordinator"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/coordinator/ -->
...
> [!NOTE]
> **info**
> This policy currently cannot handle the situation when there are a lot of small segments which have the same interval, and their total size exceeds [`inputSegmentSizeBytes`](#configuration--automatic-compaction-dynamic-configuration).
> If it finds such segments, it simply skips them.

---

<a id="design-overlord"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/overlord/ -->
...
For Apache Druid Overlord service configuration, see [Overlord Configuration](#configuration--overlord).

For basic tuning guidance for the Overlord service, see [Basic cluster tuning](#operations-basic-cluster-tuning--overlord).

For a list of API endpoints supported by the Overlord, please see the [Service status API reference](#api-reference-service-status-api--overlord).
...
<a id="design-broker"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/broker/ -->
...
<a id="design-router"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/router/ -->
...
<a id="design-historical"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/historical/ -->
...
For Apache Druid Historical service configuration, see [Historical configuration](#configuration--historical).

For basic tuning guidance for the Historical service, see [Basic cluster tuning](#operations-basic-cluster-tuning--historical).

For a list of API endpoints supported by the Historical, please see the [Service status API reference](#api-reference-service-status-api--historical).
...
For more information, see [Segment cache size](#operations-basic-cluster-tuning--segment-cache-size).

The [Coordinator](#design-coordinator) controls the assignment of segments to Historicals and the balance of segments between Historicals. Historical services do not communicate directly with each other, nor do they communicate directly with the Coordinator. Instead, the Coordinator creates ephemeral entries in ZooKeeper in a [load queue path](#configuration--path-configuration). Each Historical service maintains a connection to ZooKeeper, watching those paths for segment information.
...
For more information about segment metadata and Druid segments in general, see [Segments](#design-segments).

After a Historical service pulls down and processes a segment from deep storage, Druid advertises the segment as being available for queries from the Broker. This announcement by the Historical is made via ZooKeeper, in a [served segments path](#configuration--path-configuration).

For more information about how the Broker determines what data is available for queries, see [Broker](#design-broker).
...
Note that this memory-mapped segment cache is in addition to other [query-level caches](#querying-caching).

You can configure a Historical service to log and report metrics for every query it services.
For information on querying Historical services, see [Querying](#querying).

---

<a id="design-middlemanager"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/middlemanager/ -->

<!-- page_index: 32 -->

# Middle Manager service

The Middle Manager service is a worker service that executes submitted tasks. Middle Managers forward tasks to [Peons](#design-peons) that run in separate JVMs.
...
For Apache Druid Middle Manager service configuration, see [Middle Manager and Peons](#configuration--middle-manager-and-peon).

For basic tuning guidance for the Middle Manager service, see [Basic cluster tuning](#operations-basic-cluster-tuning--middle-manager).

For a list of API endpoints supported by the Middle Manager, see the [Service status API reference](#api-reference-service-status-api--middle-manager).
...
<!-- source_url: https://druid.apache.org/docs/latest/design/peons/ -->
...
For Apache Druid Peon configuration, see [Peon Query Configuration](#configuration--peon-query-configuration) and [Additional Peon Configuration](#configuration--additional-peon-configuration).

For basic tuning guidance for Middle Manager tasks, see [Basic cluster tuning](#operations-basic-cluster-tuning--task-configurations).
...
<a id="design-indexer"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/indexer/ -->
...
> [!NOTE]
> **info**
> The Indexer is an optional and experimental feature. If you're primarily performing batch ingestion, we recommend you use either the MiddleManager and Peon task execution system or [MiddleManager-less ingestion using Kubernetes](https://druid.apache.org/docs/latest/development/extensions-core/k8s-jobs). If you're primarily doing streaming ingestion, you may want to try either [MiddleManager-less ingestion using Kubernetes](https://druid.apache.org/docs/latest/development/extensions-core/k8s-jobs) or the Indexer service.

---

<a id="design-storage"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/storage/ -->

<!-- page_index: 35 -->

# Storage overview

Druid stores data in datasources, which are similar to tables in a traditional RDBMS. Each datasource is partitioned by time and, optionally, further partitioned by other attributes. Each time range is called a chunk (for example, a single day, if your datasource is partitioned by day). Within a chunk, data is partitioned into one or more [segments](#design-segments). Each segment is a single file, typically comprising up to a few million rows of data. Since segments are organized into time chunks, it's sometimes helpful to think of segments as living on a timeline like the following:

![Segment timeline](assets/images/druid-timeline-1eb095729ad3c5084dc7ff60aad39421_6e06d0fb8b231053.png)
...
Periodically, segments are committed and published to [deep storage](#design-deep-storage), become immutable, and move from Middle Managers to the Historical services. An entry about the segment is also written to the [metadata store](#design-metadata-storage). This entry is a self-describing bit of metadata about the segment, including things like the schema of the segment, its size, and its location on deep storage. These entries tell the Coordinator what data is available on the cluster.

For details on the segment file format, see [segment files](#design-segments).

For details on modeling your data in Druid, see [schema design](#ingestion-schema-design).
...
- Datasource name.
- Time interval for the time chunk containing the segment; this corresponds to the `segmentGranularity` specified at ingestion time. Uses the same format as [query granularity](#querying-granularities).
...
[`sys.segments` table](#querying-sql-metadata-tables--segments-table). It includes the following flags:
...
[Indexing and handoff](#design-storage--indexing-and-handoff). This means that when understanding Druid's availability and consistency properties, we must look at each function separately.

On the ingestion side, Druid's primary [ingestion methods](#ingestion--ingestion-methods) are all pull-based and offer transactional guarantees. This means that you are guaranteed that ingestion using these methods will publish in an all-or-nothing manner:

- Supervised "seekable-stream" ingestion methods like [Kafka](#ingestion-kafka-ingestion) and [Kinesis](#ingestion-kinesis-ingestion). With these methods, Druid commits stream offsets to its [metadata store](#design-metadata-storage) alongside segment metadata, in the same transaction. Note that ingestion of data that has not yet been published can be rolled back if ingestion tasks fail. In this case, partially-ingested data is
...
- [SQL REPLACE](#multi-stage-query-reference--replace) statements. The controller task publishes all segment metadata when the statement is finished executing.
- [Native batch ingestion](#ingestion-native-batch). In parallel mode, the supervisor task publishes all segment metadata in a single transaction after the subtasks are finished. In simple (single-task) mode, the single task publishes all segment metadata in a single transaction after it is complete.
...
- Supervised "seekable-stream" ingestion methods like [Kafka](#ingestion-kafka-ingestion) and [Kinesis](#ingestion-kinesis-ingestion) are idempotent due to the fact that stream offsets and segment metadata are stored together and updated in lock-step.
- [SQL REPLACE](#multi-stage-query-reference--replace) statements are idempotent unless reading from the same Druid datasource that you are ingesting into. In this case, running the same task twice is non-idempotent, because you are adding to existing data instead of overwriting it.
- [Native batch ingestion](#ingestion-native-batch) is idempotent unless
  [`appendToExisting`](#ingestion-native-batch) is true, or one of your input sources is the same Druid datasource that you are ingesting into. In either of these two cases, running the same task twice is non-idempotent, because you are adding to existing data instead of overwriting it.
...
[`forceTimeChunkLock`](#ingestion-tasks--context-parameters) to false in the context of an ingestion task. In this case, Druid creates an atomic update group using the existing version for the time chunk, instead of creating a new core set with a new version number. There can be multiple atomic update groups with the same version number per time chunk. Each one replaces a specific set of earlier segments in the same time chunk and with the same version number. Druid will query the latest one that is fully available. This is a more powerful version of the core set concept, because it enables atomically replacing a subset of data for a time chunk, as well as doing atomic replacement and appending simultaneously.
...
<a id="design-segments"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/segments/ -->

<!-- page_index: 36 -->

# Segments

Apache Druid stores its data and indexes in *segment files* partitioned by time. Druid creates a segment for each segment interval that contains data. If an interval is empty—that is, containing no rows—no segment exists for that time interval. Druid may create multiple segments for the same interval if you ingest data for that period via different ingestion jobs. [Compaction](#data-management-compaction) is the Druid process that attempts to combine these segments into a single segment per interval for optimal performance.

The time interval is configurable in the `segmentGranularity` parameter of the [`granularitySpec`](#ingestion-ingestion-spec--granularityspec).
...
the [Batch ingestion](#ingestion-native-batch--partitionsspec) documentation
for more guidance.
...
![Druid column types](assets/images/druid-column-types-0cd9a011b4cc2b4d05f93b3b31a7e735_2ac3eafcc502f0c6.png "Druid Column Types")
...
```text
1: Dictionary {"Justin Bieber": 0,"Ke$ha":         1}
2: List of column data [0,0,1,1]
3: Bitmaps value="Justin Bieber": [1,1,0,0] value="Ke$ha":         [0,0,1,1]
```
...
```text
1: Dictionary {"Justin Bieber": 0,"Ke$ha":         1}
2: List of column data [0,[0,1],  <--Row value in a multi-value column can contain an array of values 1,1]
3: Bitmaps value="Justin Bieber": [1,1,0,0] value="Ke$ha":         [0,1,1,1] ^ | | Multi-value column contains multiple non-zero entries
```
...
Druid also supports Concise bitmap compression. For string column bitmaps, the differences between using Roaring and Concise are most pronounced for high cardinality columns. In this case, Roaring is substantially faster on filters that match many values, but in some cases Concise can have a lower footprint due to the overhead of the Roaring format (but is still slower when many values are matched). You configure compression at the segment level, not for individual columns. See [IndexSpec](#ingestion-ingestion-spec--indexspec) for more details.
...
<a id="design-deep-storage"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/deep-storage/ -->
...
In addition to being the backing store for segments, you can use [query from deep storage](#design-deep-storage--querying-from-deep-storage) and run queries against segments stored primarily in deep storage. The [load rules](#operations-rule-configuration--load-rules) you configure determine whether segments exist primarily in deep storage or in a combination of deep storage and Historical processes.
...
use cloud-based deep storage ([Amazon S3](#design-deep-storage--amazon-s3-or-s3-compatible), [Google Cloud Storage](#design-deep-storage--google-cloud-storage), or [Azure Blob Storage](#design-deep-storage--azure-blob-storage)), S3-compatible storage (like Minio), or [HDFS](#design-deep-storage--hdfs). These options are
...
See [`druid-s3-extensions`](https://druid.apache.org/docs/latest/development/extensions-core/s3).

See [`druid-google-extensions`](https://druid.apache.org/docs/latest/development/extensions-core/google).

See [`druid-azure-extensions`](https://druid.apache.org/docs/latest/development/extensions-core/azure).

See [druid-hdfs-storage extension documentation](https://druid.apache.org/docs/latest/development/extensions-core/hdfs).

For additional deep storage options, please see our [extensions list](#configuration-extensions).
...
For information about how to run queries, see [Query from deep storage](#querying-query-deep-storage).

---

<a id="design-metadata-storage"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/metadata-storage/ -->
...
<a id="design-zookeeper"></a>

<!-- source_url: https://druid.apache.org/docs/latest/design/zookeeper/ -->
...
1. [Coordinator](#design-coordinator) leader election
2. Segment "publishing" protocol from [Historical](#design-historical)
3. [Overlord](#design-overlord) leader election
4. [Overlord](#design-overlord) and [Middle Manager](#design-middlemanager) task management
...
All [Historical](#design-historical) processes publish themselves on the `announcementsPath`, specifically, they will create an ephemeral znode at
...
Processes like the [Coordinator](#design-coordinator) and [Broker](#design-broker) can then watch these paths to see which processes are currently serving which segments.

---

<a id="ingestion"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/ -->

<!-- page_index: 40 -->

# Ingestion overview

Loading data in Druid is called *ingestion* or *indexing*. When you ingest data into Druid, Druid reads the data from
your source system and stores it in data files called [*segments*](#design-segments).
In general, segment files contain a few million rows each.

For most ingestion methods, the Druid [Middle Manager](#design-middlemanager) processes or the
[Indexer](#design-indexer) processes load your source data.

During ingestion, Druid creates segments and stores them in [deep storage](#design-deep-storage). Historical nodes load the segments into memory to respond to queries. For streaming ingestion, the Middle Managers and indexers can respond to queries in real-time with arriving data. For more information, see [Storage overview](#design-storage).

This topic introduces streaming and batch ingestion methods. The following topics describe ingestion concepts and information that apply to all [ingestion methods](#ingestion--ingestion-methods):

- [Druid schema model](#ingestion-schema-model) introduces concepts of datasources, primary timestamp, dimensions, and metrics.
- [Data rollup](#ingestion-rollup) describes rollup as a concept and provides suggestions to maximize the benefits of rollup.
- [Partitioning](#ingestion-partitioning) describes time chunk and secondary partitioning in Druid.
- [Ingestion spec reference](#ingestion-ingestion-spec) provides a reference for the configuration options in the ingestion spec.
...
| **Method** | [Kafka](#ingestion-kafka-ingestion) | [Kinesis](#ingestion-kinesis-ingestion) |
| --- | --- | --- |
...
| **Method** | [Native batch](#ingestion-native-batch) | [SQL](#multi-stage-query) |
| --- | --- | --- |
| **Controller task type** | `index_parallel` | `query_controller` |
| **How you submit it** | Send an `index_parallel` spec to the [Tasks API](#api-reference-tasks-api). | Send an [INSERT](#multi-stage-query-concepts--load-data-with-insert) or [REPLACE](#multi-stage-query-concepts--overwrite-data-with-replace) statement to the [SQL task API](#api-reference-sql-ingestion-api--submit-a-query). |
| **Parallelism** | Using subtasks, if [`maxNumConcurrentSubTasks`](#ingestion-native-batch--tuningconfig) is greater than 1. | Using `query_worker` subtasks. |
...
| **Input sources** | Any [`inputSource`](#ingestion-input-sources). | Any [`inputSource`](#ingestion-input-sources) (using [EXTERN](#multi-stage-query-concepts--write-to-an-external-destination-with-extern)) or Druid datasource (using FROM). |
| **Input formats** | Any [`inputFormat`](#ingestion-data-formats--input-format). | Any [`inputFormat`](#ingestion-data-formats--input-format). |
| **Secondary partitioning options** | Dynamic, hash-based, and range-based partitioning methods are available. See [partitionsSpec](#ingestion-native-batch--partitionsspec) for details. | Range partitioning ([CLUSTERED BY](#multi-stage-query-concepts--clustering)). |
| **[Rollup modes](#ingestion-rollup--perfect-rollup-vs-best-effort-rollup)** | Perfect if `forceGuaranteedRollup` = true in the [`tuningConfig`](#ingestion-native-batch--tuningconfig). | Always perfect. |

---

<a id="ingestion-data-formats"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/data-formats/ -->
...
> [!NOTE]
> **info**
> The "schema\_inline" decoder reads Avro records using a fixed schema and does not support schema migration. If you
...
<a id="ingestion-input-sources"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/input-sources/ -->
...
For general information on native batch indexing and parallel task indexing, see [Native batch ingestion](#ingestion-native-batch).

> [!NOTE]
> **Required extension**
> To use the S3 input source, load the extension [`druid-s3-extensions`](https://druid.apache.org/docs/latest/development/extensions-core/s3) in your `common.runtime.properties` file.
...
The S3 input source is splittable. Therefore, you can use it with the [parallel task](#ingestion-native-batch). Each worker task of `index_parallel` reads one or multiple objects.
...
```json
..."ioConfig": {"type": "index_parallel","inputSource": {"type": "s3","uris": ["s3://foo/bar/file.json", "s3://bar/foo/file2.json"],"endpointConfig": {"url" : "s3-store.aws.com","signingRegion" : "us-west-2" },"clientConfig": {"protocol" : "http","disableChunkedEncoding" : true,"enablePathStyleAccess" : true,"crossRegionAccessEnabled" : false },"proxyConfig": {"host" : "proxy-s3.aws.com","port" : 8888,"username" : "admin","password" : "admin" },
"properties": {"accessKeyId": "KLJ78979SDFdS2","secretAccessKey": "KLS89s98sKJHKJKJH8721lljkd","assumeRoleArn": "arn:aws:iam::2981002874992:role/role-s3"} },"inputFormat": {"type": "json" },...},...
```
...
| endpointConfig | Config for overriding the default S3 endpoint and signing region. This would allow ingesting data from a different S3 store. Please see [s3 config](https://druid.apache.org/docs/latest/development/extensions-core/s3#connecting-to-s3-configuration) for more information. | None | No (defaults will be used if not given) |
| clientConfig | S3 client properties for the overridden s3 endpoint. This is used in conjunction with `endPointConfig`. Please see [s3 config](https://druid.apache.org/docs/latest/development/extensions-core/s3#connecting-to-s3-configuration) for more information. | None | No (defaults will be used if not given) |
| proxyConfig | Properties for specifying proxy information for the overridden s3 endpoint. This is used in conjunction with `clientConfig`. Please see [s3 config](https://druid.apache.org/docs/latest/development/extensions-core/s3#connecting-to-s3-configuration) for more information. | None | No (defaults will be used if not given) |
...
| Property | Description | Default | Required |
| --- | --- | --- | --- |
| accessKeyId | The [Password Provider](#operations-password-provider) or plain text string of this S3 input source access key | None | Yes, if `secretAccessKey` or `sessionToken` is given. |
| secretAccessKey | The [Password Provider](#operations-password-provider) or plain text string of this S3 input source secret key | None | Yes, if `accessKeyId` or `sessionToken` is given. |
| sessionToken | The [Password Provider](#operations-password-provider) or plain text string of this S3 input source session token | None | no |
...
> [!NOTE]
> **info**
> If `accessKeyId` and `secretAccessKey` are not given, the default [S3 credentials provider chain](https://druid.apache.org/docs/latest/development/extensions-core/s3#s3-authentication-methods) is used.

> [!NOTE]
> **Required extension**
> To use the Google Cloud Storage input source, load the extension [`druid-google-extensions`](https://druid.apache.org/docs/latest/development/extensions-core/google) in your `common.runtime.properties` file.
...
and can be used by the [parallel task](#ingestion-native-batch), where each worker task of `index_parallel` will read
...
> [!NOTE]
> **Required extension**
> To use the Azure input source, load the extension [`druid-azure-extensions`](https://druid.apache.org/docs/latest/development/extensions-core/azure) in your `common.runtime.properties` file.
...
specify objects as a list of file URI strings or prefixes. You can split the Azure input source for use with [parallel task](#ingestion-native-batch) indexing and each worker task reads one chunk of the split data.
...
The Azure input source that uses the type `azure` is an older version of the Azure input type and is not recommended. It doesn't support specifying which storage account to ingest from. We recommend using the [`azureStorage` input source schema](#ingestion-input-sources--azure-input-source) instead since it provides more functionality.
...
> [!NOTE]
> **Required extension**
> To use the HDFS input source, load the extension [`druid-hdfs-storage`](https://druid.apache.org/docs/latest/development/extensions-core/hdfs) in your `common.runtime.properties` file.
...
of HDFS URI strings. The HDFS input source is splittable and can be used by the [parallel task](#ingestion-native-batch), where each worker task of `index_parallel` will read one or multiple files.
...
in `druid.ingestion.hdfs.allowedProtocols`. See [HDFS input source security configuration](#configuration--hdfs-input-source) for more details.
...
For more information about security best practices, see [Security overview](#operations-security-overview--best-practices).

The HTTP input source is *splittable* and can be used by the [parallel task](#ingestion-native-batch), where each worker task of `index_parallel` will read only one file. This input source does not support Split Hint Spec.
...
The `http` and `https` protocols are allowed by default. See [HTTP input source security configuration](#configuration--http-input-source) for more details.
...
The Local input source is to support reading files directly from local storage, and is mainly intended for proof-of-concept testing.
The Local input source is *splittable* and can be used by the [parallel task](#ingestion-native-batch), where each worker task of `index_parallel` will read one or multiple files.
...
The Druid input source is to support reading data directly from existing Druid segments, potentially using a new schema and changing the name, dimensions, metrics, rollup, etc. of the segment.
The Druid input source is *splittable* and can be used by the [parallel task](#ingestion-native-batch).
...
| filter | See [Filters](#querying-filters). Only rows that match the filter, if specified, will be returned. | no |
...
- Changing the [partitioning or sorting](#ingestion-partitioning) of a datasource to improve performance.
- Updating or removing rows using a [`transformSpec`](#ingestion-ingestion-spec--transformspec).
...
your goals can be satisfied by [compaction](#data-management-compaction), consider that instead as a simpler
approach.
...
> [!NOTE]
> **info**
> Older versions (0.19 and earlier) did not respect the timestampSpec when using the Druid input source. If you have ingestion specs that rely on this and cannot rewrite them, set [`druid.indexer.task.ignoreTimestampSpecForDruidInputSource`](#configuration--indexer-general-configuration) to `true` to enable a compatibility mode where the timestampSpec is ignored.

The [secondary partitioning method](#ingestion-native-batch--partitionsspec) determines the requisite number of concurrent worker tasks that run in parallel to complete ingestion with the Combining input source.
...
For more information on the `maxNumConcurrentSubTasks` field, see [Implementation considerations](#ingestion-native-batch--implementation-considerations).

> [!NOTE]
> **Required extension**
> To use the SQL input source, you must load the appropriate extension in your `common.runtime.properties` file.
>
> - To connect to MySQL, load the extension [`mysql-metadata-storage`](https://druid.apache.org/docs/latest/development/extensions-core/mysql).
> - To connect to PostgreSQL, load the extension [`postgresql-metadata-storage`](https://druid.apache.org/docs/latest/development/extensions-core/postgresql).
>
> The MySQL extension requires a JDBC driver.
> For more information, see the [Installing the MySQL connector library](https://druid.apache.org/docs/latest/development/extensions-core/mysql).

The SQL input source is used to read data directly from RDBMS.
You can *split* the ingestion tasks for a SQL input source. When you use the [parallel task](#ingestion-native-batch) type, each worker task reads from one SQL query from the list of queries.
...
Refer to the [recommended practices](#ingestion-input-sources--recommended-practices) before using this input source.

| Property | Description | Required |
| --- | --- | --- |
| type | Set the value to `sql`. | Yes |
| database | Specifies the database connection details. The database type corresponds to the extension that supplies the `connectorConfig` support. You can selectively allow JDBC properties in `connectURI`. See [JDBC connections security config](#configuration--jdbc-connections-to-external-databases) for more details. | Yes |
| foldCase | Boolean to toggle case folding of database column names. For example, to ingest a database column named `Entry_Date` as `entry_date`, set `foldCase` to true and include `entry_date` in the [`dimensionsSpec`](#ingestion-ingestion-spec--dimensionsspec). | No |
...
Each delegate input source must be splittable and compatible with the [parallel task type](#ingestion-native-batch).
...
If you include the [Druid input source](#ingestion-input-sources--druid-input-source), the timestamp column is stored in the `__time` field.
...
> [!NOTE]
> **Required extension**
> To use the Iceberg input source, load the extension [`druid-iceberg-extensions`](https://druid.apache.org/docs/latest/development/extensions-contrib/iceberg) in your `common.runtime.properties` file.
...
For example, if the warehouse associated with an Iceberg catalog is on S3, you must also load the [`druid-s3-extensions`](https://druid.apache.org/docs/latest/development/extensions-core/s3) extension.
...
It is strongly recommended to apply filtering only on Iceberg partition columns. When filtering on non-partition columns, Iceberg filters may return rows that do not fully match the expression. To address this, it may help to define an additional filter in the [`transformSpec`](#ingestion-ingestion-spec--transformspec) to remove residual rows.
...
> [!NOTE]
> **Required extension**
> To use the Delta Lake input source, load the extension [`druid-deltalake-extensions`](https://druid.apache.org/docs/latest/development/extensions-contrib/delta-lake) in your `common.runtime.properties` file.
...
<a id="ingestion-schema-model"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/schema-model/ -->
...
Druid schemas must always include a primary timestamp. Druid uses the primary timestamp to [partition and sort](#ingestion-partitioning) your data. Druid uses the primary timestamp to rapidly identify and retrieve data within the time range of queries. Druid also uses the primary timestamp column
for time-based [data management operations](#data-management) such as dropping time chunks, overwriting time chunks, and time-based retention rules.

Druid parses the primary timestamp based on the [`timestampSpec`](#ingestion-ingestion-spec--timestampspec) configuration at ingestion time. Regardless of the source field for the primary timestamp, Druid always stores the timestamp in the `__time` column in your Druid datasource.

You can control other important operations that are based on the primary timestamp in the
[`granularitySpec`](#ingestion-ingestion-spec--granularityspec). If you have more than one timestamp column, you can store the others as
[secondary timestamps](#ingestion-schema-design--secondary-timestamps).
...
If you disable [rollup](#ingestion-rollup), then Druid treats the set of
...
At ingestion time, you configure dimensions in the [`dimensionsSpec`](#ingestion-ingestion-spec--dimensionsspec).

Metrics are columns that Druid stores in an aggregated form. Metrics are most useful when you enable [rollup](#ingestion-rollup). If you specify a metric, you can apply an aggregation function to each row during ingestion. This
...
- [Rollup](#ingestion-rollup) is a form of aggregation that combines multiple rows with the same timestamp value and dimension values. For example, the [rollup tutorial](#tutorials-tutorial-rollup) demonstrates using rollup to collapse netflow data to a single row per `(minute, srcIP, dstIP)` tuple, while retaining aggregate information about total packet and byte counts.
...
At ingestion time, you configure Metrics in the [`metricsSpec`](#ingestion-ingestion-spec--metricsspec).

---

<a id="ingestion-rollup"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/rollup/ -->
...
At ingestion time, you control rollup with the `rollup` setting in the [`granularitySpec`](#ingestion-ingestion-spec--granularityspec). Rollup is enabled by default. This means Druid combines into a single row any rows that have identical [dimension](#ingestion-schema-model--dimensions) values and [timestamp](#ingestion-schema-model--primary-timestamp) values after [`queryGranularity`-based truncation](#ingestion-ingestion-spec--granularityspec).
...
- You don't need raw values from [high-cardinality dimensions](#ingestion-schema-design--sketches-for-high-cardinality-columns).

Conversely, disable roll-up if either:

- You need results for individual rows.
- You need to execute `GROUP BY` or `WHERE` queries on *any* column.
...
To measure the rollup ratio of a datasource, compare the number of rows in Druid (`COUNT`) with the number of ingested events. For example, run a [Druid SQL](#querying-sql) query where "num\_rows" refers to a `count`-type metric generated at ingestion time as follows:

```sql
SELECT SUM("num_rows") / (COUNT(*) * 1.0) FROM datasource 
```

The higher the result, the greater the benefit you gain from rollup. See [Counting the number of ingested events](#ingestion-schema-design--counting-the-number-of-ingested-events) for more details about how counting works with rollup is enabled.
...
- Use [sketches](#ingestion-schema-design--sketches-for-high-cardinality-columns) to avoid storing high cardinality dimensions, which decrease rollup ratios.
...
- If you use a [best-effort rollup](#ingestion-rollup--perfect-rollup-vs-best-effort-rollup) ingestion configuration that does not guarantee perfect rollup, try one of the following:
  - Switch to a guaranteed perfect rollup option.
  - [Reindex](#data-management-update--reindex) or [compact](#data-management-compaction) your data in the background after initial ingestion.
...
- The ingestion method uses *incremental publishing* which means it finalizes and publishes segments before all data for a time chunk has been received.
...
| Method | How it works |
| --- | --- |
| [Native batch](#ingestion-native-batch) | `index_parallel` and `index` type may be either perfect or best-effort, based on configuration. |
| [SQL-based batch](#multi-stage-query) | Always perfect. |
| [Kafka indexing service](#ingestion-kafka-ingestion) | Always best-effort. |
| [Kinesis indexing service](#ingestion-kinesis-ingestion) | Always best-effort. |

See the following topic for more information:

- [Rollup tutorial](#tutorials-tutorial-rollup) for an example of how to configure rollup, and of how the feature modifies your data.

---

<a id="ingestion-partitioning"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/partitioning/ -->
...
> [query context](#multi-stage-query-reference--context-parameters) (for SQL) or in your
> [`dimensionsSpec`](#ingestion-ingestion-spec--dimensionsspec) (for other ingestion forms).
>
...
<a id="ingestion-tasks"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/tasks/ -->
...
<a id="multi-stage-query"></a>

<!-- source_url: https://druid.apache.org/docs/latest/multi-stage-query/ -->

<!-- page_index: 47 -->

# SQL-based ingestion

This page describes SQL-based batch ingestion using the [multi-stage query (MSQ) task engine](#multi-stage-query). Refer to the [ingestion methods](#ingestion--batch) table to determine which ingestion method is right for you.

Apache® Druid supports SQL-based ingestion using MSQ task engine that allows running SQL
[INSERT](#multi-stage-query-concepts--load-data-with-insert) and [REPLACE](#multi-stage-query-concepts--overwrite-data-with-replace) statements as batch tasks. As an experimental feature, the task engine also supports running `SELECT` queries as batch tasks.
...
issues](#multi-stage-query-known-issues--select-statement) page. This allows great flexibility to apply transformations, filters, JOINs, aggregations, and so on as part of `INSERT ... SELECT` and `REPLACE ... SELECT` statements. This also allows in-database
...
To use [EXTERN](#multi-stage-query-reference--extern-function), you need READ permission on the resource named "EXTERNAL" of the resource type
...
- [Read about key concepts](#multi-stage-query-concepts) to learn more about how SQL-based ingestion and multi-stage queries work.
- [Check out the examples](#multi-stage-query-examples) to see SQL-based ingestion in action.
- [Explore the Query view](#operations-web-console) to get started in the web console.

---

<a id="multi-stage-query-concepts"></a>

<!-- source_url: https://druid.apache.org/docs/latest/multi-stage-query/concepts/ -->
...
The MSQ task engine executes SQL statements as batch tasks in the indexing service, which execute on [Middle Managers](#design-architecture--druid-services).
[INSERT](#multi-stage-query-reference--insert) and [REPLACE](#multi-stage-query-reference--replace) tasks publish [segments](#design-storage) just like [all other forms of batch ingestion](#ingestion--batch). Each query occupies at least two task slots while running: one controller task, and at least one worker task. As an experimental feature, the MSQ task engine also supports running SELECT queries as batch tasks. The behavior and result format of plain SELECT (without INSERT or REPLACE) is subject to change.

You can execute SQL statements using the MSQ task engine through the **Query** view in the [web
console](#operations-web-console) or through the [`/druid/v2/sql/task` API](#api-reference-sql-ingestion-api).

For more details on how SQL queries are executed using the MSQ task engine, see [multi-stage query
tasks](#multi-stage-query-concepts--multi-stage-query-tasks).
...
source](#ingestion-input-sources) and [input format](#ingestion-data-formats--input-format).
...
For more information about the syntax, see [`EXTERN`](#multi-stage-query-reference--extern-function).

See also the set of SQL-friendly input-source-specific table functions which may be more convenient
than `EXTERN`.

`INSERT` statements can create a new datasource or append to an existing datasource. In Druid SQL, unlike standard SQL, there is no syntactical difference between creating a table and appending data to a table. Druid does not include a
`CREATE TABLE` statement.

Nearly all `SELECT` capabilities are available for `INSERT ... SELECT` queries. Certain exceptions are listed on the [Known
issues](#multi-stage-query-known-issues--select-statement) page.

`INSERT` statements acquire a shared lock to the target datasource. Multiple `INSERT` statements can run at the same time, for the same datasource, if your cluster has enough task slots.

Like all other forms of [batch ingestion](#ingestion--batch), each `INSERT` statement generates new segments and
...
ingestion](#ingestion--streaming) instead.
...
for dimension-based pruning, see [Clustering](#multi-stage-query-concepts--clustering).

For more information about the syntax, see [INSERT](#multi-stage-query-reference--insert).
...
`REPLACE` uses an [OVERWRITE clause](#multi-stage-query-reference--replace-specific-time-ranges) to determine which data to overwrite. You
...
For more information about the syntax, see [REPLACE](#multi-stage-query-reference--replace).
...
functions](#querying-sql-scalar--date-and-time-functions); for example: `TIME_FORMAT("timestamp", 'yyyy-MM-dd HH:mm:ss') AS __time`.

The `__time` column is used for [partitioning by time](#multi-stage-query-concepts--partitioning-by-time). If you use `PARTITIONED BY ALL` or
...
For more information, see [Primary timestamp](#ingestion-schema-model--primary-timestamp).
...
`PARTITIONED BY HOUR` and `PARTITIONED BY DAY` are the most common choices to balance these considerations. `PARTITIONED BY ALL` is suitable if your dataset does not have a [primary timestamp](#multi-stage-query-concepts--primary-timestamp).

For more information about the syntax, see [PARTITIONED BY](#multi-stage-query-reference--partitioned-by).

Within each time chunk defined by [time partitioning](#multi-stage-query-concepts--partitioning-by-time), data can be further split by the optional
[CLUSTERED BY](#multi-stage-query-reference--clustered-by) clause.
...
[`rowsPerSegment`](#multi-stage-query-reference--context-parameters) — with lexicographic ranges of `hostName`s grouped into segments.
...
If these requirements are *not* met, Druid still clusters data during ingestion but will not be able to perform
...
For more information about syntax, see [`CLUSTERED BY`](#multi-stage-query-reference--clustered-by).
...
[Rollup](#ingestion-rollup) is a technique that pre-aggregates data during ingestion to reduce the amount of data
...
2. Set [`finalizeAggregations: false`](#multi-stage-query-reference--context-parameters) in your context. This causes aggregation
...
3. See [ARRAY types](#querying-arrays--sql-based-ingestion) for information about ingesting `ARRAY` columns
4. See [multi-value dimensions](#querying-multi-value-dimensions--sql-based-ingestion) for information to ingest multi-value VARCHAR columns
...
queries](#querying-segmentmetadataquery) to retrieve rollup-related information.

The following [aggregation functions](#querying-sql-aggregations) are supported for rollup at ingestion time:
`COUNT` (but switch to `SUM` at query time), `SUM`, `MIN`, `MAX`, `EARLIEST` and `EARLIEST_BY`, `LATEST` and `LATEST_BY`, `APPROX_COUNT_DISTINCT`, `APPROX_COUNT_DISTINCT_BUILTIN`, `APPROX_COUNT_DISTINCT_DS_HLL`, `APPROX_COUNT_DISTINCT_DS_THETA`, and `DS_QUANTILES_SKETCH` (but switch to
...
For an example, see [INSERT with rollup example](#multi-stage-query-examples--insert-with-rollup).

When you execute a SQL statement using the task endpoint [`/druid/v2/sql/task`](#api-reference-sql-ingestion-api--submit-a-query), the following
happens:
...
4. The controller task launches some number of worker tasks determined by
   the `maxNumTasks` and `taskAssignment` [context parameters](#multi-stage-query-reference--context-parameters). You can set these settings individually for each query.
...
The [`maxNumTasks`](#multi-stage-query-reference--context-parameters) query parameter determines the maximum number of tasks your
...
free slots available in your cluster; doing so will result in a [TaskStartTimeout](#multi-stage-query-reference--error-codes)
error.

When [reading external data](#multi-stage-query-concepts--read-external-data-with-extern), EXTERN can read multiple files in parallel across
...
The `druid.worker.capacity` server property on each [Middle Manager](#design-architecture--druid-services)
determines the maximum number of worker tasks that can run on each server at once. Worker tasks run single-threaded, which also determines the maximum number of processors on the server that can contribute towards multi-stage queries.
...
[lookups](#querying-lookups)) is split up into two bundles of equal size: one processor bundle and one worker
bundle. Each one comprises 37.5% of the available JVM heap, less any space used by [lookups](#querying-lookups).
...
[`druid.indexer.task.baseDir`](#configuration--additional-peon-configuration), for these items. It is
important that this directory has enough space available for these purposes.

---

<a id="multi-stage-query-security"></a>

<!-- source_url: https://druid.apache.org/docs/latest/multi-stage-query/security/ -->
...
<a id="multi-stage-query-examples"></a>

<!-- source_url: https://druid.apache.org/docs/latest/multi-stage-query/examples/ -->
...
usage, refer to the [Examples](#multi-stage-query-examples) page.

`INSERT` and `REPLACE` load data into a Druid datasource from either an external input source, or from another
datasource. When loading from an external datasource, you typically must provide the kind of input source, the data format, and the schema (signature) of the input file. Druid provides *table functions* to allow you to
...
input-source-specific functions allow you to use SQL query parameters to specify the set of files (or URIs), making it easy to reuse the same SQL statement for each ingest: just specify the set of files to use each time.
...
```sql
SELECT <column> FROM TABLE(EXTERN('<Druid input source>','<Druid input format>','<row signature>'))
```

`EXTERN` consists of the following parts:

1. Any [Druid input source](#ingestion-input-sources) as a JSON-encoded string.
2. Any [Druid input format](#ingestion-data-formats) as a JSON-encoded string.
...
For more information, see [Read external data with EXTERN](#multi-stage-query-concepts--read-external-data-with-extern).
...
For details on applying context parameters using SET, see [SET](#querying-sql--set).
...
For a list of S3 permissions the MSQ task engine requires to perform export, see [Permissions for durable storage](#multi-stage-query-security--s3).
...
For more information, see [Export external data with EXTERN](#multi-stage-query-concepts--write-to-an-external-destination-with-extern).

Use the `INSERT` statement to insert data.

Unlike standard SQL, `INSERT` loads data into the target table according to column name, not positionally. If necessary, use `AS` in your `SELECT` column list to assign the correct names. Do not rely on their positions within the SELECT
clause.
...
1. Optional [context parameters](#multi-stage-query-reference--context-parameters).
2. An `INSERT INTO <dataSource>` clause at the start of your query, such as `INSERT INTO your-table`.
3. A clause for the data you want to insert, such as `SELECT ... FROM ...`. You can use [`EXTERN`](#multi-stage-query-reference--extern-function)
   to reference external tables using `FROM TABLE(EXTERN(...))`.
4. A [PARTITIONED BY](#multi-stage-query-reference--partitioned-by) clause, such as `PARTITIONED BY DAY`.
5. An optional [CLUSTERED BY](#multi-stage-query-reference--clustered-by) clause.

For more information, see [Load data with INSERT](#multi-stage-query-concepts--load-data-with-insert).

You can use the `REPLACE` function to replace all or some of the data.

Unlike standard SQL, `REPLACE` loads data into the target table according to column name, not positionally. If necessary, use `AS` in your `SELECT` column list to assign the correct names. Do not rely on their positions within the SELECT
clause.
...
5. A [PARTITIONED BY](#multi-stage-query-reference--partitioned-by) clause, such as `PARTITIONED BY DAY`.
6. An optional [CLUSTERED BY](#multi-stage-query-reference--clustered-by) clause.

For more information, see [Overwrite data with REPLACE](#multi-stage-query-concepts--overwrite-data-with-replace).

The `PARTITIONED BY <time granularity>` clause is required for [INSERT](#multi-stage-query-reference--insert) and [REPLACE](#multi-stage-query-reference--replace). See
[Partitioning](#multi-stage-query-concepts--partitioning-by-time) for details.
...
- `TIME_FLOOR(__time, 'granularity_string')`, where granularity\_string is one of the ISO 8601 periods listed below. The
  first argument must be `__time`.
- `FLOOR(__time TO TimeUnit)`, where `TimeUnit` is any unit supported by the [FLOOR function](#querying-sql-scalar--date-and-time-functions). The first argument must be `__time`.
...
For more information about partitioning, see [Partitioning](#multi-stage-query-concepts--partitioning-by-time).
...
The `CLUSTERED BY <column list>` clause is optional for [INSERT](#multi-stage-query-reference--insert) and [REPLACE](#multi-stage-query-reference--replace). It accepts a list of
column names or expressions.

This column list is used for [secondary partitioning](#ingestion-partitioning--secondary-partitioning) of segments
within a time chunk, and [sorting](#ingestion-partitioning--sorting) of rows within a segment. For sorting purposes, Druid implicitly prepends `__time` to the `CLUSTERED BY` column list, unless
[`forceSegmentSortByTime`](#multi-stage-query-reference--context-parameters) is set to `false`
(an experimental feature; see [Sorting](#ingestion-partitioning--sorting) for details).

For more information about clustering, see [Clustering](#multi-stage-query-concepts--clustering).

The multi-stage query task engine supports the [SQL context parameters](#querying-sql-query-context), as well as its own context parameters described in this section. Use these parameters to tailor how Druid executes your query.

You can specify the context parameters in SELECT, INSERT, or REPLACE statements.

For detailed instructions on configuring query context parameters, refer to [Set query context](#querying-query-context).

The following table lists the context parameters for the MSQ task engine:

<table><thead><tr><th>Parameter</th><th>Description</th><th>Default value</th></tr></thead><tbody><tr><td><code>maxNumTasks</code></td><td>SELECT, INSERT, REPLACE

The maximum total number of tasks to launch, including the controller task. The lowest possible value for this setting is 2: one controller and one worker. All tasks must be able to launch simultaneously. If they cannot, the query returns a <code>TaskStartTimeout</code> error code after approximately 10 minutes.

May also be provided as <code>numTasks</code>. If both are present, <code>maxNumTasks</code> takes priority.</td><td>2</td></tr><tr><td><code>taskAssignment</code></td><td>SELECT, INSERT, REPLACE

Determines how many tasks to use. Possible values include: <ul><li><code>max</code>: Uses as many tasks as possible, up to <code>maxNumTasks</code>.</li><li><code>auto</code>: When file sizes can be determined through directory listing (for example: local files, S3, GCS, HDFS) uses as few tasks as possible without exceeding 512 MiB or 10,000 files per task, unless exceeding these limits is necessary to stay within <code>maxNumTasks</code>. When calculating the size of files, the weighted size is used, which considers the file format and compression format used if any. When file sizes cannot be determined through directory listing (for example: http), behaves the same as <code>max</code>.</li></ul></td><td><code>max</code></td></tr><tr><td><code>finalizeAggregations</code></td><td>SELECT, INSERT, REPLACE  Determines the type of aggregation to return. If true, Druid finalizes the results of complex aggregations that directly appear in query results. If false, Druid returns the aggregation's intermediate type rather than finalized type. This parameter is useful during ingestion, where it enables storing sketches directly in Druid tables. For more information about aggregations, see <a href="#querying-sql-aggregations">SQL aggregation functions</a>.</td><td><code>true</code></td></tr><tr><td><code>arrayIngestMode</code></td><td>INSERT, REPLACE

 Controls how ARRAY type values are stored in Druid segments. When set to <code>array</code> (recommended for SQL compliance), Druid will store all ARRAY typed values in <a href="#querying-arrays">ARRAY typed columns</a>, and supports storing both VARCHAR and numeric typed arrays. When set to <code>mvd</code> (the default, for backwards compatibility), Druid only supports VARCHAR typed arrays, and will store them as <a href="#querying-multi-value-dimensions">multi-value string columns</a>. See [<code>arrayIngestMode</code>] in the <a href="#querying-arrays">Arrays</a> page for more details.</td><td><code>mvd</code> (for backwards compatibility, recommended to use <code>array</code> for SQL compliance)</td></tr><tr><td><code>sqlJoinAlgorithm</code></td><td>SELECT, INSERT, REPLACE

Algorithm to use for JOIN. Use <code>broadcast</code> (the default) for broadcast hash join or <code>sortMerge</code> for sort-merge join. Affects all JOIN operations in the query. This is a hint to the MSQ engine and the actual joins in the query may proceed in a different way than specified. See <a href="#multi-stage-query-reference--joins">Joins</a> for more details.</td><td><code>broadcast</code></td></tr><tr><td><code>maxRowsInMemory</code></td><td>INSERT or REPLACE  Maximum number of rows to store in memory at once before flushing to disk during the segment generation process. Ignored for non-INSERT queries. In most cases, use the default value. You may need to override the default if you run into one of the <a href="#multi-stage-query-known-issues">known issues</a> around memory usage.</td><td>100,000</td></tr><tr><td><code>rowsInMemory</code></td><td>INSERT or REPLACE

Alternate spelling of <code>maxRowsInMemory</code>. Ignored if <code>maxRowsInMemory</code> is set.</td><td>100,000</td></tr><tr><td><code>segmentSortOrder</code></td><td>INSERT or REPLACE

Normally, Druid sorts rows in individual segments using <code>__time</code> first, followed by the <a href="#multi-stage-query-reference--clustered-by">CLUSTERED BY</a> clause. When you set <code>segmentSortOrder</code>, Druid uses the order from this context parameter instead. Provide the column list as comma-separated values or as a JSON array in string form.
&lt; br/&gt;For example, consider an INSERT query that uses <code>CLUSTERED BY country</code> and has <code>segmentSortOrder</code> set to <code>__time,city,country</code>. Within each time chunk, Druid assigns rows to segments based on <code>country</code>, and then within each of those segments, Druid sorts those rows by <code>__time</code> first, then <code>city</code>, then <code>country</code>.</td><td>empty list</td></tr><tr><td><code>forceSegmentSortByTime</code></td><td>INSERT or REPLACE

When set to <code>true</code> (the default), Druid prepends <code>__time</code> to <a href="#multi-stage-query-reference--clustered-by">CLUSTERED BY</a> when determining the sort order for individual segments. Druid also requires that <code>segmentSortOrder</code>, if provided, starts with <code>__time</code>.

When set to <code>false</code>, Druid uses the <a href="#multi-stage-query-reference--clustered-by">CLUSTERED BY</a> alone to determine the sort order for individual segments, and does not require that <code>segmentSortOrder</code> begin with <code>__time</code>. Setting this parameter to <code>false</code> is an experimental feature; see <a href="#ingestion-partitioning--sorting">Sorting</a> for details.</td><td><code>true</code></td></tr><tr><td><code>maxParseExceptions</code></td><td>SELECT, INSERT, REPLACE

Maximum number of parse exceptions that are ignored while executing the query before it stops with <code>TooManyWarningsFault</code>. To ignore all the parse exceptions, set the value to -1.</td><td>0</td></tr><tr><td><code>rowsPerSegment</code></td><td>INSERT or REPLACE  The number of rows per segment to target. The actual number of rows per segment may be somewhat higher or lower than this number. In most cases, use the default. For general information about sizing rows per segment, see <a href="#operations-segment-optimization">Segment Size Optimization</a>.</td><td>3,000,000</td></tr><tr><td><code>indexSpec</code></td><td>INSERT or REPLACE

An <a href="#ingestion-ingestion-spec--indexspec"><code>indexSpec</code></a> to use when generating segments. May be a JSON string or object. See <a href="#ingestion-ingestion-spec--front-coding">Front coding</a> for details on configuring an <code>indexSpec</code> with front coding.</td><td>See <a href="#ingestion-ingestion-spec--indexspec"><code>indexSpec</code></a>.</td></tr><tr><td><code>durableShuffleStorage</code></td><td>SELECT, INSERT, REPLACE

Whether to use durable storage for shuffle mesh. To use this feature, configure the durable storage at the server level using <code>druid.msq.intermediate.storage.enable=true</code>). If these properties are not configured, any query with the context variable <code>durableShuffleStorage=true</code> fails with a configuration error.

</td><td><code>false</code></td></tr><tr><td><code>faultTolerance</code></td><td>SELECT, INSERT, REPLACE

 Whether to turn on fault tolerance mode or not. Failed workers are retried based on <a href="#multi-stage-query-reference--limits">Limits</a>. Cannot be used when <code>durableShuffleStorage</code> is explicitly set to false.</td><td><code>false</code></td></tr><tr><td><code>selectDestination</code></td><td>SELECT

 Controls where the final result of the select query is written.
Use <code>taskReport</code>(the default) to write select results to the task report.  This is not scalable since task reports size explodes for large results Use <code>durableStorage</code> to write results to durable storage location. For large results sets, its recommended to use <code>durableStorage</code> . To configure durable storage see <a href="#multi-stage-query-reference--durable-storage"><code>this</code></a> section.</td><td><code>taskReport</code></td></tr><tr><td><code>waitUntilSegmentsLoad</code></td><td>INSERT, REPLACE   If set, the ingest query waits for the generated segments to be loaded before exiting, else the ingest query exits without waiting. The task and live reports contain the information about the status of loading segments if this flag is set. This will ensure that any future queries made after the ingestion exits will include results from the ingestion. The drawback is that the controller task will stall till the segments are loaded.</td><td><code>false</code></td></tr><tr><td><code>includeSegmentSource</code></td><td>SELECT, INSERT, REPLACE

 Controls the sources, which will be queried for results in addition to the segments present on deep storage. Can be <code>NONE</code> or <code>REALTIME</code>. If this value is <code>NONE</code>, only non-realtime (published and used) segments will be downloaded from deep storage. If this value is <code>REALTIME</code>, results will also be included from realtime tasks. <code>REALTIME</code> cannot be used while writing data into the same datasource it is read from.</td><td><code>NONE</code></td></tr><tr><td><code>rowsPerPage</code></td><td>SELECT
...
The number of rows per page to target. The actual number of rows per page may be somewhat higher or lower than this number. In most cases, use the default.
 This property comes into effect only when <code>selectDestination</code> is set to <code>durableStorage</code></td><td>100000</td></tr><tr><td><code>skipTypeVerification</code></td><td>INSERT or REPLACE  During query validation, Druid validates that <a href="#querying-arrays">string arrays</a> and <a href="#querying-multi-value-dimensions">multi-value dimensions</a> are not mixed in the same column. If you are intentionally migrating from one to the other, use this context parameter to disable type validation.  Provide the column list as comma-separated values or as a JSON array in string form.</td><td>empty list</td></tr><tr><td><code>failOnEmptyInsert</code></td><td>INSERT or REPLACE

 When set to false (the default), an INSERT query generating no output rows will be no-op, and a REPLACE query generating no output rows will delete all data that matches the OVERWRITE clause.  When set to true, an ingest query generating no output rows will throw an <code>InsertCannotBeEmpty</code> fault.</td><td><code>false</code></td></tr><tr><td><code>storeCompactionState</code></td><td>REPLACE

 When set to true, a REPLACE query stores as part of each segment's metadata a <code>lastCompactionState</code> field that captures the various specs used to create the segment. Future compaction jobs skip segments whose <code>lastCompactionState</code> matches the desired compaction state. Works the same as <a href="#ingestion-tasks--context-parameters"><code>storeCompactionState</code></a> task context flag.</td><td><code>false</code></td></tr><tr><td><code>removeNullBytes</code></td><td>SELECT, INSERT or REPLACE

 The MSQ engine cannot process null bytes in strings and throws <code>InvalidNullByteFault</code> if it encounters them in the source data. If the parameter is set to true, The MSQ engine will remove the null bytes in string fields when reading the data.</td><td><code>false</code></td></tr><tr><td><code>includeAllCounters</code></td><td>SELECT, INSERT or REPLACE

Whether to include counters that were added in Druid 31 or later. This is a backwards compatibility option that must be set to <code>false</code> during a rolling update from versions prior to Druid 31.</td><td><code>true</code></td></tr><tr><td><code>maxFrameSize</code></td><td>SELECT, INSERT or REPLACE  Size of frames used for data transfer within the MSQ engine. You generally do not need to change this unless you have very large rows.</td><td><code>1000000</code> (1 MB)</td></tr><tr><td><code>maxInputFilesPerWorker</code></td><td>SELECT, INSERT, REPLACE

Maximum number of input files or segments per worker. If a single worker would need to read more than this number of files, the query fails with a <code>TooManyInputFiles</code> error. In this case, you should either increase this limit if your tasks have enough memory to handle more files, add more workers by increasing <code>maxNumTasks</code>, or split your query into smaller queries that process fewer files.</td><td>10,000</td></tr><tr><td><code>maxPartitions</code></td><td>SELECT, INSERT, REPLACE

Maximum number of output partitions for any single stage. For INSERT or REPLACE queries, this controls the maximum number of segments that can be generated. If the query would exceed this limit, it fails with a <code>TooManyPartitions</code> error. You can increase this limit if needed, break your query into smaller queries, or use a larger target segment size (via <code>rowsPerSegment</code>).</td><td>25,000</td></tr><tr><td><code>maxThreads</code></td><td>SELECT, INSERT or REPLACE  Maximum number of threads to use for processing. This only has an effect if it is greater than zero and less than the default thread count based on system configuration. Otherwise, it is ignored, and workers use the default thread count.</td><td>Not set (use default thread count)</td></tr></tbody></table>

Joins in multi-stage queries use one of two algorithms based on what you set the [context parameter](#multi-stage-query-reference--context-parameters) `sqlJoinAlgorithm` to:

- [`broadcast`](#multi-stage-query-reference--broadcast) (default)
- [`sortMerge`](#multi-stage-query-reference--sort-merge).
...
appropriately. See the advantages and the drawbacks for the [broadcast](#multi-stage-query-reference--broadcast) and the [sort-merge](#multi-stage-query-reference--sort-merge) join to
...
[joins are executed with native queries](#querying-query-execution--join).
...
Together, all of these non-base leaf inputs must not exceed the [limit on broadcast table footprint](#multi-stage-query-reference--limits). There
...
You can use the sort-merge join algorithm to make queries more scalable at the cost of performance. If your goal is performance, consider [broadcast joins](#multi-stage-query-reference--broadcast). There are various scenarios where broadcast join would return a [`BroadcastTablesTooLarge`](#multi-stage-query-reference--error-codes) error, but a sort-merge join would succeed.
...
- There is a limit on the amount of data associated with each individual key. If *both* sides of the join exceed this limit, the query returns a [`TooManyRowsWithSameKey`](#multi-stage-query-reference--error-codes) error. If only one side exceeds the limit, the query does not return this error.
...
SQL-based ingestion supports using durable storage to store intermediate files temporarily. Enabling it can improve reliability. For more information, see [Durable storage](#operations-durable-storage).
...
| Parameter | Required | Description | Default |
| --- | --- | --- | --- |
| `druid.msq.intermediate.storage.enable` | Yes | Whether to enable durable storage for the cluster. Set it to true to enable durable storage. For more information about enabling durable storage, see [Durable storage](#operations-durable-storage). | false |
...
Knowing the limits for the MSQ task engine can help you troubleshoot any [errors](#multi-stage-query-reference--error-codes) that you encounter. Many of the errors occur as a result of reaching a limit.

The following table lists query limits:

| Limit | Value | Error if exceeded |
| --- | --- | --- |
| Size of an individual row written to a frame. Row size when written to a frame may differ from the original row size. Configurable with [`maxFrameSize`](#multi-stage-query-reference--context). | 1 MB | `RowTooLarge` |
| Number of segment-granular time chunks encountered during ingestion. | 5,000 | `TooManyBuckets` |
| Number of input files/segments per worker. Configurable with [`maxInputFilesPerWorker`](#multi-stage-query-reference--context). | 10,000 | `TooManyInputFiles` |
| Number of output partitions for any one stage. Number of segments generated during ingestion. Configurable with [`maxPartitions`](#multi-stage-query-reference--context). | 25,000 | `TooManyPartitions` |
...
| Maximum memory occupied by broadcasted tables. | 30% of each [processor memory bundle](#multi-stage-query-concepts--memory-usage). | `BroadcastTablesTooLarge` |
...
<table><thead><tr><th>Code</th><th>Meaning</th><th>Additional fields</th></tr></thead><tbody><tr><td><code>BroadcastTablesTooLarge</code></td><td>The size of the broadcast tables used in the right hand side of the join exceeded the memory reserved for them in a worker task.  Try increasing the peon memory or reducing the size of the broadcast tables.</td><td><code>maxBroadcastTablesSize</code>: Memory reserved for the broadcast tables, measured in bytes.</td></tr><tr><td><code>Canceled</code></td><td>The query was canceled. Common reasons for cancellation:

<ul><li>User-initiated shutdown of the controller task via the <code>/druid/indexer/v1/task/{taskId}/shutdown</code> API.</li><li>Restart or failure of the server process that was running the controller task.</li></ul></td><td></td></tr><tr><td><code>CannotParseExternalData</code></td><td>A worker task could not parse data from an external datasource.</td><td><code>errorMessage</code>: More details on why parsing failed.</td></tr><tr><td><code>ColumnNameRestricted</code></td><td>The query uses a restricted column name.</td><td><code>columnName</code>: The restricted column name.</td></tr><tr><td><code>ColumnTypeNotSupported</code></td><td>The column type is not supported. This can be because:

<ul><li>Support for writing or reading from a particular column type is not supported.</li><li>The query attempted to use a column type that is not supported by the frame format. This occurs with ARRAY types, which are not yet implemented for frames.</li></ul></td><td><code>columnName</code>: The column name with an unsupported type.

<code>columnType</code>: The unknown column type.</td></tr><tr><td><code>DruidException</code></td><td>An error occurred with structured information such as code, persona, and category. It does not fall into one of the other categories.</td><td><code>druidErrorCode</code>: More specific error code.

<code>persona</code>: Target persona.

<code>category</code>: Error category.

<code>context</code>: Additional context.</td></tr><tr><td><code>DurableStorageConfiguration</code></td><td>Durable storage mode was enabled but is not configured correctly. Check the documentation on how to enable durable storage mode.</td><td><code>message</code>: Details on the configuration error.</td></tr><tr><td><code>InsertCannotAllocateSegment</code></td><td>The controller task could not allocate a new segment ID due to conflict with existing segments or pending segments. Common reasons for such conflicts:

<ul><li>Attempting to mix different granularities in the same intervals of the same datasource.</li><li>Prior ingestions that used non-extendable shard specs.</li></ul>

 Use REPLACE to overwrite the existing data or if the error contains the <code>allocatedInterval</code> then alternatively rerun the INSERT job with the mentioned granularity to append to existing data. Note that it might not always be possible to append to the existing data using INSERT and can only be done if <code>allocatedInterval</code> is present.</td><td><code>dataSource</code>
<code>interval</code>: The interval for the attempted new segment allocation.

 <code>allocatedInterval</code>: The incorrect interval allocated by the overlord. It can be null</td></tr><tr><td><code>InsertCannotBeEmpty</code></td><td>An INSERT or REPLACE query did not generate any output rows when <code>failOnEmptyInsert</code> query context is set to true. <code>failOnEmptyInsert</code> defaults to false, so an INSERT query generating no output rows will be no-op, and a REPLACE query generating no output rows will delete all data that matches the OVERWRITE clause.</td><td><code>dataSource</code></td></tr><tr><td><code>InsertLockPreempted</code></td><td>An INSERT or REPLACE query was canceled by a higher-priority ingestion job, such as a real-time ingestion task.</td><td></td></tr><tr><td><code>InsertTimeNull</code></td><td>An INSERT or REPLACE query encountered a null timestamp in the <code>__time</code> field.

This can happen due to using an expression like <code>TIME_PARSE(timestamp) AS __time</code> with a timestamp that cannot be parsed. (<a href="#querying-sql-scalar--date-and-time-functions"><code>TIME_PARSE</code></a> returns null when it cannot parse a timestamp.) In this case, try parsing your timestamps using a different function or pattern. Or, if your timestamps may genuinely be null, consider using <a href="#querying-sql-scalar--other-scalar-functions"><code>COALESCE</code></a> to provide a default value. One option is <a href="#querying-sql-scalar--date-and-time-functions"><code>CURRENT_TIMESTAMP</code></a>, which represents the start time of the job.</td><td></td></tr><tr><td><code>InsertTimeOutOfBounds</code></td><td>A REPLACE query generated a timestamp outside the bounds of the TIMESTAMP parameter for your OVERWRITE WHERE clause.   To avoid this error, verify that the you specified is valid.</td><td><code>interval</code>: time chunk interval corresponding to the out-of-bounds timestamp</td></tr><tr><td><code>InvalidField</code></td><td>An error was encountered while writing a field.</td><td><code>error</code>: Encountered error.

 <code>source</code>: Source for the error.

 <code>rowNumber</code>: Row number (1-indexed) for the error.

 <code>column</code>: Column for the error.</td></tr><tr><td><code>InvalidNullByte</code></td><td>A string column included a null byte. Null bytes in strings are not permitted.</td><td><code>source</code>: The source that included the null byte

 <code>rowNumber</code>: The row number (1-indexed) that included the null byte

 <code>column</code>: The column that included the null byte

 <code>value</code>: Actual string containing the null byte

 <code>position</code>: Position (1-indexed) of occurrence of null byte</td></tr><tr><td><code>QueryNotSupported</code></td><td>QueryKit could not translate the provided native query to a multi-stage query.   This can happen if the query uses features that aren't supported, like GROUPING SETS.</td><td></td></tr><tr><td><code>RowTooLarge</code></td><td>The query tried to process a row that was too large to write to a single frame. See the <a href="#multi-stage-query-reference--limits">Limits</a> table for specific limits on frame size. Note that the effective maximum row size is smaller than the maximum frame size due to alignment considerations during frame writing.</td><td><code>maxFrameSize</code>: The limit on the frame size.</td></tr><tr><td><code>TaskStartTimeout</code></td><td>Unable to launch <code>pendingTasks</code> worker out of total <code>totalTasks</code> workers tasks within <code>timeout</code> seconds of the last successful worker launch.

There may be insufficient available slots to start all the worker tasks simultaneously. Try splitting up your query into smaller chunks using a smaller value of <a href="#multi-stage-query-reference--context-parameters"><code>maxNumTasks</code></a>. Another option is to increase capacity.</td><td><code>pendingTasks</code>: Number of tasks not yet started.

<code>totalTasks</code>: The number of tasks attempted to launch.

<code>timeout</code>: Timeout, in milliseconds, that was exceeded.</td></tr><tr><td><code>TooManyAttemptsForJob</code></td><td>Total relaunch attempt count across all workers exceeded max relaunch attempt limit. See the <a href="#multi-stage-query-reference--limits">Limits</a> table for the specific limit.</td><td><code>maxRelaunchCount</code>: Max number of relaunches across all the workers defined in the <a href="#multi-stage-query-reference--limits">Limits</a> section.

 <code>currentRelaunchCount</code>: current relaunch counter for the job across all workers.

 <code>taskId</code>: Latest task id which failed

 <code>rootErrorMessage</code>: Error message of the latest failed task.</td></tr><tr><td><code>TooManyAttemptsForWorker</code></td><td>Worker exceeded maximum relaunch attempt count as defined in the <a href="#multi-stage-query-reference--limits">Limits</a> section.</td><td><code>maxPerWorkerRelaunchCount</code>: Max number of relaunches allowed per worker as defined in the <a href="#multi-stage-query-reference--limits">Limits</a> section.

 <code>workerNumber</code>: the worker number for which the task failed

 <code>taskId</code>: Latest task id which failed

 <code>rootErrorMessage</code>: Error message of the latest failed task.</td></tr><tr><td><code>TooManyBuckets</code></td><td>Exceeded the maximum number of partition buckets for a stage (5,000 partition buckets).
&lt; br /&gt;Partition buckets are created for each <a href="#multi-stage-query-reference--partitioned-by"><code>PARTITIONED BY</code></a> time chunk for INSERT and REPLACE queries. The most common reason for this error is that your <code>PARTITIONED BY</code> is too narrow relative to your data.</td><td><code>maxBuckets</code>: The limit on partition buckets.</td></tr><tr><td><code>TooManyInputFiles</code></td><td>Exceeded the maximum number of input files or segments per worker (10,000 files or segments).  If you encounter this limit, consider adding more workers, or breaking up your query into smaller queries that process fewer files or segments per query.</td><td><code>numInputFiles</code>: The total number of input files/segments for the stage.

<code>maxInputFiles</code>: The maximum number of input files/segments per worker per stage.

<code>minNumWorker</code>: The minimum number of workers required for a successful run.</td></tr><tr><td><code>TooManyPartitions</code></td><td>Exceeded the maximum number of partitions for a stage (25,000 partitions).  This can occur with INSERT or REPLACE statements that generate large numbers of segments, since each segment is associated with a partition. If you encounter this limit, consider breaking up your INSERT or REPLACE statement into smaller statements that process less data per statement.</td><td><code>maxPartitions</code>: The limit on partitions which was exceeded</td></tr><tr><td><code>TooManyClusteredByColumns</code></td><td>Exceeded the maximum number of clustering columns for a stage (1,500 columns).

This can occur with <code>CLUSTERED BY</code>, <code>ORDER BY</code>, or <code>GROUP BY</code> with a large number of columns.</td><td><code>numColumns</code>: The number of columns requested.

<code>maxColumns</code>: The limit on columns which was exceeded.<code>stage</code>: The stage number exceeding the limit

</td></tr><tr><td><code>TooManyRowsWithSameKey</code></td><td>The number of rows for a given key exceeded the maximum number of buffered bytes on both sides of a join. See the <a href="#multi-stage-query-reference--limits">Limits</a> table for the specific limit. Only occurs when join is executed via the sort-merge join algorithm.</td><td><code>key</code>: The key that had a large number of rows.

<code>numBytes</code>: Number of bytes buffered, which may include other keys.

<code>maxBytes</code>: Maximum number of bytes buffered.</td></tr><tr><td><code>TooManyRowsInAWindow</code></td><td>Too many rows materialized in a single window partition. Try using a window with a higher cardinality <code>PARTITION BY</code> column, changing the query shape, or increasing the limit with the <code>maxRowsMaterializedInWindow</code> query context parameter.</td><td><code>numRows</code>: Number of rows encountered.

<code>maxRows</code>: Maximum number of rows allowed.</td></tr><tr><td><code>TooManySegmentsInTimeChunk</code></td><td>Too many segments requested to be generated in a single time chunk. Try breaking up the query or increase the limit using the <code>maxNumSegments</code> query context parameter.</td><td><code>timeChunk</code>: The time chunk that exceeded the limit.

<code>numSegments</code>: Number of segments requested.

<code>maxNumSegments</code>: Maximum number of segments allowed.

<code>segmentGranularity</code>: The segment granularity.</td></tr><tr><td><code>TooManyColumns</code></td><td>Exceeded the maximum number of columns for a stage (2,000 columns).</td><td><code>numColumns</code>: The number of columns requested.

<code>maxColumns</code>: The limit on columns which was exceeded.</td></tr><tr><td><code>TooManyWarnings</code></td><td>Exceeded the maximum allowed number of warnings of a particular type.</td><td><code>rootErrorCode</code>: The error code corresponding to the exception that exceeded the required limit.

<code>maxWarnings</code>: Maximum number of warnings that are allowed for the corresponding <code>rootErrorCode</code>.</td></tr><tr><td><code>TooManyWorkers</code></td><td>Exceeded the maximum number of simultaneously-running workers. See the <a href="#multi-stage-query-reference--limits">Limits</a> table for more details.</td><td><code>workers</code>: The number of simultaneously running workers that exceeded a hard or soft limit. This may be larger than the number of workers in any one stage if multiple stages are running simultaneously.

<code>maxWorkers</code>: The hard or soft limit on workers that was exceeded. If this is lower than the hard limit (1,000 workers), then you can increase the limit by adding more memory to each task.</td></tr><tr><td><code>NotEnoughMemory</code></td><td>Insufficient memory to launch a stage.</td><td><code>suggestedServerMemory</code>: Suggested number of bytes of memory to allocate to a given process.

<code>serverMemory</code>: The number of bytes of memory available to a single process.

<code>usableMemory</code>: The number of usable bytes of memory for a single process.

<code>serverWorkers</code>: The number of workers running in a single process.

<code>serverThreads</code>: The number of threads in a single process.</td></tr><tr><td><code>NotEnoughTemporaryStorage</code></td><td>Insufficient temporary storage configured to launch a stage. This limit is set by the property <code>druid.indexer.task.tmpStorageBytesPerTask</code>. This property should be increased to the minimum suggested limit to resolve this.</td><td><code>suggestedMinimumStorage</code>: Suggested number of bytes of temporary storage space to allocate to a given process.

<code>configuredTemporaryStorage</code>: The number of bytes of storage currently configured.</td></tr><tr><td><code>WorkerFailed</code></td><td>A worker task failed unexpectedly.</td><td><code>errorMsg</code>
<code>workerTaskId</code>: The ID of the worker task.</td></tr><tr><td><code>WorkerRpcFailed</code></td><td>A remote procedure call to a worker task failed and could not recover.</td><td><code>workerTaskId</code>: the id of the worker task</td></tr><tr><td><code>UnknownError</code></td><td>All other errors.</td><td><code>message</code></td></tr></tbody></table>

---

<a id="multi-stage-query-known-issues"></a>

<!-- source_url: https://druid.apache.org/docs/latest/multi-stage-query/known-issues/ -->
...
- Worker task stage outputs are stored in the working directory given by `druid.indexer.task.baseDir`. Stages that
  ...
  an [UnknownError](#multi-stage-query-reference--error-codes) with a message including "No space left on device".

- `GROUPING SETS` are not implemented. Queries using these features return a
  [QueryNotSupported](#multi-stage-query-reference--error-codes) error.
...
- `INSERT` and `REPLACE` do not support all options available in [ingestion specs](#ingestion-ingestion-spec),
  including the `createBitmapIndex` and `multiValueHandling` [dimension](#ingestion-ingestion-spec--dimension-objects)
  properties, and the `indexSpec` [`tuningConfig`](#ingestion-ingestion-spec--tuningconfig) property.

- The [schemaless dimensions](#ingestion-ingestion-spec--inclusions-and-exclusions)
  feature is not available. All columns and their types must be specified explicitly using the `signature` parameter
  of the [`EXTERN` function](#multi-stage-query-reference--extern-function).
...
- The `metricSpec` field is only supported for certain aggregators. For more information, see [Supported aggregators](#data-management-automatic-compaction--supported-aggregators).
...
<a id="ingestion-streaming"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/streaming/ -->
...
- Apache Kafka through the bundled [Kafka indexing service](#ingestion-kafka-ingestion) extension.
- Amazon Kinesis through the bundled [Kinesis indexing service](#ingestion-kinesis-ingestion) extension.

Each indexing service provides real-time data ingestion with exactly-once stream processing guarantee.
To use either of the streaming ingestion methods, you must first load the associated extension on both the Overlord and the Middle Manager. See [Loading extensions](#configuration-extensions--loading-extensions) for more information.

Streaming ingestion is controlled by a continuously running [supervisor](#ingestion-supervisor).
...
You start a supervisor by submitting a JSON specification, often referred to as the supervisor spec, either though the Druid web console or using the [Supervisor API](#api-reference-supervisor-api).

---

<a id="ingestion-supervisor"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/supervisor/ -->
...
<a id="ingestion-kafka-ingestion"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/kafka-ingestion/ -->
...
<a id="ingestion-kinesis-ingestion"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/kinesis-ingestion/ -->
...
<a id="ingestion-native-batch"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/native-batch/ -->
...
> [!NOTE]
> **info**
> This page describes JSON-based batch ingestion using [ingestion specs](#ingestion-ingestion-spec). For SQL-based batch ingestion using the [`druid-multi-stage-query`](#multi-stage-query) engine, see [SQL-based ingestion](#multi-stage-query). Refer to the [ingestion methods](#ingestion--batch) table to determine which ingestion method is right for you.

---

<a id="ingestion-ingestion-spec"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/ingestion-spec/ -->
...
> first [`flattenSpec`](#ingestion-data-formats--flattenspec) (if any), then [`timestampSpec`](#ingestion-ingestion-spec--timestampspec), then [`transformSpec`](#ingestion-ingestion-spec--transformspec), and finally [`dimensionsSpec`](#ingestion-ingestion-spec--dimensionsspec) and [`metricsSpec`](#ingestion-ingestion-spec--metricsspec). Keep this in mind when writing
> your ingestion spec.

---

<a id="ingestion-schema-design"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/schema-design/ -->
...
<a id="ingestion-concurrent-append-replace"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/concurrent-append-replace/ -->
...
For details on accessing the compaction config in the UI, see [Enable automatic compaction with the web console](#data-management-automatic-compaction--manage-auto-compaction-using-the-web-console).
...
> [!NOTE]
> **Click here to read more about the lock types.**
...
<a id="ingestion-faq"></a>

<!-- source_url: https://druid.apache.org/docs/latest/ingestion/faq/ -->
...
If the number of ingested events seem correct, make sure your query is correctly formed. If you included a `count` aggregator in your ingestion spec, you will need to query for the results of this aggregate with a `longSum` aggregator. Issuing a query with a count aggregator will count the number of Druid rows, which includes [roll-up](#design).

Depending on what `druid.storage.type` is set to, Druid will upload segments to some [Deep Storage](#design-deep-storage). Local disk is used as the default deep storage.
...
2. Repeatedly poll the [Overlord's tasks API](#api-reference-tasks-api) ( `/druid/indexer/v1/task/{taskId}/status`) until your task is shown to be successfully completed.
3. Poll the [Segment Loading by Datasource API](#api-reference-legacy-metadata-api--segment-loading-by-datasource) (`/druid/coordinator/v1/datasources/{dataSourceName}/loadstatus`) with
...
4. Repeatedly poll the [Segment Loading by Datasource API](#api-reference-legacy-metadata-api--segment-loading-by-datasource) (`/druid/coordinator/v1/datasources/{dataSourceName}/loadstatus`) with
   ...
   Note that this workflow only guarantees that the segments are available at the time of the [Segment Loading by Datasource API](#api-reference-legacy-metadata-api--segment-loading-by-datasource) call. Segments can still become missing because of historical process failures or any other reasons afterward.

You can check the [web console](#operations-web-console) to make sure that your segments have actually loaded on [Historical processes](#design-historical). If your segments are not present, check the Coordinator logs for messages about capacity of replication errors. One reason that segments are not downloaded is because Historical processes have maxSizes that are too small, making them incapable of downloading more data. You can change that with (for example):

```text
-Ddruid.segmentCache.locations=[{"path":"/tmp/druid/storageLocation","maxSize":"500000000000"}] 
```

You can use a [segment metadata query](#querying-segmentmetadataquery) for the dimensions and metrics that have been created for your datasource. Make sure that the name of the aggregators you use in your query match one of these metrics. Also make sure that the query interval you specify match a valid time range where data exists.
...
<!-- source_url: https://druid.apache.org/docs/latest/data-management/ -->

<!-- page_index: 62 -->

# Data management

Apache Druid stores data [partitioned by time chunk](#design-storage) in immutable
files called [segments](#design-segments). Data management operations involving replacing, or deleting, these segments include:

- [Updates](#data-management-update) to existing data.
- [Deletion](#data-management-delete) of existing data.
- [Schema changes](#data-management-schema-changes) for new and existing data.
- [Compaction](#data-management-compaction) and [automatic compaction](#data-management-automatic-compaction), which reindex existing data to
  optimize storage footprint and performance.

---

<a id="data-management-update"></a>

<!-- source_url: https://druid.apache.org/docs/latest/data-management/update/ -->
...
<a id="data-management-delete"></a>

<!-- source_url: https://druid.apache.org/docs/latest/data-management/delete/ -->
...
>
> - Can be used only if [segment metadata caching](#configuration--segment-metadata-cache) is enabled on the Overlord.
> - MUST NOT be used if auto-kill of unused segments is already enabled on the Coordinator.

---

<a id="data-management-schema-changes"></a>

<!-- source_url: https://druid.apache.org/docs/latest/data-management/schema-changes/ -->
...
It is sufficient to update your supervisor spec, if using [streaming ingestion](#ingestion--streaming), or to
provide the new schema the next time you do a [batch ingestion](#ingestion--batch). This is made possible by
the fact that each [segment](#design-segments), at the time it is created, stores a
...
previously-ingested data, or drop a column entirely. Druid handles this using [reindexing](#data-management-update), the same method
it uses to handle updates of existing data. Reindexing involves rewriting all affected segments and can be a
time-consuming operation.

---

<a id="data-management-compaction"></a>

<!-- source_url: https://druid.apache.org/docs/latest/data-management/compaction/ -->
...
<a id="data-management-automatic-compaction"></a>

<!-- source_url: https://druid.apache.org/docs/latest/data-management/automatic-compaction/ -->
...
<a id="data-management-manual-compaction"></a>

<!-- source_url: https://druid.apache.org/docs/latest/data-management/manual-compaction/ -->
...
<a id="data-management-cascading-reindexing"></a>

<!-- source_url: https://druid.apache.org/docs/latest/data-management/cascading-reindexing/ -->
...
> [!NOTE]
> **info**
> Cascading reindexing is an experimental feature introduced in Druid 37. Its API may change in future releases. This feature is only for automatic compaction using [compaction supervisors](#data-management-automatic-compaction--auto-compaction-using-compaction-supervisors) with the [MSQ task engine](#data-management-automatic-compaction--use-msq-for-auto-compaction).

---

<a id="querying-sql"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql/ -->
...
> [!NOTE]
> **info**
> Apache Druid supports two query languages: Druid SQL and [native queries](#querying).
> This document describes the SQL language.

---

<a id="querying-sql-functions"></a>
...
<a id="querying-tips-good-queries"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/tips-good-queries/ -->

<!-- page_index: 72 -->

# Tips for writing good queries in Druid

This topic includes tips and examples that can help you investigate and improve query performance and accuracy using [Apache Druid SQL](#querying-sql).
...
Your ability to effectively query your data depends in large part on the way you've ingested and stored the data in Apache Druid. This document assumes that you've followed the best practices described in [Schema design tips and best practices](#ingestion-schema-design--general-tips-and-best-practices) when modeling your data.

If your queries run slower than anticipated, you can use the following tools to investigate query performance issues.

You can configure Druid processes to emit metrics that are essential for monitoring query execution. See [Query metrics](#operations-metrics--query-metrics) for more information.
...
See [Explain plan](#querying-sql--explain-plan) and [Interpreting explain plan output](#querying-sql-translation--interpreting-explain-plan-output) for more information.

You can follow the [Get to know Query view tutorial](#tutorials-tutorial-sql-query-view) to create an example explain plan in the Druid console.
...
See [TopN queries](#querying-topnquery) for more information.
...
The tutorial [Get to know Query view](#tutorials-tutorial-sql-query-view) demonstrates running aggregate queries in the Druid console.
...
Consider whether you can pre-compute a subquery task and store it as a join or make it a part of the datasource. See [Datasources: join](#querying-datasource--join) and [SQL query translation: Joins](#querying-sql-translation--joins) for more information and examples.

Consider using Timeseries and TopN as alternatives to GroupBy. See [GroupBy queries: alternatives](#querying-groupbyquery--alternatives) for more information.
...
<a id="querying-query-deep-storage"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/query-deep-storage/ -->
...
- You have the centralized datasource schema feature enabled. For more information, see [Centralized datasource schema](#configuration--centralized-datasource-schema-experimental).
...
To manage which segments are kept only in deep storage and which get loaded onto Historical processes, configure [load rules](#operations-rule-configuration--load-rules)
...
Apart from the context parameters mentioned [here](#multi-stage-query-reference--context-parameters) there are additional context parameters for `sql/statements`:

- `executionMode` (required) determines how query results are fetched. Set this to `ASYNC`.
- `selectDestination` (optional) set to `durableStorage` instructs Druid to write the results of SELECT queries to durable storage. For result sets with more than 3000 rows, it is highly recommended to use `durableStorage`. Note that this requires you to have [durable storage for MSQ enabled](#operations-durable-storage).
...
Note that you can also submit context parameters using [SET](#querying-sql--set). For example:
...
```json
{"queryId": "query-ALPHANUMBERIC-STRING","state": "ACCEPTED","createdAt": CREATION_TIMESTAMP,"schema": [{"name": COLUMN_NAME,"type": COLUMN_TYPE,"nativeType": COLUMN_TYPE },...],"durationMs": DURATION_IN_MS,}
```
...
```json
{..."result": {"numTotalRows": INTEGER,"totalSizeInBytes": INTEGER,"dataSource": "__query_select","sampleRecords": [[RECORD_1,RECORD_2,...] ],"pages": [{"numRows": INTEGER,"sizeInBytes": INTEGER,"id": INTEGER_PAGE_NUMBER} ...]}}
```
...
- For `resultFormat` the following options are supported `arrayLines`,`objectLines`,`array`,`object`, and `csv`. Default value is `object`. More documentation present [here](#api-reference-sql-api--request-body).
...
- [Query from deep storage tutorial](#tutorials-tutorial-query-deep-storage)
- [Query from deep storage API reference](#api-reference-sql-api--query-from-deep-storage)

---

<a id="querying-sql-data-types"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-data-types/ -->
...
<a id="querying-sql-operators"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-operators/ -->
...
<a id="querying-sql-scalar"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-scalar/ -->
...
<a id="querying-sql-aggregations"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-aggregations/ -->
...
<a id="querying-sql-window-functions"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-window-functions/ -->
...
> [!NOTE]
> **info**
> Apache Druid supports two query languages: [Druid SQL](#querying-sql) and [native queries](#querying).
> This document describes the SQL language.

---

<a id="querying-sql-array-functions"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-array-functions/ -->
...
<a id="querying-sql-multivalue-string-functions"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-multivalue-string-functions/ -->
...
<a id="querying-sql-json-functions"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-json-functions/ -->

<!-- page_index: 81 -->

# SQL JSON functions

Druid supports nested columns, which provide optimized storage and indexes for nested data structures. See [Nested columns](#querying-nested-columns) for more information.
...
<a id="querying-sql-query-context"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-query-context/ -->
...
<a id="querying-sql-metadata-tables"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-metadata-tables/ -->
...
<a id="querying-sql-translation"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sql-translation/ -->
...
<!-- source_url: https://druid.apache.org/docs/latest/querying/ -->
...
> it runs a SQL query, refer to the [SQL documentation](#querying-sql-translation--query-types).

---

<a id="querying-query-processing"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/query-processing/ -->
...
1. A query enters the [Broker](#design-broker) service, which identifies the segments with data that may pertain to that query. The list of segments is always pruned by time, and may also be pruned by other attributes depending on how the datasource is partitioned.
2. The Broker identifies which [Historical](#design-historical) and [Middle Manager](#design-middlemanager) services are serving those segments and distributes a rewritten subquery to each of the services.
...
- [Query execution](#querying-query-execution) to learn how Druid services process query statements.

---

<a id="querying-projections"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/projections/ -->
...
To create a projection at ingestion time, use the [`projectionsSpec` block in your ingestion spec](#ingestion-ingestion-spec--projections).
...
To create projections for SQL-based ingestion, you need to also have the [`druid-catalog`](https://druid.apache.org/docs/latest/development/extensions-core/catalog) extension loaded.

You can define a projection for an existing datasource. We recommend using the [`druid-catalog`](https://druid.apache.org/docs/latest/development/extensions-core/catalog) extension, but you can also define the projection in a compaction spec.
...
> **View the payload**
> ```json
> {"type": "datasource","columns": [],"properties": {"segmentGranularity": "PT1H","projections": [{"spec": {"name": "channel_page_hourly_distinct_user_added_deleted","type": "aggregate","virtualColumns": [{"type": "expression","name": "__gran","expression": "timestamp_floor(__time, 'PT1H')","outputType": "LONG"} ],"groupingColumns": [{"type": "long","name": "__gran","multiValueHandling": "SORTED_ARRAY","createBitmapIndex": false },{"type": "string","name": "channel","multiValueHandling": "SORTED_ARRAY","createBitmapIndex": true },{"type": "string","name": "page","multiValueHandling": "SORTED_ARRAY","createBitmapIndex": true} ],"aggregators": [{"type": "HLLSketchBuild","name": "distinct_users","fieldName": "user","lgK": 12,"tgtHllType": "HLL_4" },{"type": "longSum","name": "sum_added","fieldName": "added" },{"type": "longSum","name": "sum_deleted","fieldName": "deleted"}]}}]}}
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>Coordinator duties</li><li>Supervisors</li></ul><div><div><div><div><pre><code><span><span>{</span><span></span> </span><span><span>    </span><span>"type"</span><span>:</span><span> </span><span>"catalog"</span><span>,</span><span></span> </span><span><span>    </span><span>"dataSource"</span><span>:</span><span> YOUR_DATASOURCE</span><span>,</span><span></span> </span><span><span>    </span><span>"engine"</span><span>:</span><span> </span><span>"native"</span><span>,</span><span></span> </span><span><span>    </span><span>"skipOffsetFromLatest"</span><span>:</span><span> </span><span>"PT0H"</span><span>,</span><span></span> </span><span><span>    </span><span>"taskPriority"</span><span>:</span><span> </span><span>25</span><span>,</span><span></span> </span><span><span>    </span><span>"inputSegmentSizeBytes"</span><span>:</span><span> </span><span>100000000000000</span><span>,</span><span></span> </span><span><span>    </span><span>"taskContext"</span><span>:</span><span> </span><span>null</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>{</span><span></span> </span><span><span>    </span><span>"type"</span><span>:</span><span> </span><span>"autocompact"</span><span>,</span><span></span> </span><span><span>    </span><span>"spec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>      </span><span>"type"</span><span>:</span><span> </span><span>"catalog"</span><span>,</span><span></span> </span><span><span>      </span><span>"dataSource"</span><span>:</span><span> YOUR_DATASOURCE</span><span>,</span><span></span> </span><span><span>      </span><span>"engine"</span><span>:</span><span> </span><span>"native"</span><span>,</span><span></span> </span><span><span>      </span><span>"skipOffsetFromLatest"</span><span>:</span><span> </span><span>"PT0H"</span><span>,</span><span></span> </span><span><span>      </span><span>"taskPriority"</span><span>:</span><span> </span><span>25</span><span>,</span><span></span> </span><span><span>      </span><span>"inputSegmentSizeBytes"</span><span>:</span><span> </span><span>100000000000000</span><span>,</span><span></span> </span><span><span>      </span><span>"taskContext"</span><span>:</span><span> </span><span>null</span><span></span> </span><span><span>    </span><span>}</span><span>,</span><span></span> </span><span><span>    </span><span>"suspended"</span><span>:</span><span> </span><span>true</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>

---

<a id="querying-query-execution"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/query-execution/ -->
...
> [!NOTE]
> **info**
> This document describes how Druid executes [native queries](#querying), but since [Druid SQL](#querying-sql) queries
> are translated to native queries, this document applies to the SQL runtime as well. Refer to the SQL
> [Query translation](#querying-sql-translation) page for information about how SQL queries are translated to native
> queries.

---

<a id="querying-dart"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/dart/ -->

<!-- page_index: 89 -->

# SQL queries using the Dart engine
...
<div class="tabs-container tabList__CuJ"><ul><li>SET</li><li>Context block</li></ul><div><div><p>As part of your query using <code>SET engine = 'msq-dart'</code>:</p><div><div><pre><code><span><span>{</span><span></span> </span><span><span></span><span>"query"</span><span>:</span><span>"SET \"engine\"='msq-dart';\nSELECT\n  user,\n  commentLength,\n  COUNT(*) AS \"COUNT\"\nFROM \"wikipedia\"\nGROUP BY 1, 2\nORDER BY 2 DESC"</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div></div><div><p>As part of a <code>context</code> block:</p><div><div><pre><code><span><span>{</span><span></span> </span><span><span>  </span><span>"query"</span><span>:</span><span> </span><span>"SELECT\n  user,\n  commentLength,\n  COUNT(*) AS \"COUNT\"\nFROM \"wikipedia\"\nGROUP BY 1, 2\nORDER BY 2 DESC"</span><span>,</span><span></span> </span><span><span>  </span><span>"context"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>    </span><span>"engine"</span><span>:</span><span> </span><span>"msq-dart"</span><span></span> </span><span><span>  </span><span>}</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<a id="querying-troubleshooting"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/troubleshooting/ -->
...
2. The data servers started to push data but paused for longer than the maximum idle time such as due to [Broker backpressure](#operations-basic-cluster-tuning--broker-backpressure).
...
- Increase the max idle time for the web server.
  ...
  See [Configuration reference](#configuration) for more information on configuring the server.
- If the timeout occurs because the data servers have not pushed any results to the Broker, consider optimizing data server performance. Significant slowdown in the data servers may be a result of spilling too much data to disk in [groupBy queries](#querying-groupbyquery--performance-tuning-for-groupby), large [`IN` filters](#querying-filters--in-filter) in the query, or an under scaled cluster. Analyze your [Druid query metrics](#operations-metrics--query-metrics) to determine the bottleneck.
...
<a id="querying-datasource"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/datasource/ -->
...
> [!NOTE]
> **info**
> Performance tip: Lookups can be joined with a base table either using an explicit [join](#querying-datasource--join), or by using the
> SQL [`LOOKUP` function](#querying-sql-scalar--string-functions).
...
<a id="querying-joins"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/joins/ -->
...
1. [Join](#querying-datasource--join) operators. These are available using a [join datasource](#querying-datasource--join) in native
   queries, or using the [JOIN operator](#querying-sql) in Druid SQL. Refer to the
   [join datasource](#querying-datasource--join) documentation for information about how joins work in Druid native queries,
   or the [multi-stage query join documentation](#multi-stage-query-reference--joins) for information about how joins
   work in multi-stage query tasks.
2. [Query-time lookups](#querying-lookups), simple key-to-value mappings. These are preloaded on all servers that are involved
   in queries and can be accessed with or without an explicit join operator. Refer to the [lookups](#querying-lookups)
...
<a id="querying-lookups"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/lookups/ -->
...
> [!NOTE]
> **info**
> [Multi-value dimensions](#querying-multi-value-dimensions) (MVDs) are not supported as keys in lookups. For example, to map the MVD `["A", "B", "C"]` to the value `x` in your lookup, flatten the MVD and map each element of the MVD to the value. Your lookup will have separate key-value pairs for each element of the MVD: `"A": "x"`, `"B": "x"`, and `"C": "x"`.

---

<a id="querying-lookups-cached-global"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/lookups-cached-global/ -->
...
> [dynamic configuration](#querying-lookups--configuration).

---

<a id="querying-kafka-extraction-namespace"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/kafka-extraction-namespace/ -->

<!-- page_index: 95 -->

# Apache Kafka Lookups

To use this Apache Druid extension, [include](#configuration-extensions--loading-extensions) `druid-lookups-cached-global` and `druid-kafka-extraction-namespace` in the extensions load list.
...
| `isOneToOne` | The map is a one-to-one (see [Lookup DimensionSpecs](#querying-dimensionspecs)) | No | `false` |
...
The extractor works by consuming the configured Kafka topic from the beginning, and appending every record to an internal map. The key of the Kafka record is used as they key of the map, and the payload of the record is used as the value. At query time, a lookup can be used to transform the key into the associated value. See [lookups](#querying-lookups) for how to configure and use lookups in a query. Keys and values are both stored as strings by the lookup extractor.
...
<a id="querying-multi-value-dimensions"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/multi-value-dimensions/ -->
...
It is important to be aware that multi-value dimensions are distinct from [array types](#querying-arrays). While array types behave like standard SQL arrays, multi-value dimensions do not. This document describes the behavior of multi-value dimensions, and some additional details can be found in the [SQL data type documentation](#querying-sql-data-types--multi-value-strings-behavior).
...
[segments documentation](#design-segments--multi-value-columns). Examples in this document
are in the form of both [SQL](#querying-sql) and [native Druid queries](#querying). Refer to the [Druid SQL documentation](#querying-sql-multivalue-string-functions) for details
...
When using native [batch](#ingestion-native-batch) or streaming ingestion such as with [Apache Kafka](#ingestion-kafka-ingestion), the Druid web console data loader can detect multi-value dimensions and configure the `dimensionsSpec` accordingly.
...
```text
"dimensions": [{"type": "string","name": "label" },{"type": "string","name": "tags","multiValueHandling": "SORTED_ARRAY","createBitmapIndex": true} ],
```
...
See [Dimension Objects](#ingestion-ingestion-spec--dimension-objects) for information on configuring multi-value handling.
...
All query types, as well as [filtered aggregators](#querying-aggregations--filtered-aggregator), can filter on multi-value
...
```text
{"type": "and","fields": [{"type": "selector","dimension": "tags","value": "t1" },{"type": "selector","dimension": "tags","value": "t3"}]}
```
...
When grouping on a multi-value dimension with SQL or a native [topN](#querying-topnquery) or [groupBy](#querying-groupbyquery) queries, *all* values
...
If you only need to include values that match your filter, you can use the SQL functions [`MV_FILTER_ONLY`/`MV_FILTER_NONE`](#querying-sql-multivalue-string-functions), [filtered virtual column](#querying-virtual-columns--list-filtered-virtual-column), or [filtered dimensionSpec](#querying-dimensionspecs--filtered-dimensionspecs). This can also improve performance.
...
See [GroupBy querying](#querying-groupbyquery) for details.
...
See [query filters](#querying-filters) for details of selector query filter.
...
applied on the row before explosion. For multi-value dimensions, a filter for value "t3" would match row1 and row2, after which exploding is done. For multi-value dimensions, a query filter matches a row if any individual value inside
...
See filtered `dimensionSpecs` in [dimensionSpecs](#querying-dimensionspecs--filtered-dimensionspecs) for details.
...
Note that, for groupBy queries, you could get similar result with a [having spec](#querying-having) but using a filtered
...
[query context](#querying-query-context-reference). In this mode, the groupBy engine will return an error instead of completing the query. This is a safety
...
Avoid confusing string arrays with [multi-value dimensions](#querying-multi-value-dimensions). Arrays and multi-value dimensions are stored in different column types, and query behavior is different. You can use the functions `MV_TO_ARRAY` and `ARRAY_TO_MV` to convert between the two if needed. In general, we recommend using arrays whenever possible, since they are a newer and more powerful feature and have SQL compliant behavior.

Use care during ingestion to ensure you get the type you want.

To get arrays when performing an ingestion using JSON ingestion specs, such as [native batch](#ingestion-native-batch) or streaming ingestion such as with [Apache Kafka](#ingestion-kafka-ingestion), use dimension type `auto` or enable `useSchemaDiscovery`. When performing a [SQL-based ingestion](#multi-stage-query), write a query that generates arrays. Arrays may contain strings or numbers.

To get multi-value dimensions when performing an ingestion using JSON ingestion specs, use dimension type `string` and do not enable `useSchemaDiscovery`. When performing a [SQL-based ingestion](#multi-stage-query), wrap arrays in [`ARRAY_TO_MV`](#querying-multi-value-dimensions--sql-based-ingestion). Multi-value dimensions can only contain strings.
...
<a id="querying-arrays"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/arrays/ -->

<!-- page_index: 97 -->

# Arrays

Apache Druid supports SQL standard `ARRAY` typed columns for `VARCHAR`, `BIGINT`, and `DOUBLE` types (native types `ARRAY<STRING>`, `ARRAY<LONG>`, and `ARRAY<DOUBLE>`). Other more complicated ARRAY types must be stored in [nested columns](#querying-nested-columns). Druid ARRAY types are distinct from [multi-value dimension](#querying-multi-value-dimensions), which have significantly different behavior than standard arrays.

This document describes inserting, filtering, and grouping behavior for `ARRAY` typed columns.
Refer to the [Druid SQL data type documentation](#querying-sql-data-types--arrays) and [SQL array function reference](#querying-sql-array-functions) for additional details
...
When using native [batch](#ingestion-native-batch) or streaming ingestion such as with [Apache Kafka](#ingestion-kafka-ingestion), arrays can be ingested using the [`"auto"`](#ingestion-ingestion-spec--dimension-objects) type dimension schema which is shared with [type-aware schema discovery](#ingestion-schema-design--type-aware-schema-discovery).
...
```text
"dimensions": [{"type": "auto","name": "label" },{"type": "auto","name": "arrayString" },{"type": "auto","name": "arrayLong" },{"type": "auto","name": "arrayDouble"} ],
```

Arrays can be inserted with [SQL-based ingestion](#multi-stage-query).
...
When `arrayIngestMode` is `mvd` (legacy), SQL `VARCHAR ARRAY` are implicitly wrapped in [`ARRAY_TO_MV`](#querying-sql-functions--array_to_mv).
This causes them to be stored as [multi-value strings](#querying-multi-value-dimensions), using the same `STRING` column type
...
| --- | --- | --- |
| `VARCHAR ARRAY` | `ARRAY<STRING>` | [multi-value `STRING`](#querying-multi-value-dimensions) |
...
[multi-value strings](#querying-multi-value-dimensions).
...
All query types, as well as [filtered aggregators](#querying-aggregations--filtered-aggregator), can filter on array typed columns. Filters follow these rules for array types:

- All filters match against the entire array value for the row
- Native value filters like [equality](#querying-filters--equality-filter) and [range](#querying-filters--range-filter) match on entire array values, as do SQL constructs that plan into these native filters
- The [`IS NULL`](#querying-filters--null-filter) filter will match rows where the entire array value is null
- [Array specific functions](#querying-sql-array-functions) like `ARRAY_CONTAINS` and `ARRAY_OVERLAP` follow the behavior specified by those functions
...
When grouping on an array with SQL or a native [groupBy query](#querying-groupbyquery), grouping follows standard SQL behavior and groups on the entire array as a single value. The [`UNNEST`](#querying-sql--unnest) function allows grouping on the individual array elements.
...
To get multi-value dimensions when performing an ingestion using JSON ingestion specs, use dimension type `string` and do not enable `useSchemaDiscovery`. When performing a [SQL-based ingestion](#multi-stage-query), wrap arrays in [`ARRAY_TO_MV`](#querying-multi-value-dimensions--sql-based-ingestion), which ensures you get multi-value dimensions. Multi-value dimensions can only contain strings.
...
<a id="querying-nested-columns"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/nested-columns/ -->
...
<a id="querying-multitenancy"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/multitenancy/ -->
...
for each tenant, or can share one or more datasources between tenants using a "tenant\_id" dimension. When deciding
...
If your multitenant cluster uses shared datasources, most of your queries will likely filter on a "tenant\_id"
dimension. These sorts of queries perform best when data is well-partitioned by tenant. There are a few ways to
accomplish this.

With batch indexing, you can use [multi-dimension partitioning](#ingestion-native-batch--multi-dimension-range-partitioning)
to partition your data by tenant\_id. Druid always partitions by time first, but the secondary partition within each
time bucket will be on tenant\_id.

With realtime indexing, you'd do this by tweaking the stream you send to Druid. For example, if you're using Kafka then
you can have your Kafka producer partition your topic by a hash of tenant\_id.
...
can be configured into [tiers](#operations-rule-configuration), and [rules](#operations-rule-configuration)
...
Druid uses a [segment](#design-segments) as its fundamental unit of computation. Processes scan segments in parallel and a given process can scan `druid.processing.numThreads` concurrently. You can add more cores to a cluster to process more data in parallel and increase performance. Size your Druid segments such that any computation over any given segment should complete in at most 500ms. Use the [`query/segment/time`](#operations-metrics--historical) metric to monitor computation times.
...
Druid queries can optionally set a `priority` flag in the [query context reference](#querying-query-context-reference). Queries known to be
slow (download or reporting style queries) can be de-prioritized and more interactive queries can have higher priority.

Broker processes can also be dedicated to a given tier. For example, one set of Broker processes can be dedicated to fast interactive queries, and a second set of Broker processes can be dedicated to slower reporting queries. Druid also provides a [Router](#design-router)
process that can route queries to different Brokers based on various query parameters (datasource, interval, etc.).

---

<a id="querying-caching"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/caching/ -->
...
<a id="querying-using-caching"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/using-caching/ -->

<!-- page_index: 101 -->

# Using query caching

This topic covers how to configure services to populate and use the Druid query caches. For a conceptual overview and use cases, see [Query caching](#querying-caching). For information on how to configure the caching mechanism, see [Cache configuration](#configuration--cache-configuration).
...
See [Historical caching](#configuration--historical-caching) for a description of all available Historical cache configurations.
...
See [Peon caching](#configuration--peon-caching) and [Indexer caching](#configuration--indexer-caching) for a description of all available task executor service caching options.
...
See [Broker caching](#configuration--broker-caching) for a description of all available Broker cache configurations.

As long as the service is set to populate the cache, you can set cache options for individual queries in the [query context](#querying-query-context-reference). For example, you can send a POST request to the Druid SQL API and include the context as a JSON object:
...
In this example the user has set `populateCache` to `false` to avoid filling the result cache with results for segments that are over a year old. For more information, see [Druid SQL client APIs](#api-reference-sql-api).

You can also use the SET command to specify cache options directly within your SQL query string. For more information, see [SET](#querying-sql--set).

See the following topics for more information:

- [Query caching](#querying-caching) for an overview of caching.
- [Query context reference](#querying-query-context-reference) for more details about query context parameters.
- [Cache configuration](#configuration--cache-configuration) for information about different cache types and additional configuration options.

---

<a id="querying-query-context"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/query-context/ -->
...
<a id="querying-query-context-reference"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/query-context-reference/ -->
...
To learn how to set the query context, see [Set query context](#querying-query-context).

For reference on query context parameters specific to Druid SQL, visit [SQL query context](#querying-sql-query-context).
...
See the list of [GroupBy query context](#querying-groupbyquery--advanced-configurations) parameters available on the groupBy
query page.

The GroupBy and Timeseries query types can run in *vectorized* mode, which speeds up query execution by processing
...
- All virtual columns must offer vectorized implementations. Currently for expression virtual columns, support for vectorization is decided on a per expression basis, depending on the type of input and the functions used by the expression. See the currently supported list in the [expression documentation](#querying-math-expr--vectorization-support).
...
- Only [table datasources](#querying-datasource--table) (not joins, subqueries, lookups, or inline datasources).
...
- [Set query context](#querying-query-context) to learn how to configure query context parameters.
- [SQL query context](#querying-sql-query-context) for query context parameters specific to Druid SQL.
...
<a id="querying-timeseriesquery"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/timeseriesquery/ -->
...
> [SQL documentation](#querying-sql-translation--query-types).

---

<a id="querying-topnquery"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/topnquery/ -->
...
<a id="querying-groupbyquery"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/groupbyquery/ -->
...
<a id="querying-scan-query"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/scan-query/ -->
...
<a id="querying-searchquery"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/searchquery/ -->
...
<a id="querying-timeboundaryquery"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/timeboundaryquery/ -->
...
<a id="querying-segmentmetadataquery"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/segmentmetadataquery/ -->
...
> its [metadata tables](#querying-sql-metadata-tables).

---

<a id="querying-datasourcemetadataquery"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/datasourcemetadataquery/ -->
...
<a id="querying-filters"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/filters/ -->
...
> [SQL documentation](#querying-sql-scalar).

---

<a id="querying-granularities"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/granularities/ -->
...
> [SQL documentation](#querying-sql-scalar--date-and-time-functions).

---

<a id="querying-dimensionspecs"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/dimensionspecs/ -->
...
<a id="querying-aggregations"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/aggregations/ -->
...
> This document describes the native
> language. For information about aggregators available in SQL, refer to the
> [SQL documentation](#querying-sql-aggregations).

---

<a id="querying-post-aggregations"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/post-aggregations/ -->
...
<a id="querying-math-expr"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/math-expr/ -->
...
> [!NOTE]
> **info**
> Apache Druid supports two query languages: [native queries](#querying) and [Druid SQL](#querying-sql).
> This document describes the native language. For information about functions available in SQL, refer to the
> [SQL documentation](#querying-sql-scalar).

---

<a id="querying-having"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/having/ -->
...
<a id="querying-limitspec"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/limitspec/ -->
...
> Apache Druid supports two query languages: [Druid SQL](#querying-sql) and [native queries](#querying).
> This document describes the native
> language. For information about sorting in SQL, refer to the [SQL documentation](#querying-sql--order-by).

---

<a id="querying-topnmetricspec"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/topnmetricspec/ -->
...
<a id="querying-sorting-orders"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/sorting-orders/ -->
...
<a id="querying-virtual-columns"></a>

<!-- source_url: https://druid.apache.org/docs/latest/querying/virtual-columns/ -->
...
<!-- source_url: https://druid.apache.org/docs/latest/querying/geo/ -->
...
<!-- source_url: https://druid.apache.org/docs/latest/api-reference/ -->
...
- [Druid SQL queries](#api-reference-sql-api) to submit SQL queries using the Druid SQL API.
- [SQL-based ingestion](#api-reference-sql-ingestion-api) to submit SQL-based batch ingestion requests.
- [JSON querying](#api-reference-json-querying-api) to submit JSON-based native queries.
- [Tasks](#api-reference-tasks-api) to manage data ingestion operations.
- [Supervisors](#api-reference-supervisor-api) to manage supervisors for data ingestion lifecycle and data processing.
- [Retention rules](#api-reference-retention-rules-api) to define and manage data retention rules across datasources.
- [Data management](#api-reference-data-management-api) to manage data segments.
- [Automatic compaction](#api-reference-automatic-compaction-api) to optimize segment sizes after ingestion.
- [Lookups](#api-reference-lookups-api) to manage and modify key-value datasources.
- [Service status](#api-reference-service-status-api) to monitor components within the Druid cluster.
- [Dynamic configuration](#api-reference-dynamic-configuration-api) to configure the behavior of the Coordinator and Overlord processes.
- [Legacy metadata](#api-reference-legacy-metadata-api) to retrieve datasource metadata.

- [SQL JDBC driver](#api-reference-sql-jdbc) to connect to Druid and make Druid SQL queries using the Avatica JDBC driver.

---

<a id="api-reference-sql-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/sql-api/ -->
...
<a id="api-reference-sql-ingestion-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/sql-ingestion-api/ -->
...
<a id="api-reference-json-querying-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/json-querying-api/ -->

<!-- page_index: 127 -->

# JSON querying API

This topic describes the API endpoints to submit JSON-based [native queries](#querying) to Apache Druid.
...
- `dataSource`: A string or object defining the source of data to query. The most common value is the name of the datasource to query. For more information, see [Datasources](#querying-datasource).

For additional properties based on your query type or use case, see [available native queries](#querying--available-queries).
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>400 BAD REQUEST</li></ul><div><div><p><em>Successfully submitted query</em></p></div><div><p><em>Error thrown due to bad query. Returns a JSON object detailing the error with the following format:</em></p><div><div><pre><code><span><span>{</span><span></span> </span><span><span>    </span><span>"error"</span><span>:</span><span> </span><span>"A well-defined error code."</span><span>,</span><span></span> </span><span><span>    </span><span>"errorMessage"</span><span>:</span><span> </span><span>"A message with additional details about the error."</span><span>,</span><span></span> </span><span><span>    </span><span>"errorClass"</span><span>:</span><span> </span><span>"Class of exception that caused this error."</span><span>,</span><span></span> </span><span><span>    </span><span>"host"</span><span>:</span><span> </span><span>"The host on which the error occurred."</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div><p>For more information on possible error messages, see <a href="#querying--query-execution-failures">query execution failures</a>.</p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/v2?pretty=null" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>  "queryType": "topN",</span> </span><span><span>  "dataSource": "social_media",</span> </span><span><span>  "dimension": "username",</span> </span><span><span>  "threshold": 5,</span> </span><span><span>  "metric": "views",</span> </span><span><span>  "granularity": "all",</span> </span><span><span>  "aggregations": [</span> </span><span><span>    {</span> </span><span><span>      "type": "longSum",</span> </span><span><span>      "name": "views",</span> </span><span><span>      "fieldName": "views"</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "intervals": [</span> </span><span><span>    "2022-01-01T00:00:00.000/2024-01-01T00:00:00.000"</span> </span><span><span>  ]</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/v2?pretty=null HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 336</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "queryType": "topN",</span> </span><span><span>  "dataSource": "social_media",</span> </span><span><span>  "dimension": "username",</span> </span><span><span>  "threshold": 5,</span> </span><span><span>  "metric": "views",</span> </span><span><span>  "granularity": "all",</span> </span><span><span>  "aggregations": [</span> </span><span><span>    {</span> </span><span><span>      "type": "longSum",</span> </span><span><span>      "name": "views",</span> </span><span><span>      "fieldName": "views"</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "intervals": [</span> </span><span><span>    "2022-01-01T00:00:00.000/2024-01-01T00:00:00.000"</span> </span><span><span>  ]</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/v2" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>  "queryType": "groupBy",</span> </span><span><span>  "dataSource": "social_media",</span> </span><span><span>  "dimensions": ["username"],</span> </span><span><span>  "granularity": "all",</span> </span><span><span>  "aggregations": [</span> </span><span><span>    { "type": "doubleSum", "name": "upvoteSum", "fieldName": "upvotes" },</span> </span><span><span>    { "type": "count", "name": "postCount", "fieldName": "post_title" }</span> </span><span><span>  ],</span> </span><span><span>  "postAggregations": [</span> </span><span><span>    {</span> </span><span><span>      "type": "arithmetic",</span> </span><span><span>      "name": "upvoteToPostRatio",</span> </span><span><span>      "fn": "/",</span> </span><span><span>      "fields": [</span> </span><span><span>        { "type": "fieldAccess", "name": "upvoteSum", "fieldName": "upvoteSum" },</span> </span><span><span>        { "type": "fieldAccess", "name": "postCount", "fieldName": "postCount" }</span> </span><span><span>      ]</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "intervals": ["2022-01-01T00:00:00.000/2024-01-01T00:00:00.000"],</span> </span><span><span>  "limitSpec": {</span> </span><span><span>    "type": "default",</span> </span><span><span>    "limit": 1,</span> </span><span><span>    "columns": [</span> </span><span><span>      { "dimension": "upvoteToPostRatio", "direction": "descending" }</span> </span><span><span>    ]</span> </span><span><span>  }</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/v2?pretty=null HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 817</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "queryType": "groupBy",</span> </span><span><span>  "dataSource": "social_media",</span> </span><span><span>  "dimensions": ["username"],</span> </span><span><span>  "granularity": "all",</span> </span><span><span>  "aggregations": [</span> </span><span><span>    { "type": "doubleSum", "name": "upvoteSum", "fieldName": "upvotes" },</span> </span><span><span>    { "type": "count", "name": "postCount", "fieldName": "post_title" }</span> </span><span><span>  ],</span> </span><span><span>  "postAggregations": [</span> </span><span><span>    {</span> </span><span><span>      "type": "arithmetic",</span> </span><span><span>      "name": "upvoteToPostRatio",</span> </span><span><span>      "fn": "/",</span> </span><span><span>      "fields": [</span> </span><span><span>        { "type": "fieldAccess", "name": "upvoteSum", "fieldName": "upvoteSum" },</span> </span><span><span>        { "type": "fieldAccess", "name": "postCount", "fieldName": "postCount" }</span> </span><span><span>      ]</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "intervals": ["2022-01-01T00:00:00.000/2024-01-01T00:00:00.000"],</span> </span><span><span>  "limitSpec": {</span> </span><span><span>    "type": "default",</span> </span><span><span>    "limit": 1,</span> </span><span><span>    "columns": [</span> </span><span><span>      { "dimension": "upvoteToPostRatio", "direction": "descending" }</span> </span><span><span>    ]</span> </span><span><span>  }</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> [{"version": "v1","timestamp": "2022-01-01T00:00:00.000Z","event": {"upvoteSum": 8.0419541E7,"upvoteToPostRatio": 69.53014661762697,"postCount": 1156614,"username": "miette"}}]
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>400 BAD REQUEST</li></ul><div><div><p><em>Successfully retrieved segment information</em></p></div><div><p><em>Error thrown due to bad query. Returns a JSON object detailing the error with the following format:</em></p><div><div><pre><code><span><span>{</span><span></span> </span><span><span>    </span><span>"error"</span><span>:</span><span> </span><span>"A well-defined error code."</span><span>,</span><span></span> </span><span><span>    </span><span>"errorMessage"</span><span>:</span><span> </span><span>"A message with additional details about the error."</span><span>,</span><span></span> </span><span><span>    </span><span>"errorClass"</span><span>:</span><span> </span><span>"Class of exception that caused this error."</span><span>,</span><span></span> </span><span><span>    </span><span>"host"</span><span>:</span><span> </span><span>"The host on which the error occurred."</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div><p>For more information on possible error messages, see <a href="#querying--query-execution-failures">query execution failures</a>.</p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/v2/candidates" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>  "queryType": "topN",</span> </span><span><span>  "dataSource": "social_media",</span> </span><span><span>  "dimension": "username",</span> </span><span><span>  "threshold": 5,</span> </span><span><span>  "metric": "views",</span> </span><span><span>  "granularity": "all",</span> </span><span><span>  "aggregations": [</span> </span><span><span>    {</span> </span><span><span>      "type": "longSum",</span> </span><span><span>      "name": "views",</span> </span><span><span>      "fieldName": "views"</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "intervals": [</span> </span><span><span>    "2022-01-01T00:00:00.000/2024-01-01T00:00:00.000"</span> </span><span><span>  ]</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/v2/candidates HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 336</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "queryType": "topN",</span> </span><span><span>  "dataSource": "social_media",</span> </span><span><span>  "dimension": "username",</span> </span><span><span>  "threshold": 5,</span> </span><span><span>  "metric": "views",</span> </span><span><span>  "granularity": "all",</span> </span><span><span></span> </span><span><span>  "aggregations": [</span> </span><span><span>    {</span> </span><span><span>      "type": "longSum",</span> </span><span><span>      "name": "views",</span> </span><span><span>      "fieldName": "views"</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "intervals": [</span> </span><span><span>    "2020-01-01T00:00:00.000/2024-01-01T00:00:00.000"</span> </span><span><span>  ]</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<a id="api-reference-tasks-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/tasks-api/ -->
...
<a id="api-reference-supervisor-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/supervisor-api/ -->
...
| `state` | String | Generic state of the supervisor. Available states:`UNHEALTHY_SUPERVISOR`, `UNHEALTHY_TASKS`, `PENDING`, `RUNNING`, `SUSPENDED`, `STOPPING`. See [Supervisor reference](#ingestion-supervisor--status-report) for more information. |
| `detailedState` | String | Detailed state of the supervisor. This property contains a more descriptive, implementation-specific state that may provide more insight into the supervisor's activities than the `state` property. See [Apache Kafka ingestion](#ingestion-kafka-ingestion) and [Amazon Kinesis ingestion](#ingestion-kinesis-ingestion) for supervisor-specific states. |
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved array of active supervisor IDs</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
Retrieves an array of active supervisor objects. If there are no active supervisors, it returns an empty array. For reference on the supervisor object properties, see the preceding [table](#api-reference-supervisor-api--supervisor-information).

`GET` `/druid/indexer/v1/supervisor?full`

<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved supervisor objects</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor?full=null"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor?full=null HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
Retrieves an array of objects representing active supervisors and their current state. If there are no active supervisors, it returns an empty array. For reference on the supervisor object properties, see the preceding [table](#api-reference-supervisor-api--supervisor-information).

`GET` `/druid/indexer/v1/supervisor?state=true`

<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved supervisor state objects</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor?state=true"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor?state=true HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully retrieved supervisor spec</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>

---

The following example shows how to retrieve the specification of a supervisor with the name `wikipedia_stream`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/wikipedia_stream"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor/wikipedia_stream HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
For additional information about the status report, see [Supervisor reference](#ingestion-supervisor--status-report).

`GET` `/druid/indexer/v1/supervisor/{supervisorId}/status`

<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully retrieved supervisor status</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>

---

The following example shows how to retrieve the status of a supervisor with the name `social_media`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/status"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor/social_media/status HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li><li>503 SERVICE UNAVAILABLE</li></ul><div><div><p><em>Supervisor is healthy</em></p></div><div><p><em>Invalid supervisor ID</em></p></div><div><p><em>Supervisor is unhealthy</em></p></div></div></div>

---

The following example shows how to retrieve the health report for a supervisor with the name `social_media`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/health"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor/social_media/health HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
Returns a snapshot of the current ingestion row counters for each task being managed by the supervisor, along with moving averages for the row counters. See [Row stats](#ingestion-tasks--row-stats) for more information.

The response is a nested map of task group IDs to task IDs to row stats:

```json
{"<taskGroupId>": {"<taskId>": {"movingAverages": { ... },"totals": { ... }}}}
```

- **taskGroupId** (string): Supervisor task group identifier
- **taskId** (string): Druid indexing task identifier within that group
- **movingAverages** / **totals**: Per-task row stats; see [Row stats](#ingestion-tasks--row-stats) for field definitions

For example, a supervisor with two task groups:

```json
{"0": {"index_kafka_my_datasource_abc123": {"movingAverages": {"buildSegments": {"5m": { "processed": 10.0, "unparseable": 0.0, "thrownAway": 0.0, "processedWithError": 0.0 },"15m": { "processed": 5.0, "unparseable": 0.0, "thrownAway": 0.0, "processedWithError": 0.0 },"1m": { "processed": 12.0, "unparseable": 0.0, "thrownAway": 0.0, "processedWithError": 0.0 }} },"totals": {"buildSegments": { "processed": 2000, "processedWithError": 0, "thrownAway": 0, "unparseable": 0 }}} },"1": {"index_kafka_my_datasource_def456": {"movingAverages": {"buildSegments": {"5m": { "processed": 8.0, "unparseable": 0.0, "thrownAway": 0.0, "processedWithError": 0.0 },"15m": { "processed": 4.0, "unparseable": 0.0, "thrownAway": 0.0, "processedWithError": 0.0 },"1m": { "processed": 9.0, "unparseable": 0.0, "thrownAway": 0.0, "processedWithError": 0.0 }} },"totals": {"buildSegments": { "processed": 1500, "processedWithError": 0, "thrownAway": 0, "unparseable": 0 }}}}}
```
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully retrieved supervisor stats</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/custom_data/stats"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor/custom_data/stats HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved audit history</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/history"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor/history HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully retrieved supervisor audit history</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/wikipedia_stream/history"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor/wikipedia_stream/history HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

**Get last 10 history entries:**

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/wikipedia_stream/history?count=10"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/supervisor/wikipedia_stream/history?count=10 HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
You can define a supervisor spec for [Apache Kafka](#ingestion-kafka-ingestion) or [Amazon Kinesis](#ingestion-kinesis-ingestion) streaming ingestion methods.
...
| `dataSchema` | Object | The schema for the indexing task to use during ingestion. See [`dataSchema`](#ingestion-ingestion-spec--dataschema) for more information. | Yes |
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>415 UNSUPPORTED MEDIA TYPE</li></ul><div><div><p><em>Successfully created a new supervisor or updated an existing supervisor</em></p></div><div><p><em>Request body content type is not in JSON format</em></p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>    "type": "kafka",</span> </span><span><span>    "spec": {</span> </span><span><span>        "ioConfig": {</span> </span><span><span>            "type": "kafka",</span> </span><span><span>            "consumerProperties": {</span> </span><span><span>                "bootstrap.servers": "localhost:9094"</span> </span><span><span>            },</span> </span><span><span>            "topic": "social_media",</span> </span><span><span>            "inputFormat": {</span> </span><span><span>                "type": "json"</span> </span><span><span>            },</span> </span><span><span>            "useEarliestOffset": true</span> </span><span><span>        },</span> </span><span><span>        "tuningConfig": {</span> </span><span><span>            "type": "kafka"</span> </span><span><span>        },</span> </span><span><span>        "dataSchema": {</span> </span><span><span>            "dataSource": "social_media",</span> </span><span><span>            "timestampSpec": {</span> </span><span><span>                "column": "__time",</span> </span><span><span>                "format": "iso"</span> </span><span><span>            },</span> </span><span><span>            "dimensionsSpec": {</span> </span><span><span>                "dimensions": [</span> </span><span><span>                    "username",</span> </span><span><span>                    "post_title",</span> </span><span><span>                    {</span> </span><span><span>                        "type": "long",</span> </span><span><span>                        "name": "views"</span> </span><span><span>                    },</span> </span><span><span>                    {</span> </span><span><span>                        "type": "long",</span> </span><span><span>                        "name": "upvotes"</span> </span><span><span>                    },</span> </span><span><span>                    {</span> </span><span><span>                        "type": "long",</span> </span><span><span>                        "name": "comments"</span> </span><span><span>                    },</span> </span><span><span>                    "edited"</span> </span><span><span>                ]</span> </span><span><span>            },</span> </span><span><span>            "granularitySpec": {</span> </span><span><span>                "queryGranularity": "none",</span> </span><span><span>                "rollup": false,</span> </span><span><span>                "segmentGranularity": "hour"</span> </span><span><span>            }</span> </span><span><span>        }</span> </span><span><span>    }</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 1359</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>    "type": "kafka",</span> </span><span><span>    "spec": {</span> </span><span><span>        "ioConfig": {</span> </span><span><span>            "type": "kafka",</span> </span><span><span>            "consumerProperties": {</span> </span><span><span>                "bootstrap.servers": "localhost:9094"</span> </span><span><span>            },</span> </span><span><span>            "topic": "social_media",</span> </span><span><span>            "inputFormat": {</span> </span><span><span>                "type": "json"</span> </span><span><span>            },</span> </span><span><span>            "useEarliestOffset": true</span> </span><span><span>        },</span> </span><span><span>        "tuningConfig": {</span> </span><span><span>            "type": "kafka"</span> </span><span><span>        },</span> </span><span><span>        "dataSchema": {</span> </span><span><span>            "dataSource": "social_media",</span> </span><span><span>            "timestampSpec": {</span> </span><span><span>                "column": "__time",</span> </span><span><span>                "format": "iso"</span> </span><span><span>            },</span> </span><span><span>            "dimensionsSpec": {</span> </span><span><span>                "dimensions": [</span> </span><span><span>                    "username",</span> </span><span><span>                    "post_title",</span> </span><span><span>                    {</span> </span><span><span>                        "type": "long",</span> </span><span><span>                        "name": "views"</span> </span><span><span>                    },</span> </span><span><span>                    {</span> </span><span><span>                        "type": "long",</span> </span><span><span>                        "name": "upvotes"</span> </span><span><span>                    },</span> </span><span><span>                    {</span> </span><span><span>                        "type": "long",</span> </span><span><span>                        "name": "comments"</span> </span><span><span>                    },</span> </span><span><span>                    "edited"</span> </span><span><span>                ]</span> </span><span><span>            },</span> </span><span><span>            "granularitySpec": {</span> </span><span><span>                "queryGranularity": "none",</span> </span><span><span>                "rollup": false,</span> </span><span><span>                "segmentGranularity": "hour"</span> </span><span><span>            }</span> </span><span><span>        }</span> </span><span><span>    }</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
Indexing tasks remain suspended until you [resume the supervisor](#api-reference-supervisor-api--resume-a-supervisor).

`POST` `/druid/indexer/v1/supervisor/{supervisorId}/suspend`

<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>400 BAD REQUEST</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully shut down supervisor</em></p></div><div><p><em>Supervisor already suspended</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>

---

The following example shows how to suspend a running supervisor with the name `social_media`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/suspend"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/social_media/suspend HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully suspended all supervisors</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/suspendAll"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/suspendAll HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>400 BAD REQUEST</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully resumed supervisor</em></p></div><div><p><em>Supervisor already running</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>

---

The following example resumes a previously suspended supervisor with name `social_media`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/resume"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/social_media/resume HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully resumed all supervisors</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/resumeAll"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/resumeAll HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
After clearing all stored offsets, the supervisor kills and recreates active tasks, so that tasks begin reading from valid positions.
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully reset supervisor</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>

---

The following example shows how to reset a supervisor with the name `social_media`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/reset"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/social_media/reset HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully reset offsets</em></p></div><div><p><em>Invalid supervisor ID</em></p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/resetOffsets"</span> </span><span><span>--header 'Content-Type: application/json'</span> </span><span><span>--data-raw '{"type":"kafka","partitions":{"type":"end","stream":"ads_media_stream","partitionOffsetMap":{"0":100, "2": 650}}}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/social_media/resetOffsets HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "type": "kafka",</span> </span><span><span>  "partitions": {</span> </span><span><span>    "type": "end",</span> </span><span><span>    "stream": "ads_media_stream",</span> </span><span><span>    "partitionOffsetMap": {</span> </span><span><span>      "0": 100,</span> </span><span><span>      "2": 650</span> </span><span><span>    }</span> </span><span><span>  }</span> </span><span><span>}</span> </span></code></pre><div></div></div></div><p>The example operation resets offsets only for partitions <code>0</code> and <code>2</code> to 100 and 650 respectively. After a successful reset, when the supervisor's tasks restart, they resume reading from <code>{"0": 100, "1": 10, "2": 650, "3": 40}</code>.</p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully terminated a supervisor</em></p></div><div><p><em>Invalid supervisor ID or supervisor not running</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/terminate"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/social_media/terminate HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully terminated all supervisors</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/terminateAll"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/terminateAll HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>202 ACCEPTED</li><li>404 NOT FOUND</li><li>400 BAD REQUEST</li></ul><div><div><p><em>Request has been accepted and handoff will be initiated in the background.</em></p></div><div><p><em>Invalid supervisor ID or the supervisor is not running.</em></p></div><div><p><em>Supervisor does not support early handoff.</em></p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/supervisor/social_media/taskGroups/handoff"</span> </span><span><span>--header 'Content-Type: application/json'</span> </span><span><span>--data-raw '{"taskGroupIds": [1, 2, 3]}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/supervisor/social_media/taskGroups/handoff HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "taskGroupIds": [1, 2, 3],</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **202 Accepted**
> *Empty response*

Shuts down a supervisor. This endpoint is deprecated and will be removed in future releases. Use the equivalent [terminate](#api-reference-supervisor-api--terminate-a-supervisor) endpoint instead.

`POST` `/druid/indexer/v1/supervisor/{supervisorId}/shutdown`

---

<a id="api-reference-retention-rules-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/retention-rules-api/ -->
...
Druid uses retention rules to determine what data is retained in the cluster. Druid supports load, drop, and broadcast rules. For more information, see [Using rules to drop and retain data](#operations-rule-configuration).
...
- [Load rules](#operations-rule-configuration--load-rules)
- [Drop rules](#operations-rule-configuration--drop-rules)
- [Broadcast rules](#operations-rule-configuration--broadcast-rules)

This request overwrites any existing rules for the datasource.
Druid reads rules in the order in which they appear; for more information, see [rule structure](#operations-rule-configuration).
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully updated retention rules for specified datasource</em></p></div></div></div>

---

The following example sets a set of broadcast, load, and drop retention rules for the `kttm1` datasource.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/rules/kttm1" \</span> </span><span><span>--header 'X-Druid-Author: doc intern' \</span> </span><span><span>--header 'X-Druid-Comment: submitted via api' \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '[</span> </span><span><span>    {</span> </span><span><span>        "type": "broadcastForever"</span> </span><span><span>    },</span> </span><span><span>    {</span> </span><span><span>        "type": "loadForever",</span> </span><span><span>        "tieredReplicants": {</span> </span><span><span>            "_default_tier": 2</span> </span><span><span>        },</span> </span><span><span>        "useDefaultTierForNull": true</span> </span><span><span>    },</span> </span><span><span>    {</span> </span><span><span>        "type": "dropByPeriod",</span> </span><span><span>        "period": "P1M"</span> </span><span><span>    }</span> </span><span><span>]'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/coordinator/v1/rules/kttm1 HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>X-Druid-Author: doc intern</span> </span><span><span>X-Druid-Comment: submitted via api</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 273</span> </span><span><span></span> </span><span><span>[</span> </span><span><span>    {</span> </span><span><span>        "type": "broadcastForever"</span> </span><span><span>    },</span> </span><span><span>    {</span> </span><span><span>        "type": "loadForever",</span> </span><span><span>        "tieredReplicants": {</span> </span><span><span>            "_default_tier": 1</span> </span><span><span>        },</span> </span><span><span>        "useDefaultTierForNull": true</span> </span><span><span>    },</span> </span><span><span>    {</span> </span><span><span>        "type": "dropByPeriod",</span> </span><span><span>        "period": "P1M"</span> </span><span><span>    }</span> </span><span><span>]</span> </span></code></pre><div></div></div></div></div></div></div>
...
This request overwrites any existing rules for all datasources. To remove default retention rules for all datasources, submit an empty rule array in the request body. Rules are read in the order in which they appear; for more information, see [rule structure](#operations-rule-configuration).
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>500 SERVER ERROR</li></ul><div><div><p><em>Successfully updated default retention rules</em></p></div><div><p><em>Error with request body</em></p></div></div></div>

---

The following example updates the default retention rule for all datasources with a `loadByInterval` rule.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/rules/_default" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '[</span> </span><span><span>    {</span> </span><span><span>        "type": "loadByInterval",</span> </span><span><span>        "tieredReplicants": {},</span> </span><span><span>        "useDefaultTierForNull": false,</span> </span><span><span>        "interval": "2010-01-01/2020-01-01"</span> </span><span><span>    }</span> </span><span><span>]'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/coordinator/v1/rules/_default HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 205</span> </span><span><span></span> </span><span><span>[</span> </span><span><span>    {</span> </span><span><span>        "type": "loadByInterval",</span> </span><span><span>        "tieredReplicants": {},</span> </span><span><span>        "useDefaultTierForNull": false,</span> </span><span><span>        "interval": "2010-01-01/2020-01-01"</span> </span><span><span>    }</span> </span><span><span>]</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved retention rules</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/rules"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/rules HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/rules/social_media?full=null"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/rules/social_media?full=null HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>400 BAD REQUEST</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully retrieved audit history</em></p></div><div><p><em>Request in the incorrect format</em></p></div><div><p><em><code>count</code> query parameter too large</em></p></div></div></div>

---

The following example retrieves the audit history for all datasources from `2023-07-13` to `2023-07-19`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/rules/history?interval=2023-07-13%2F2023-07-19"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/rules/history?interval=2023-07-13/2023-07-19 HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<a id="api-reference-data-management-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/data-management-api/ -->
...
<a id="api-reference-automatic-compaction-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/automatic-compaction-api/ -->

<!-- page_index: 132 -->

# Automatic compaction API

This topic describes the status and configuration API endpoints for [automatic compaction using Coordinator duties](#data-management-automatic-compaction--auto-compaction-using-coordinator-duties) in Apache Druid. You can configure automatic compaction in the Druid web console or API.

> [!NOTE]
> **Experimental**
> Instead of the automatic compaction API, you can use the supervisor API to submit auto-compaction jobs using compaction supervisors. For more information, see [Auto-compaction using compaction supervisors](#data-management-automatic-compaction--auto-compaction-using-compaction-supervisors).
...
The automatic compaction configuration requires only the `dataSource` property. Druid fills all other properties with default values if not specified. See [Automatic compaction dynamic configuration](#configuration--automatic-compaction-dynamic-configuration) for configuration details.
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully submitted auto compaction configuration</em></p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config/compaction"\</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>    "dataSource": "wikipedia_hour",</span> </span><span><span>    "skipOffsetFromLatest": "PT0S",</span> </span><span><span>    "tuningConfig": {</span> </span><span><span>        "partitionsSpec": {</span> </span><span><span>            "type": "dynamic"</span> </span><span><span>        },</span> </span><span><span>        "type": "index_parallel"</span> </span><span><span>    },</span> </span><span><span>    "granularitySpec": {</span> </span><span><span>        "segmentGranularity": "DAY"</span> </span><span><span>    }</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/coordinator/v1/config/compaction HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 281</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>    "dataSource": "wikipedia_hour",</span> </span><span><span>    "skipOffsetFromLatest": "PT0S",</span> </span><span><span>    "tuningConfig": {</span> </span><span><span>        "partitionsSpec": {</span> </span><span><span>            "type": "dynamic"</span> </span><span><span>        },</span> </span><span><span>        "type": "index_parallel"</span> </span><span><span>    },</span> </span><span><span>    "granularitySpec": {</span> </span><span><span>        "segmentGranularity": "DAY"</span> </span><span><span>    }</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully deleted automatic compaction configuration</em></p></div><div><p><em>Datasource does not have automatic compaction or invalid datasource name</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request DELETE "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config/compaction/wikipedia_hour"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>DELETE /druid/coordinator/v1/config/compaction/wikipedia_hour HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

A successful request returns an HTTP `200 OK` message code and an empty response body.

> [!NOTE]
> **info**
> This API is now deprecated. Use [Update cluster-level compaction config](#api-reference-automatic-compaction-api--update-cluster-level-compaction-config) instead.
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully updated compaction configuration</em></p></div><div><p><em>Invalid <code>max</code> value</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config/compaction/taskslots?ratio=0.2&amp;max=250000"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/coordinator/v1/config/compaction/taskslots?ratio=0.2&amp;max=250000 HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved automatic compaction configurations</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config/compaction"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/config/compaction HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully retrieved configuration for datasource</em></p></div><div><p><em>Invalid datasource or datasource does not have automatic compaction enabled</em></p></div></div></div>

---

The following example retrieves the automatic compaction configuration for datasource `wikipedia_hour`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config/compaction/wikipedia_hour"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/config/compaction/wikipedia_hour HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>400 BAD REQUEST</li></ul><div><div><p><em>Successfully retrieved configuration history</em></p></div><div><p><em>Invalid <code>count</code> value</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config/compaction/wikipedia_hour/history"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/config/compaction/wikipedia_hour/history HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div><p><em>Successfully retrieved segment size awaiting compaction</em></p></div><div><p><em>Unknown datasource name or datasource does not have automatic compaction enabled</em></p></div></div></div>

---

The following example retrieves the remaining segments to be compacted for datasource `wikipedia_hour`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/compaction/progress?dataSource=wikipedia_hour"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/compaction/progress?dataSource=wikipedia_hour HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved <code>latestStatus</code> object</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/compaction/status"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/compaction/status HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> {"latestStatus": [{"dataSource": "wikipedia_api","scheduleStatus": "RUNNING","bytesAwaitingCompaction": 0,"bytesCompacted": 0,"bytesSkipped": 64133616,"segmentCountAwaitingCompaction": 0,"segmentCountCompacted": 0,"segmentCountSkipped": 8,"intervalCountAwaitingCompaction": 0,"intervalCountCompacted": 0,"intervalCountSkipped": 1 },{"dataSource": "wikipedia_hour","scheduleStatus": "RUNNING","bytesAwaitingCompaction": 0,"bytesCompacted": 5998634,"bytesSkipped": 0,"segmentCountAwaitingCompaction": 0,"segmentCountCompacted": 1,"segmentCountSkipped": 0,"intervalCountAwaitingCompaction": 0,"intervalCountCompacted": 1,"intervalCountSkipped": 0}]}
> ```
...
| `compactionPolicy` | Policy to choose intervals for compaction. Supported policies are [Newest segment first](#api-reference-automatic-compaction-api--compaction-policy-newestsegmentfirst), [Most fragmented first](#api-reference-automatic-compaction-api--compaction-policy-mostfragmentedfirst), and [Fixed interval order](#api-reference-automatic-compaction-api--compaction-policy-fixedintervalorder). | Newest segment first |
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request POST "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config/compaction/cluster" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>    "compactionTaskSlotRatio": 0.5,</span> </span><span><span>    "maxCompactionTaskSlots": 1500,</span> </span><span><span>    "compactionPolicy": {</span> </span><span><span>        "type": "newestSegmentFirst",</span> </span><span><span>        "priorityDatasource": "wikipedia"</span> </span><span><span>    },</span> </span><span><span>    "useSupervisors": true,</span> </span><span><span>    "engine": "msq"</span> </span><span><span>}'</span> </span><span><span></span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/compaction/config/cluster HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>    "compactionTaskSlotRatio": 0.5,</span> </span><span><span>    "maxCompactionTaskSlots": 1500,</span> </span><span><span>    "compactionPolicy": {</span> </span><span><span>        "type": "newestSegmentFirst",</span> </span><span><span>        "priorityDatasource": "wikipedia"</span> </span><span><span>    },</span> </span><span><span>    "useSupervisors": true,</span> </span><span><span>    "engine": "msq"</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
This includes all the fields listed in [Update cluster-level compaction config](#api-reference-automatic-compaction-api--update-cluster-level-compaction-config).

`GET` `/druid/indexer/v1/compaction/config/cluster`

<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved cluster compaction configuration</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/compaction/config/cluster"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/compaction/config/cluster HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> {"compactionTaskSlotRatio": 0.5,"maxCompactionTaskSlots": 1500,"compactionPolicy": {"type": "newestSegmentFirst","priorityDatasource": "wikipedia" },"useSupervisors": true,"engine": "msq"}
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/compaction/config/datasources"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/compaction/config/datasources HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/compaction/config/datasources/wikipedia_hour"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/compaction/config/datasources/wikipedia_hour HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/compaction/config/datasources/wikipedia_hour"\</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>    "dataSource": "wikipedia_hour",</span> </span><span><span>    "skipOffsetFromLatest": "PT0S",</span> </span><span><span>    "tuningConfig": {</span> </span><span><span>        "partitionsSpec": {</span> </span><span><span>            "type": "dynamic"</span> </span><span><span>        },</span> </span><span><span>        "type": "index_parallel"</span> </span><span><span>    },</span> </span><span><span>    "granularitySpec": {</span> </span><span><span>        "segmentGranularity": "DAY"</span> </span><span><span>    }</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/compaction/config/datasources/wikipedia_hour HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 281</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>    "dataSource": "wikipedia_hour",</span> </span><span><span>    "skipOffsetFromLatest": "PT0S",</span> </span><span><span>    "tuningConfig": {</span> </span><span><span>        "partitionsSpec": {</span> </span><span><span>            "type": "dynamic"</span> </span><span><span>        },</span> </span><span><span>        "type": "index_parallel"</span> </span><span><span>    },</span> </span><span><span>    "granularitySpec": {</span> </span><span><span>        "segmentGranularity": "DAY"</span> </span><span><span>    }</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl --request DELETE "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/compaction/config/datasources/wikipedia_hour"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>DELETE /druid/indexer/v1/compaction/config/wikipedia_hour HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
The response payload is in the same format as [Compaction status response](#api-reference-automatic-compaction-api--compaction-status-response).
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/compaction/status/datasources"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/compaction/status/datasources HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
Retrieves the latest status from the latest automatic compaction run for a datasource. The response payload is in the same format as [Compaction status response](#api-reference-automatic-compaction-api--compaction-status-response) with zero or one entry.
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/compaction/status/datasources/wikipedia_hour"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/compaction/status/datasources/wikipedia_hour HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> {"latestStatus": [{"dataSource": "wikipedia_hour","scheduleStatus": "RUNNING","bytesAwaitingCompaction": 0,"bytesCompacted": 5998634,"bytesSkipped": 0,"segmentCountAwaitingCompaction": 0,"segmentCountCompacted": 1,"segmentCountSkipped": 0,"intervalCountAwaitingCompaction": 0,"intervalCountCompacted": 1,"intervalCountSkipped": 0}]}
> ```

---

<a id="api-reference-lookups-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/lookups-api/ -->
...
```json
{"<tierName>": {"<lookupName>": {"version": "<version>","lookupExtractorFactory": {"type": "<someExtractorFactoryType>","<someExtractorField>": "<someExtractorValue>"}}}}
```
...
```json
{"version": "v1","lookupExtractorFactory": {"type": "map","map": {"847632": "Internal Use Only"}}}
```
...
<a id="api-reference-service-status-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/service-status-api/ -->
...
Additional checks for readiness should use the [Historical segment readiness](#api-reference-service-status-api--get-segment-readiness) and [Broker query readiness](#api-reference-service-status-api--get-broker-query-readiness) endpoints.

`GET` `/status/health`

<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved service health</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/status/health"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /status/health HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved service configuration properties</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/status/properties"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /status/properties HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Node was successfully added to the cluster</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/status/selfDiscovered/status"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /status/selfDiscovered/status HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>503 SERVICE UNAVAILABLE</li></ul><div><div>
<p><em>Successfully retrieved node status</em></p></div><div>
<p><em>Unsuccessful node self-discovery</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/status/selfDiscovered"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /status/selfDiscovered HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved leader Coordinator address</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/leader"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/leader HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
Retrieves a JSON object with a `leader` key. Returns `true` if this server is the current leader Coordinator of the cluster. To get the individual address of the leader Coordinator node, see the [leader endpoint](#api-reference-service-status-api--get-coordinator-leader-address).
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>404 NOT FOUND</li></ul><div><div>
<p><em>Current server is the leader</em></p></div><div>
<p><em>Current server is not the leader</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://COORDINATOR_IP:COORDINATOR_PORT/druid/coordinator/v1/isLeader"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/isLeader HTTP/1.1</span> </span><span><span>Host: http://COORDINATOR_IP:COORDINATOR_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved cloning status</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://COORDINATOR_IP:COORDINATOR_PORT/druid/coordinator/v1/config/cloneStatus"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/config/cloneStatus HTTP/1.1</span> </span><span><span>Host: http://COORDINATOR_IP:COORDINATOR_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> {"cloneStatus": [{"sourceServer": "localhost:8089","targetServer": "localhost:8083","state": "IN_PROGRESS","segmentLoadsRemaining": 0,"segmentDropsRemaining": 0,"bytesToLoad": 0}]}
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved Broker Configuration view</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://COORDINATOR_IP:COORDINATOR_PORT/druid/coordinator/v1/config/syncedBrokers"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/config/syncedBrokers HTTP/1.1</span> </span><span><span>Host: http://COORDINATOR_IP:COORDINATOR_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> {"syncedBrokers": [{"host": "localhost","port": 8082,"lastSyncTimestampMillis": 1745756337472}]}
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved leader Overlord address</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/leader"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/leader HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
Retrieves a JSON object with a `leader` property. The value can be `true` or `false`, indicating if this server is the current leader Overlord of the cluster. To get the individual address of the leader Overlord node, see the [leader endpoint](#api-reference-service-status-api--get-overlord-leader-address).
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://OVERLORD_IP:OVERLORD_PORT/druid/indexer/v1/isLeader"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/isLeader HTTP/1.1</span> </span><span><span>Host: http://OVERLORD_IP:OVERLORD_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved Middle Manager state</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT/druid/worker/v1/enabled"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/worker/v1/enabled HTTP/1.1</span> </span><span><span>Host: http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
Retrieves a list of active tasks being run on the Middle Manager. Returns JSON list of task ID strings. Note that for normal usage, you should use the `/druid/indexer/v1/tasks` [Tasks API](#api-reference-tasks-api) endpoint or one of the task state specific variants instead.

`GET` `/druid/worker/v1/tasks`

<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved active tasks</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT/druid/worker/v1/tasks"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/worker/v1/tasks HTTP/1.1</span> </span><span><span>Host: http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
[Tasks API](#api-reference-tasks-api) endpoint instead.
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully shut down a task</em></p></div></div></div>

---

The following example shuts down a task with specified ID `index_kafka_wikiticker_f7011f8ffba384b_fpeclode`.

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT/druid/worker/v1/task/index_kafka_wikiticker_f7011f8ffba384b_fpeclode/shutdown"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/worker/v1/task/index_kafka_wikiticker_f7011f8ffba384b_fpeclode/shutdown HTTP/1.1</span> </span><span><span>Host: http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully disabled Middle Manager</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT/druid/worker/v1/disable"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/worker/v1/disable HTTP/1.1</span> </span><span><span>Host: http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully enabled Middle Manager</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT/druid/worker/v1/enable"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/worker/v1/enable HTTP/1.1</span> </span><span><span>Host: http://MIDDLEMANAGER_IP:MIDDLEMANAGER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Successfully retrieved status</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://HISTORICAL_IP:HISTORICAL_PORT/druid/historical/v1/loadstatus"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/historical/v1/loadstatus HTTP/1.1</span> </span><span><span>Host: http://HISTORICAL_IP:HISTORICAL_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>503 SERVICE UNAVAILABLE</li></ul><div><div>
<p><em>Segments in local cache successfully loaded</em></p></div><div>
<p><em>Segments in local cache have not been loaded</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://HISTORICAL_IP:HISTORICAL_PORT/druid/historical/v1/readiness"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/historical/v1/readiness HTTP/1.1</span> </span><span><span>Host: http://HISTORICAL_IP:HISTORICAL_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div>
<p><em>Segments successfully loaded</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://BROKER_IP:BROKER_PORT/druid/broker/v1/loadstatus"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/broker/v1/loadstatus HTTP/1.1</span> </span><span><span>Host: http://&lt;BROKER_IP&gt;:&lt;BROKER_PORT&gt;</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li><li>503 SERVICE UNAVAILABLE</li></ul><div><div>
<p><em>Segments successfully loaded</em></p></div><div>
<p><em>Segments have not been loaded</em></p></div></div></div>

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://BROKER_IP:BROKER_PORT/druid/broker/v1/readiness"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/broker/v1/readiness HTTP/1.1</span> </span><span><span>Host: http://BROKER_IP:BROKER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

A successful response to this endpoint results in an empty response body.

---

<a id="api-reference-dynamic-configuration-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/dynamic-configuration-api/ -->

<!-- page_index: 135 -->

# Dynamic configuration API

This document describes the API endpoints to retrieve and manage dynamic configurations for the [Coordinator](#design-coordinator) and [Overlord](#design-overlord) in Apache Druid.
...
For information on the supported properties, see [Coordinator dynamic configuration](#configuration--dynamic-configuration).
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved dynamic configuration</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/config HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
For information on the supported properties, see [Dynamic configuration](#configuration--dynamic-configuration).
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully updated dynamic configuration</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/config" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>  "millisToWaitBeforeDeleting": 900000,</span> </span><span><span>  "maxSegmentsToMove": 5,</span> </span><span><span>  "percentOfSegmentsToConsiderPerMove": 100,</span> </span><span><span>  "useBatchedSegmentSampler": true,</span> </span><span><span>  "replicantLifetime": 15,</span> </span><span><span>  "replicationThrottleLimit": 10,</span> </span><span><span>  "balancerComputeThreads": 1,</span> </span><span><span>  "emitBalancingStats": true,</span> </span><span><span>  "killDataSourceWhitelist": [],</span> </span><span><span>  "killPendingSegmentsSkipList": [],</span> </span><span><span>  "maxSegmentsInNodeLoadingQueue": 100,</span> </span><span><span>  "decommissioningNodes": [],</span> </span><span><span>  "decommissioningMaxPercentOfMaxSegmentsToMove": 70,</span> </span><span><span>  "pauseCoordination": false,</span> </span><span><span>  "replicateAfterLoadTimeout": false,</span> </span><span><span>  "maxNonPrimaryReplicantsToLoad": 2147483647,</span> </span><span><span>  "useRoundRobinSegmentAssignment": true,</span> </span><span><span>  "turboLoadingNodes": [],</span> </span><span><span>  "cloneServers": {}</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/coordinator/v1/config HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 683</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "millisToWaitBeforeDeleting": 900000,</span> </span><span><span>  "maxSegmentsToMove": 5,</span> </span><span><span>  "percentOfSegmentsToConsiderPerMove": 100,</span> </span><span><span>  "useBatchedSegmentSampler": true,</span> </span><span><span>  "replicantLifetime": 15,</span> </span><span><span>  "replicationThrottleLimit": 10,</span> </span><span><span>  "balancerComputeThreads": 1,</span> </span><span><span>  "emitBalancingStats": true,</span> </span><span><span>  "killDataSourceWhitelist": [],</span> </span><span><span>  "killPendingSegmentsSkipList": [],</span> </span><span><span>  "maxSegmentsInNodeLoadingQueue": 100,</span> </span><span><span>  "decommissioningNodes": [],</span> </span><span><span>  "decommissioningMaxPercentOfMaxSegmentsToMove": 70,</span> </span><span><span>  "pauseCoordination": false,</span> </span><span><span>  "replicateAfterLoadTimeout": false,</span> </span><span><span>  "maxNonPrimaryReplicantsToLoad": 2147483647,</span> </span><span><span>  "useRoundRobinSegmentAssignment": true,</span> </span><span><span>  "turboLoadingNodes": [],</span> </span><span><span>  "cloneServers": {}</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved history</em></p></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved broker dynamic configuration</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/broker/config"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/broker/config HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully updated configuration</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl -X POST "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/broker/config" \</span> </span><span><span>-H "Content-Type: application/json" \</span> </span><span><span>-H "X-Druid-Author: admin" \</span> </span><span><span>-H "X-Druid-Comment: Add query blocklist rules and set default context" \</span> </span><span><span>-d '{</span> </span><span><span>  "queryBlocklist": [</span> </span><span><span>    {</span> </span><span><span>      "ruleName": "block-expensive-scans",</span> </span><span><span>      "dataSources": ["large_table", "huge_dataset"],</span> </span><span><span>      "queryTypes": ["scan"]</span> </span><span><span>    },</span> </span><span><span>    {</span> </span><span><span>      "ruleName": "block-debug-queries",</span> </span><span><span>      "contextMatches": {</span> </span><span><span>        "debug": "true"</span> </span><span><span>      }</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "queryContext": {</span> </span><span><span>    "priority": 0,</span> </span><span><span>    "timeout": 300000</span> </span><span><span>  },</span> </span><span><span>  "perSegmentTimeoutConfig": {</span> </span><span><span>    "large_table": {</span> </span><span><span>      "perSegmentTimeoutMs": 10000</span> </span><span><span>    },</span> </span><span><span>    "huge_dataset": {</span> </span><span><span>      "perSegmentTimeoutMs": 5000,</span> </span><span><span>      "monitorOnly": true</span> </span><span><span>    }</span> </span><span><span>  }</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/coordinator/v1/broker/config HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>X-Druid-Author: admin</span> </span><span><span>X-Druid-Comment: Add query blocklist rules and set default context</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "queryBlocklist": [</span> </span><span><span>    {</span> </span><span><span>      "ruleName": "block-expensive-scans",</span> </span><span><span>      "dataSources": ["large_table", "huge_dataset"],</span> </span><span><span>      "queryTypes": ["scan"]</span> </span><span><span>    },</span> </span><span><span>    {</span> </span><span><span>      "ruleName": "block-debug-queries",</span> </span><span><span>      "contextMatches": {</span> </span><span><span>        "debug": "true"</span> </span><span><span>      }</span> </span><span><span>    }</span> </span><span><span>  ],</span> </span><span><span>  "queryContext": {</span> </span><span><span>    "priority": 0,</span> </span><span><span>    "timeout": 300000</span> </span><span><span>  },</span> </span><span><span>  "perSegmentTimeoutConfig": {</span> </span><span><span>    "large_table": {</span> </span><span><span>      "perSegmentTimeoutMs": 10000</span> </span><span><span>    },</span> </span><span><span>    "huge_dataset": {</span> </span><span><span>      "perSegmentTimeoutMs": 5000,</span> </span><span><span>      "monitorOnly": true</span> </span><span><span>    }</span> </span><span><span>  }</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
```json
{"perSegmentTimeoutConfig": {"large_datasource": {"perSegmentTimeoutMs": 10000 },"critical_datasource": {"perSegmentTimeoutMs": 5000,"monitorOnly": true}}}
```
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/coordinator/v1/broker/config/history?count=10"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/coordinator/v1/broker/config/history?count=10 HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> [{"key": "broker.config","type": "broker.config","auditInfo": {"author": "admin","comment": "Add query blocklist rules","ip": "127.0.0.1" },"payload": "{\"queryBlocklist\":[{\"ruleName\":\"block-expensive-scans\",\"dataSources\":[\"large_table\"],\"queryTypes\":[\"scan\"]}],\"queryContext\":{\"priority\":0,\"timeout\":300000},\"perSegmentTimeoutConfig\":{\"large_table\":{\"perSegmentTimeoutMs\":10000,\"monitorOnly\":false}}}","auditTime": "2024-03-06T12:00:00.000Z"}]
> ```

The Overlord has dynamic configurations to tune how Druid assigns tasks to workers.
For information on the supported properties, see [Overlord dynamic configuration](#configuration--overlord-dynamic-configuration).
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/worker"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/worker HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> {"type": "default","selectStrategy": {"type": "fillCapacityWithCategorySpec","workerCategorySpec": {"categoryMap": {},"strong": true} },"autoScaler": null}
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/worker" \</span> </span><span><span>--header 'Content-Type: application/json' \</span> </span><span><span>--data '{</span> </span><span><span>  "type": "default",</span> </span><span><span>  "selectStrategy": {</span> </span><span><span>    "type": "fillCapacityWithCategorySpec",</span> </span><span><span>    "workerCategorySpec": {</span> </span><span><span>      "categoryMap": {},</span> </span><span><span>      "strong": true</span> </span><span><span>    }</span> </span><span><span>  },</span> </span><span><span>  "autoScaler": null</span> </span><span><span>}'</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>POST /druid/indexer/v1/worker HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span><span><span>Content-Type: application/json</span> </span><span><span>Content-Length: 196</span> </span><span><span></span> </span><span><span>{</span> </span><span><span>  "type": "default",</span> </span><span><span>  "selectStrategy": {</span> </span><span><span>    "type": "fillCapacityWithCategorySpec",</span> </span><span><span>    "workerCategorySpec": {</span> </span><span><span>      "categoryMap": {},</span> </span><span><span>      "strong": true</span> </span><span><span>    }</span> </span><span><span>  },</span> </span><span><span>  "autoScaler": null</span> </span><span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/worker/history?interval=2022-07-13%2F2024-07-19&amp;count=10"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/worker/history?interval=2022-07-13%2F2024-07-19&amp;count=10 HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> [{"key": "worker.config","type": "worker.config","auditInfo": {"author": "","comment": "","ip": "127.0.0.1" },"payload": "{\"type\":\"default\",\"selectStrategy\":{\"type\":\"fillCapacityWithCategorySpec\",\"workerCategorySpec\":{\"categoryMap\":{},\"strong\":true}},\"autoScaler\":null}","auditTime": "2023-10-03T21:49:49.991Z"}]
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved worker nodes</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/workers"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/workers HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

> [!NOTE]
> **View the response**
> ```json
> [{"worker": {"scheme": "http","host": "localhost:8091","ip": "198.51.100.0","capacity": 2,"version": "0","category": "_default_worker_category" },"currCapacityUsed": 0,"currParallelIndexCapacityUsed": 0,"availabilityGroups": [],"runningTasks": [],"lastCompletedTaskTime": "2023-09-29T19:13:05.505Z","blacklistedUntil": null}]
> ```
...
<div class="tabs-container tabList__CuJ"><ul><li>200 SUCCESS</li></ul><div><div><p><em>Successfully retrieved scaling events</em></p></div></div></div>

---

<div class="tabs-container tabList__CuJ"><ul><li>cURL</li><li>HTTP</li></ul><div><div><div><div><pre><code><span><span>curl "http://ROUTER_IP:ROUTER_PORT/druid/indexer/v1/scaling"</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>GET /druid/indexer/v1/scaling HTTP/1.1</span> </span><span><span>Host: http://ROUTER_IP:ROUTER_PORT</span> </span></code></pre><div></div></div></div></div></div></div>

A successful request returns a `200 OK` response and an array of scaling events.

---

<a id="api-reference-legacy-metadata-api"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/legacy-metadata-api/ -->
...
> [`sys.segments`](#querying-sql-metadata-tables--segments-table) table.

---

<a id="api-reference-sql-jdbc"></a>

<!-- source_url: https://druid.apache.org/docs/latest/api-reference/sql-jdbc/ -->
...
<!-- source_url: https://druid.apache.org/docs/latest/configuration/ -->
...
<a id="configuration-extensions"></a>

<!-- source_url: https://druid.apache.org/docs/latest/configuration/extensions/ -->
...
<a id="configuration-logging"></a>

<!-- source_url: https://druid.apache.org/docs/latest/configuration/logging/ -->
...
<a id="operations-web-console"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/web-console/ -->
...
> [!NOTE]
> **info**
> **Security note:** Without [Druid user permissions](#operations-security-overview) configured, any user of the
...
<a id="operations-java"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/java/ -->
...
[Amazon Corretto](https://docs.aws.amazon.com/corretto/latest/corretto-17-ug/what-is-corretto-17.html), [Azul Zulu](https://www.azul.com/downloads/?version=java-17-lts&package=jdk), and
...
similar commands. In this case, there is nothing special you need to do to run successfully. However, if you have customized your Druid service launching system, you will need to ensure the required Java parameters are
...
<a id="operations-durable-storage"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/durable-storage/ -->
...
<a id="operations-security-overview"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/security-overview/ -->
...
> [!WARNING]
> Druid administrators have the same OS permissions as the Unix user account running Druid. See [Authentication and authorization model](#operations-security-user-auth--authentication-and-authorization-model). If the Druid process is running under the OS root user account, then Druid administrators can read or write all files that the root account has access to, including sensitive files such as `/etc/passwd`.

---

<a id="operations-security-user-auth"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/security-user-auth/ -->
...
- EXTERNAL – External data read through the [EXTERN function](#multi-stage-query-concepts--read-external-data-with-extern) in SQL.
- STATE – Cluster-wide state resources.
- SYSTEM\_TABLE – when the Broker property `druid.sql.planner.authorizeSystemTablesDirectly` is true, then Druid uses this resource type to authorize the system tables in the `sys` schema in SQL.

For specific resources associated with the resource types, see [Defining permissions](#operations-security-user-auth--defining-permissions) and the corresponding endpoint descriptions in [API reference](#api-reference).
...
If `druid.auth.authenticator.<authenticator-name>.initialInternalClientPassword` is set, a default internal system user named "druid\_system" will be created, with the specified initial password. If this configuration is omitted, the "druid\_system" user will not be created.

Each Authorizer will always have a default "admin" and "druid\_system" user with full privileges.
...
the [EXTERN function](#multi-stage-query-concepts--read-external-data-with-extern) in SQL
...
For information on what HTTP methods are supported on a particular request endpoint, refer to [API reference](#api-reference).

`GET` requests require READ permissions, while `POST` and `DELETE` requests require WRITE permissions.

Queries on Druid datasources require DATASOURCE READ permissions for the specified datasource.

Queries to access external data through the [EXTERN function](#multi-stage-query-concepts--read-external-data-with-extern) require EXTERNAL READ permissions.

Queries on [INFORMATION\_SCHEMA tables](#querying-sql-metadata-tables--information-schema) return information about datasources that the caller has DATASOURCE READ access to. Other
datasources are omitted.

Queries on the [system schema tables](#querying-sql-metadata-tables--system-schema) require the following permissions:
...
<a id="operations-auth-ldap"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/auth-ldap/ -->
...
> [!NOTE]
> **info**
> Druid uses the `memberOf` attribute to determine a group's membership using LDAP. If your LDAP server implementation doesn't include this attribute, you must complete some additional steps when you [map LDAP groups to Druid roles](#operations-auth-ldap--map-ldap-groups-to-druid-roles).

---

<a id="operations-password-provider"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/password-provider/ -->
...
For more information, see [Adding a new Password Provider implementation](#development-modules--adding-a-new-password-provider-implementation).
...
<a id="operations-dynamic-config-provider"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/dynamic-config-provider/ -->

<!-- page_index: 148 -->

# Dynamic Config Providers

Druid relies on dynamic config providers to supply multiple related sets of credentials, secrets, and configurations within a Druid extension. Dynamic config providers are intended to eventually replace [PasswordProvider](#operations-password-provider).

By default, Druid includes an environment variable dynamic config provider that supports Kafka consumer configuration in [Kafka ingestion](#ingestion-kafka-ingestion).

To develop a custom extension of the `DynamicConfigProvider` interface that is registered at Druid process startup, see [Adding a new DynamicConfigProvider implementation](#development-modules--adding-a-new-dynamicconfigprovider-implementation).
...
```text
..."consumerProperties": {"bootstrap.servers": "localhost:9092","ssl.keystore.location": "/opt/kafka/config/kafka01.keystore.jks","ssl.truststore.location": "/opt/kafka/config/kafka.truststore.jks","druid.dynamic.config.provider": {"type": "environment","variables": {"ssl.key.password": "SSL_KEY_PASSWORD","ssl.keystore.password": "SSL_KEYSTORE_PASSWORD","ssl.truststore.password": "SSL_TRUSTSTORE_PASSWORD"}} },...
```
...
<a id="operations-tls-support"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/tls-support/ -->
...
To get familiar with TLS/SSL, along with related concepts like keys and certificates, read [Configuring Secure Protocols](https://www.eclipse.org/jetty/documentation/jetty-12/operations-guide/index.html#og-protocols-ssl) in the Jetty documentation.
...
| `druid.server.https.keyStorePassword` | The [Password Provider](#operations-password-provider) or String password for the Key Store. | none | yes |
...
| `druid.server.https.trustStorePassword` | The [password provider](#operations-password-provider) or String password for the Trust Store. Not needed if `requireClientCertificate` and `requestClientCertificate` are false. | none | no |
...
| `druid.server.https.keyManagerPassword` | The [Password Provider](#operations-password-provider) or String password for the Key Manager. | none | no |
...
while creating the HttpClient. This binding can be achieved writing a [Druid extension](#configuration-extensions)
which can provide an instance of SSLContext. Druid comes with a simple extension present [here](https://druid.apache.org/docs/latest/development/extensions-core/simple-client-sslcontext)
which should be useful enough for most simple cases, see [this](#configuration-extensions--loading-extensions) for how to include extensions.
...
<a id="operations-basic-cluster-tuning"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/basic-cluster-tuning/ -->
...
<a id="operations-segment-optimization"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/segment-optimization/ -->
...
> If you still see some performance issue after optimizing segment size, you may need to find the optimal settings for your workload.

---

<a id="operations-mixed-workloads"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/mixed-workloads/ -->
...
- `druid.query.scheduler.laning.strategy` – The strategy used to assign queries to lanes.
  You can use the built-in [“high/low” laning strategy](#configuration--highlow-laning-strategy), or [define your own laning strategy manually](#configuration--manual-laning-strategy).
...
Consider also defining a [prioritization strategy](#configuration--prioritization-strategies) for the Broker to label queries as high or low priority. Otherwise, manually set the priority for incoming queries on the [query context](#querying-query-context-reference).
...
With manual laning, incoming queries can be labeled with the desired lane in the `lane` parameter of the [query context](#querying-query-context-reference).

See [Query prioritization and laning](#configuration--query-prioritization-and-laning) for additional details on query laning configuration.
...
Define a [load rule](#operations-rule-configuration--load-rules) to indicate how segment replicas should be assigned to different Historical tiers. For example, you may store segments of more recent data on more powerful hardware for better performance.
...
Interval and period load rules must be accompanied by corresponding [drop rules](#operations-rule-configuration--drop-rules).
...
```text
{"type" : "loadByPeriod","period" : "P1M","includeFuture" : true,"tieredReplicants": {"hot": 3,"_default_tier" : 1}}
```

See [Load rules](#operations-rule-configuration--load-rules) for more information on segment load rules. Visit [Tutorial: Configuring data retention](#tutorials-tutorial-retention) for an example of setting retention rules from the Druid web console.
...
See [Historical general configuration](#configuration--historical-general-configuration) for more details on these properties.
...
See [Broker configuration](#configuration--broker-process-configs) for more details on these properties.
...
If you plan to run Druid SQL queries, also [enable routing of SQL queries](#design-router--routing-of-sql-queries-using-strategies) by setting the following:

```text
druid.router.sql.enable=true 
```

See [Router process](#design-router--example-production-configuration) for an example production configuration.

See [Multitenancy considerations](#querying-multitenancy) for applying query concurrency to multitenant workloads.

---

<a id="operations-http-compression"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/http-compression/ -->
...
<a id="operations-clean-metadata-store"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/clean-metadata-store/ -->
...
<a id="operations-request-logging"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/request-logging/ -->
...
- [`file`](#configuration--file-request-logging): Stores logs to disk.
- [`emitter`](#configuration--emitter-request-logging): Logs request to an external location, which is configured through an emitter.
- [`slf4j`](#configuration--slf4j-request-logging): Logs queries via the SLF4J Java logging API.
- [`filtered`](#configuration--filtered-request-logging): Filters requests by query type or execution time before logging the filtered queries by the delegated request logger.
- [`composing`](#configuration--composing-request-logging): Logs all requests to multiple request loggers.
- [`switching`](#configuration--switching-request-logging): Logs native queries and SQL queries to separate request loggers.

Define the type of request logger in `druid.request.logging.type`.
See the [Request logging configuration](#configuration--request-logging) for properties to set for each type of request logger.
...
- [`logging`](#configuration--logging-emitter-module): Emits metrics to Log4j 2. See [Logging](#configuration-logging) to configure Log4j 2 for use with Druid.
- [`http`](#configuration--http-emitter-module): Sends HTTP `POST` requests containing the metrics in JSON format to a user-defined endpoint.
- [`parametrized`](#configuration--parametrized-http-emitter-module): Operates like the `http` emitter but fine-tunes the recipient URL based on the event feed.
- [`composing`](#configuration--composing-emitter-module): Emits metrics to multiple emitter types.
- [`graphite`](#configuration--graphite-emitter): Emits metrics to a [Graphite](https://graphiteapp.org/) Carbon service.

Specify these properties in the `common.runtime.properties` file.
See the [Metrics emitters configuration](#configuration--metrics-emitters) for properties to set for each type of metrics emitter.
You must restart Druid for the changes to take effect.
...
<a id="operations-alerts"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/alerts/ -->
...
<a id="operations-high-availability"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/high-availability/ -->
...
<a id="operations-rolling-updates"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/rolling-updates/ -->
...
3. Broker
4. Router
5. Overlord (Note that you can upgrade the Overlord before any Middle Manager processes if you use [autoscaling-based replacement](#operations-rolling-updates--autoscaling-based-replacement).)
...
<a id="operations-rule-configuration"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/rule-configuration/ -->
...
<a id="operations-migrate-from-firehose"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/migrate-from-firehose/ -->
...
Druid converts the spec into one with a defined input source. For example, it converts the [example firehose ingestion spec](#operations-migrate-from-firehose--example-firehose-ingestion-spec) below into the [example ingestion spec after migration](#operations-migrate-from-firehose--example-ingestion-spec-after-migration).
...
To update your ingestion spec manually, copy your existing spec into a new file. Refer to [Native batch ingestion with firehose (Deprecated)](https://druid.apache.org/docs/latest/ingestion/native-batch-firehose) for a description of firehose properties.

Edit the new file as follows:

1. In the `ioConfig` component, replace the `firehose` definition with an `inputSource` definition for your chosen input source. See [Native batch input sources](#ingestion-input-sources) for details.
...
6. Save the file.
   You can check the format of your new ingestion file against the [migrated example](#operations-migrate-from-firehose--example-ingestion-spec-after-migration) below.
...
The following example illustrates the result of migrating the [example firehose ingestion spec](#operations-migrate-from-firehose--example-firehose-ingestion-spec) to a spec with an input source:
...
- [Ingestion](#ingestion): Overview of the Druid ingestion process.
- [Native batch ingestion](#ingestion-native-batch): Description of the supported native batch indexing tasks.
- [Ingestion spec reference](#ingestion-ingestion-spec): Description of the components and properties in the ingestion spec.

---

<a id="operations-dump-segment"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/dump-segment/ -->
...
object per line, using the default serialization for each column. Normally all columns are included, but if you like, you can limit the dump to specific columns with `--column name`.
...
format as returned by the [SegmentMetadata query](#querying-segmentmetadataquery).
...
```text
{"bitmapSerdeFactory": {"type": "roaring" },"bitmaps": {"isRobot": {"false": "//aExfu+Nv3X...","true": "gAl7OoRByQ..."}}}
```

With `--dump nested`, this tool can be used to examine Druid [nested columns](#querying-nested-columns). Using
`nested` always requires exactly one `--column name` argument, and takes an optional argument to specify a specific
nested field in [JSONPath syntax](#querying-sql-json-functions--jsonpath-syntax), `--nested-path $.path.to.field`.
...
| --filter json | JSON-encoded [query filter](#querying-filters). Omit to include all rows. Only used if dumping rows. | no |
| --time-iso8601 | Format \_\_time column in ISO8601 format rather than long. Only used if dumping rows. | no |
| --decompress-bitmaps | Dump bitmaps as arrays rather than base64-encoded compressed bitmaps. Only used if dumping bitmaps. | no |
| --nested-path | Specify a specific nested column field using [JSONPath syntax](#querying-sql-json-functions--jsonpath-syntax). Only used if dumping a nested column. | no |

---

<a id="operations-reset-cluster"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/reset-cluster/ -->
...
<a id="operations-insert-segment-to-db"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/insert-segment-to-db/ -->
...
2. Ingestion methods that allocate segments optimistically (such as native Kafka or Kinesis stream ingestion, or native
   ...
   the metadata store originally (and therefore *should* be used) from the segments that did not (and therefore
   *should not* be used).
...
<a id="operations-pull-deps"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/pull-deps/ -->
...
>
> For Druid community extensions listed [here](#configuration-extensions), the groupId is "org.apache.druid.extensions.contrib" and the artifactId is the name of the extension.

---

<a id="operations-deep-storage-migration"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/deep-storage-migration/ -->
...
For information on what path structure to use in the new deep storage, please see [deep storage migration options](#operations-export-metadata--deep-storage-migration).

Druid provides an [Export Metadata Tool](#operations-export-metadata) for exporting metadata from Derby into CSV files
which can then be reimported.

By setting [deep storage migration options](#operations-export-metadata--deep-storage-migration), the `export-metadata` tool will export CSV files where the segment load specs have been rewritten to load from your new deep storage location.
...
Please refer to [import commands](#operations-export-metadata--importing-metadata) for examples. Only the `druid_segments` table needs to be imported.

After importing the segment table successfully, you can now restart your cluster.

---

<a id="operations-export-metadata"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/export-metadata/ -->
...
<a id="operations-metadata-migration"></a>

<!-- source_url: https://druid.apache.org/docs/latest/operations/metadata-migration/ -->
...
for [deep storage migration](#operations-deep-storage-migration).
...
The [MySQL extension](https://druid.apache.org/docs/latest/development/extensions-core/mysql) and [PostgreSQL extension](https://druid.apache.org/docs/latest/development/extensions-core/postgresql) docs have instructions for initial database setup.
...
After initializing the tables, please refer to the [import commands](#operations-export-metadata--importing-metadata) for your target database.

After importing the metadata successfully, you can now restart your cluster.

---

<a id="development-overview"></a>

<!-- source_url: https://druid.apache.org/docs/latest/development/overview/ -->
...
Data in Druid is stored in a custom column format known as a [segment](#design-segments). Segments are composed of
...
Druid streaming tasks are based on the 'seekable stream' classes such as `SeekableStreamSupervisor.java`, `SeekableStreamIndexTask.java`, and `SeekableStreamIndexTaskRunner.java`. The data processing happens through
...
<a id="development-modules"></a>

<!-- source_url: https://druid.apache.org/docs/latest/development/modules/ -->
...
2. Add a new input source by extending `org.apache.druid.data.input.InputSource`.
3. Add a new input entity by extending `org.apache.druid.data.input.InputEntity`.
4. Add a new input source reader if necessary by extending `org.apache.druid.data.input.InputSourceReader`. You can use `org.apache.druid.data.input.impl.InputEntityIteratingReader` in most cases.
5. Add a new input format by extending `org.apache.druid.data.input.InputFormat`.
6. Add a new input entity reader by extending `org.apache.druid.data.input.TextReader` for text formats or `org.apache.druid.data.input.IntermediateRowParsingReader` for binary formats.
7. Add Aggregators by extending `org.apache.druid.query.aggregation.AggregatorFactory`, `org.apache.druid.query.aggregation.Aggregator`,
   and `org.apache.druid.query.aggregation.BufferAggregator`.
8. Add PostAggregators by extending `org.apache.druid.query.aggregation.PostAggregator`.
9. Add ExtractionFns by extending `org.apache.druid.query.extraction.ExtractionFn`.
10. Add Complex metrics by extending `org.apache.druid.segment.serde.ComplexMetricSerde`.
11. Add new Query types by extending `org.apache.druid.query.QueryRunnerFactory`, `org.apache.druid.query.QueryToolChest`, and
    `org.apache.druid.query.Query`.
12. Add new Jersey resources by calling `Jerseys.addResource(binder, clazz)`.
13. Add new Jetty filters by extending `org.apache.druid.server.initialization.jetty.ServletFilterHolder`.
14. Add new secret providers by extending `org.apache.druid.metadata.PasswordProvider`.
15. Add new dynamic configuration providers by extending `org.apache.druid.metadata.DynamicConfigProvider`.
16. Add new ingest transform by implementing the `org.apache.druid.segment.transform.Transform` interface from the `druid-processing` package.
17. Bundle your extension with all the other Druid extensions
...
The easiest way of testing the segment killing is marking a segment as not used and then starting a killing task in the [web console](#operations-web-console).
...
in [native parallel indexing](#ingestion-native-batch).
...
```java
import com.fasterxml.jackson.annotation.JsonCreator; import com.fasterxml.jackson.annotation.JsonProperty; import org.apache.druid.segment.transform.RowFunction; import org.apache.druid.segment.transform.Transform;
public class MyTransform implements Transform {private final String name;
@JsonCreator public MyTransform(@JsonProperty("name") final String name ) {this.name = name;}
@JsonProperty @Override public String getName() {return name;}
@Override public RowFunction getRowFunction() {return new MyRowFunction();}
static class MyRowFunction implements RowFunction {@Override public Object eval(Row row) {return "transformed-value";}}}
```

Then register your transform as a Jackson module.

```java
import com.fasterxml.jackson.databind.Module; import com.fasterxml.jackson.databind.jsontype.NamedModule; import com.fasterxml.jackson.databind.module.SimpleModule; import com.google.inject.Binder; import com.google.common.collect.ImmutableList; import org.apache.druid.initialization.DruidModule;
public class MyTransformModule implements DruidModule {@Override public List<? extends Module> getJacksonModules() {return return ImmutableList.of(new SimpleModule("MyTransformModule").registerSubtypes(new NamedType(MyTransform.class, "my-transform")) ):}
...
During `mvn install`, maven will install your extension to the local maven repository, and then call [pull-deps](#operations-pull-deps) to pull your extension from
...
<a id="development-javascript"></a>

<!-- source_url: https://druid.apache.org/docs/latest/development/javascript/ -->
...
- [Aggregators](#querying-aggregations--javascript-aggregator)
- [Extraction functions](#querying-dimensionspecs--javascript-extraction-function)
- [Filters](#querying-filters--javascript-filter)
- [Post-aggregators](#querying-post-aggregations--javascript-post-aggregator)
- [Router strategy](#design-router--javascript)
- [Worker select strategy](#configuration--javascript-1)
...
the [configuration property](#configuration--javascript)
...
than performance or memory use. If security is an issue, or if performance and memory use are of the utmost importance, we recommend developing a native Druid extension.
...
<!-- source_url: https://druid.apache.org/docs/latest/development/build/ -->
...
- See our [Java documentation](#operations-java) for information about obtaining a supported JDK
...
<a id="development-versioning"></a>

<!-- source_url: https://druid.apache.org/docs/latest/development/versioning/ -->
...
<a id="development-contribute-to-docs"></a>

<!-- source_url: https://druid.apache.org/docs/latest/development/contribute-to-docs/ -->
...
<a id="development-experimental"></a>

<!-- source_url: https://druid.apache.org/docs/latest/development/experimental/ -->
...
<a id="release-info-release-notes"></a>

<!-- source_url: https://druid.apache.org/docs/latest/release-info/release-notes/ -->
...
Review the [upgrade notes](#release-info-release-notes--upgrade-notes) and [incompatible changes](#release-info-release-notes--incompatible-changes) before you upgrade to Druid 37.0.0.
If you are upgrading across multiple versions, see the [Upgrade notes](#release-info-upgrade-notes) page, which lists upgrade notes for the most recent Druid versions.
...
```text
POST /druid/coordinator/v1/config/broker {"queryBlocklist": [{"ruleName": "block-wikipedia-groupbys","dataSources": ["wikipedia"],"queryTypes": ["groupBy"],"contextMatches": {"priority": "0"}}]}
```
...
This change has upgrade impacts for metadata storage and metadata caching. For more information, see the [Metadata storage for auto-compaction with compaction supervisors](#release-info-release-notes--metadata-storage-for-auto-compaction-with-compaction-supervisors) upgrade note.
...
If `druid.manager.segments.useIncrementalCache` is set to `never`, update it to `ifSynced` or `always`. For more information about the config, see [Segment metadata cache](#configuration--segment-metadata-cache-experimental).
...
<a id="release-info-upgrade-notes"></a>

<!-- source_url: https://druid.apache.org/docs/latest/release-info/upgrade-notes/ -->
...
We recommend one of Druid's other supported ingestion methods, such as [SQL-based ingestion](#multi-stage-query) or [MiddleManager-less ingestion using Kubernetes](https://druid.apache.org/docs/latest/development/extensions-core/k8s-jobs).
...
For more information about how to update your queries, see the [migration guide](#release-info-migr-ansi-sql-null).
...
Although you can manually alter your table to add the new `used_status_last_updated` column, Druid also provides a [CLI tool](#operations-metadata-migration--create-druid-tables) to do it.
...
When you set `arrayIngestMode` to `array` (recommended for SQL compliance), the MSQ task engine stores all ARRAY typed values in [ARRAY typed columns](#querying-arrays) and supports storing both VARCHAR and numeric typed arrays.

For backwards compatibility, `arrayIngestMode` defaults to `mvd`. When `"arrayIngestMode":"mvd"`, Druid only supports VARCHAR typed arrays and stores them as [multi-value string columns](#querying-multi-value-dimensions).

When you set `arrayIngestMode` to `none`, Druid throws an exception when trying to store any type of arrays.

For more information on how to ingest `ARRAY` typed columns with SQL-based ingestion, see [SQL data types](#querying-sql-data-types--arrays) and [Array columns](#querying-arrays).
...
For more information, see [GroupBy queries](#querying-groupbyquery).
...
Use the new [smart segment loading](#configuration--smart-segment-loading) mode instead.
...
When you issue a kill task, Druid marks the underlying segments as unused only if explicitly specified. For more information, see the [API reference](#api-reference-data-management-api).
...
<a id="release-info-upgrade-notes--other-changes"></a>
...
<a id="release-info-migration-guide"></a>

<!-- source_url: https://druid.apache.org/docs/latest/release-info/migration-guide/ -->
...
Druid now supports SQL-compliant array types. Whenever possible, you should use the array type over multi-value dimensions. See [Migration guide: MVDs to arrays](#release-info-migr-mvd-array).

Druid encodes string columns into dictionaries for better compression. Front-coded dictionary encoding reduces storage and improves performance by optimizing for strings that share similar beginning substrings. See [Migration guide: front-coded dictionaries](#release-info-migr-front-coded-dict) for more information.

Druid allows you to set a byte-based limit on subquery size to prevent Brokers from running out of memory when handling large subqueries. The byte-based subquery limit overrides Druid's row-based subquery limit. We recommend that you move towards using byte-based limits starting in Druid 30.0.0. See [Migration guide: subquery limit](#release-info-migr-subquery-limit) for more information.

By default, the Druid [null handling](#querying-sql-data-types--null-values) mode is now compliant with ANSI SQL.
This guide provides strategies for Druid operators and users who rely on the legacy Druid null handling behavior in their applications to transition to ANSI SQL compliant mode. See [Migration guide: SQL compliant mode](#release-info-migr-ansi-sql-null) for more information.

---

<a id="release-info-migr-mvd-array"></a>

<!-- source_url: https://druid.apache.org/docs/latest/release-info/migr-mvd-array/ -->
...
<a id="release-info-migr-front-coded-dict"></a>

<!-- source_url: https://druid.apache.org/docs/latest/release-info/migr-front-coded-dict/ -->
...
Front coding is an optional incremental encoding strategy that lets you further compress STRING and [COMPLEX<json>](#querying-nested-columns) columns in Druid with minimal performance impact.
...
To enable front coding, set `indexSpec.stringDictionaryEncoding.type` to `frontCoded` in the `tuningConfig` object of your [ingestion spec](#ingestion-ingestion-spec).
...
<a id="release-info-migr-subquery-limit"></a>

<!-- source_url: https://druid.apache.org/docs/latest/release-info/migr-subquery-limit/ -->
...
<a id="release-info-migr-ansi-sql-null"></a>

<!-- source_url: https://druid.apache.org/docs/latest/release-info/migr-ansi-sql-null/ -->
...
Follow the [Null handling tutorial](https://druid.apache.org/docs/latest/tutorials/tutorial-sql-null) to learn how null handling works in Druid.
...
- Modify incoming data to either [avoid nulls](#release-info-migr-ansi-sql-null--replace-null-values-at-ingestion-time) or [avoid empty strings](#release-info-migr-ansi-sql-null--coerce-empty-strings-to-null-at-ingestion-time) to achieve the same query behavior as legacy mode. This means modifying your ingestion SQL queries and ingestion specs to handle nulls or empty strings.
...
- Preserve null values and [update all of your SQL queries to be ANSI SQL compliant](#release-info-migr-ansi-sql-null--rewrite-your-queries-to-be-sql-compliant).
...
<div class="tabs-container tabList__CuJ"><ul><li>SQL-based batch</li><li>JSON-based batch</li></ul><div><div><div><div><pre><code><span><span>REPLACE</span><span> </span><span>INTO</span><span> </span><span>"no_nulls_example"</span><span> OVERWRITE </span><span>ALL</span><span></span> </span><span><span></span><span>WITH</span><span> </span><span>"ext"</span><span> </span><span>AS</span><span> </span><span>(</span><span></span> </span><span><span>  </span><span>SELECT</span><span> </span><span>*</span><span></span> </span><span><span>  </span><span>FROM</span><span> </span><span>TABLE</span><span>(</span><span></span> </span><span><span>    EXTERN</span><span>(</span><span></span> </span><span><span>      </span><span>'{"type":"inline","data":"{\"time\":\"2024-01-01T00:00:00.000Z\",\"string_example\":\"my_string\",\"number_example\":99}\n{\"time\":\"2024-01-02T00:00:00.000Z\",\"string_example\":\"\",\"number_example\":0}\n{\"time\":\"2024-01-03T00:00:00.000Z\",\"string_example\":null,\"number_example\":null}"}'</span><span>,</span><span></span> </span><span><span>      </span><span>'{"type":"json"}'</span><span></span> </span><span><span>    </span><span>)</span><span></span> </span><span><span>  </span><span>)</span><span> EXTEND </span><span>(</span><span>"time"</span><span> </span><span>VARCHAR</span><span>,</span><span> </span><span>"string_example"</span><span> </span><span>VARCHAR</span><span>,</span><span> </span><span>"number_example"</span><span> </span><span>BIGINT</span><span>)</span><span></span> </span><span><span></span><span>)</span><span></span> </span><span><span></span><span>SELECT</span><span></span> </span><span><span>  TIME_PARSE</span><span>(</span><span>"time"</span><span>)</span><span> </span><span>AS</span><span> </span><span>"__time"</span><span>,</span><span></span> </span><span><span>  </span><span>-- Replace any null string values with an empty string</span><span></span> </span><span><span>  </span><span>COALESCE</span><span>(</span><span>"string_example"</span><span>,</span><span>''</span><span>)</span><span> </span><span>AS</span><span> string_example</span><span>,</span><span></span> </span><span><span>  </span><span>-- Replace any null numeric values with 0</span><span></span> </span><span><span>  NVL</span><span>(</span><span>"number_example"</span><span>,</span><span>0</span><span>)</span><span> </span><span>AS</span><span> number_example</span> </span><span><span></span><span>FROM</span><span> </span><span>"ext"</span><span></span> </span><span><span>PARTITIONED </span><span>BY</span><span> </span><span>MONTH</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>{</span><span></span> </span><span><span>  </span><span>"type"</span><span>:</span><span> </span><span>"index_parallel"</span><span>,</span><span></span> </span><span><span>  </span><span>"spec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>    </span><span>"ioConfig"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>      </span><span>"type"</span><span>:</span><span> </span><span>"index_parallel"</span><span>,</span><span></span> </span><span><span>      </span><span>"inputSource"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"type"</span><span>:</span><span> </span><span>"inline"</span><span>,</span><span></span> </span><span><span>        </span><span>"data"</span><span>:</span><span> </span><span>"{\"time\":\"2024-01-01T00:00:00.000Z\",\"string_example\":\"my_string\",\"number_example\":99}\n{\"time\":\"2024-01-02T00:00:00.000Z\",\"string_example\":\"\",\"number_example\":0}\n{\"time\":\"2024-01-03T00:00:00.000Z\",\"string_example\":null,\"number_example\":null}"</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"inputFormat"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"type"</span><span>:</span><span> </span><span>"json"</span><span></span> </span><span><span>      </span><span>}</span><span></span> </span><span><span>    </span><span>}</span><span>,</span><span></span> </span><span><span>    </span><span>"tuningConfig"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>      </span><span>"type"</span><span>:</span><span> </span><span>"index_parallel"</span><span>,</span><span></span> </span><span><span>      </span><span>"partitionsSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"type"</span><span>:</span><span> </span><span>"dynamic"</span><span></span> </span><span><span>      </span><span>}</span><span></span> </span><span><span>    </span><span>}</span><span>,</span><span></span> </span><span><span>    </span><span>"dataSchema"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>      </span><span>"dataSource"</span><span>:</span><span> </span><span>"inline_data_native"</span><span>,</span><span></span> </span><span><span>      </span><span>"timestampSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"column"</span><span>:</span><span> </span><span>"time"</span><span>,</span><span></span> </span><span><span>        </span><span>"format"</span><span>:</span><span> </span><span>"iso"</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"dimensionsSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"dimensions"</span><span>:</span><span> </span><span>[</span><span></span> </span><span><span>          </span><span>"string_example"</span><span>,</span><span></span> </span><span><span>          </span><span>{</span><span></span> </span><span><span>            </span><span>"type"</span><span>:</span><span> </span><span>"long"</span><span>,</span><span></span> </span><span><span>            </span><span>"name"</span><span>:</span><span> </span><span>"number_example"</span><span></span> </span><span><span>          </span><span>}</span><span></span> </span><span><span>        </span><span>]</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"granularitySpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"queryGranularity"</span><span>:</span><span> </span><span>"none"</span><span>,</span><span></span> </span><span><span>        </span><span>"rollup"</span><span>:</span><span> </span><span>false</span><span>,</span><span></span> </span><span><span>        </span><span>"segmentGranularity"</span><span>:</span><span> </span><span>"MONTH"</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"transformSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"transforms"</span><span>:</span><span> </span><span>[</span><span></span> </span><span><span>          </span><span>{</span><span></span> </span><span><span>            </span><span>"type"</span><span>:</span><span> </span><span>"expression"</span><span>,</span><span></span> </span><span><span>            </span><span>"name"</span><span>:</span><span> </span><span>"string_example"</span><span>,</span><span></span> </span><span><span>            </span><span>"expression"</span><span>:</span><span> </span><span>"COALESCE(\"string_example\",'')"</span><span></span> </span><span><span>          </span><span>}</span><span>,</span><span></span> </span><span><span>          </span><span>{</span><span></span> </span><span><span>            </span><span>"type"</span><span>:</span><span> </span><span>"expression"</span><span>,</span><span></span> </span><span><span>            </span><span>"name"</span><span>:</span><span> </span><span>"number_example"</span><span>,</span><span></span> </span><span><span>            </span><span>"expression"</span><span>:</span><span> </span><span>"NVL(\"number_example\",0)"</span><span></span> </span><span><span>          </span><span>}</span><span></span> </span><span><span>        </span><span>]</span><span></span> </span><span><span>      </span><span>}</span><span></span> </span><span><span>    </span><span>}</span><span></span> </span><span><span>  </span><span>}</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
<div class="tabs-container tabList__CuJ"><ul><li>SQL-based batch</li><li>JSON-based batch</li></ul><div><div><div><div><pre><code><span><span>REPLACE</span><span> </span><span>INTO</span><span> </span><span>"null_string"</span><span> OVERWRITE </span><span>ALL</span><span></span> </span><span><span></span><span>WITH</span><span> </span><span>"ext"</span><span> </span><span>AS</span><span> </span><span>(</span><span></span> </span><span><span>  </span><span>SELECT</span><span> </span><span>*</span><span></span> </span><span><span>  </span><span>FROM</span><span> </span><span>TABLE</span><span>(</span><span></span> </span><span><span>    EXTERN</span><span>(</span><span></span> </span><span><span>      </span><span>'{"type":"inline","data":"{\"time\":\"2024-01-01T00:00:00.000Z\",\"string_example\":\"my_string\"}\n{\"time\":\"2024-01-02T00:00:00.000Z\",\"string_example\":\"\"}\n{\"time\":\"2024-01-03T00:00:00.000Z\",\"string_example\":null}"}'</span><span>,</span><span></span> </span><span><span>      </span><span>'{"type":"json"}'</span><span></span> </span><span><span>    </span><span>)</span><span></span> </span><span><span>  </span><span>)</span><span> EXTEND </span><span>(</span><span>"time"</span><span> </span><span>VARCHAR</span><span>,</span><span> </span><span>"string_example"</span><span> </span><span>VARCHAR</span><span>)</span><span></span> </span><span><span></span><span>)</span><span></span> </span><span><span></span><span>SELECT</span><span></span> </span><span><span>  TIME_PARSE</span><span>(</span><span>"time"</span><span>)</span><span> </span><span>AS</span><span> </span><span>"__time"</span><span>,</span><span></span> </span><span><span>  </span><span>NULLIF</span><span>(</span><span>"string_example"</span><span>,</span><span>''</span><span>)</span><span> </span><span>AS</span><span> </span><span>"string_example"</span><span></span> </span><span><span></span><span>FROM</span><span> </span><span>"ext"</span><span></span> </span><span><span>PARTITIONED </span><span>BY</span><span> </span><span>MONTH</span> </span></code></pre><div></div></div></div></div><div><div><div><pre><code><span><span>{</span><span></span> </span><span><span>  </span><span>"type"</span><span>:</span><span> </span><span>"index_parallel"</span><span>,</span><span></span> </span><span><span>  </span><span>"spec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>    </span><span>"ioConfig"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>      </span><span>"type"</span><span>:</span><span> </span><span>"index_parallel"</span><span>,</span><span></span> </span><span><span>      </span><span>"inputSource"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"type"</span><span>:</span><span> </span><span>"inline"</span><span>,</span><span></span> </span><span><span>        </span><span>"data"</span><span>:</span><span> </span><span>"{\"time\":\"2024-01-01T00:00:00.000Z\",\"string_example\":\"my_string\"}\n{\"time\":\"2024-01-02T00:00:00.000Z\",\"string_example\":\"\"}\n{\"time\":\"2024-01-03T00:00:00.000Z\",\"string_example\":null}"</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"inputFormat"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"type"</span><span>:</span><span> </span><span>"json"</span><span></span> </span><span><span>      </span><span>}</span><span></span> </span><span><span>    </span><span>}</span><span>,</span><span></span> </span><span><span>    </span><span>"tuningConfig"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>      </span><span>"type"</span><span>:</span><span> </span><span>"index_parallel"</span><span>,</span><span></span> </span><span><span>      </span><span>"partitionsSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"type"</span><span>:</span><span> </span><span>"dynamic"</span><span></span> </span><span><span>      </span><span>}</span><span></span> </span><span><span>    </span><span>}</span><span>,</span><span></span> </span><span><span>    </span><span>"dataSchema"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>      </span><span>"dataSource"</span><span>:</span><span> </span><span>"null_string"</span><span>,</span><span></span> </span><span><span>      </span><span>"timestampSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"column"</span><span>:</span><span> </span><span>"time"</span><span>,</span><span></span> </span><span><span>        </span><span>"format"</span><span>:</span><span> </span><span>"iso"</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"transformSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"transforms"</span><span>:</span><span> </span><span>[</span><span></span> </span><span><span>          </span><span>{</span><span></span> </span><span><span>            </span><span>"type"</span><span>:</span><span> </span><span>"expression"</span><span>,</span><span></span> </span><span><span>            </span><span>"expression"</span><span>:</span><span> </span><span>"case_searched((\"string_example\" == ''),null,\"string_example\")"</span><span>,</span><span></span> </span><span><span>            </span><span>"name"</span><span>:</span><span> </span><span>"string_example"</span><span></span> </span><span><span>          </span><span>}</span><span></span> </span><span><span>        </span><span>]</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"dimensionsSpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"dimensions"</span><span>:</span><span> </span><span>[</span><span></span> </span><span><span>          </span><span>"string_example"</span><span></span> </span><span><span>        </span><span>]</span><span></span> </span><span><span>      </span><span>}</span><span>,</span><span></span> </span><span><span>      </span><span>"granularitySpec"</span><span>:</span><span> </span><span>{</span><span></span> </span><span><span>        </span><span>"queryGranularity"</span><span>:</span><span> </span><span>"none"</span><span>,</span><span></span> </span><span><span>        </span><span>"rollup"</span><span>:</span><span> </span><span>false</span><span>,</span><span></span> </span><span><span>        </span><span>"segmentGranularity"</span><span>:</span><span> </span><span>"month"</span><span></span> </span><span><span>      </span><span>}</span><span></span> </span><span><span>    </span><span>}</span><span></span> </span><span><span>  </span><span>}</span><span></span> </span><span><span></span><span>}</span> </span></code></pre><div></div></div></div></div></div></div>
...
- [Null handling tutorial](https://druid.apache.org/docs/latest/tutorials/tutorial-sql-null) to learn how the default null handling works in Druid.
- [Null values](#querying-sql-data-types--null-values) for a description of Druid's null values.
- [Handling null values](#design-segments--handling-null-values) for details about how Druid stores null values.

---

<a id="misc-papers-and-talks"></a>

<!-- source_url: https://druid.apache.org/docs/latest/misc/papers-and-talks/ -->
