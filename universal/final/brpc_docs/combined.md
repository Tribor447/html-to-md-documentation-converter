...
- Other pages
  ...
  - [Release Guide](#community-release)
...
<a id="index--brpc-overview"></a>

##### [bRPC overview](#overview)

The basic description of bRPC.

<a id="index--download"></a>
...
<a id="index--performance-benchmark"></a>
...
<a id="index--use-cases-inside-baidu"></a>
...
<a id="benchmark--performance-benchmark"></a>
...
<a id="benchmark--section-4"></a>

# 开始测试

<a id="benchmark--section-5"></a>
...
- brpc: 当请求包小于16KB时，单连接下的吞吐超过了多连接的ubrpc\_mc和thrift\_mc，随着请求包变大，内核对单个连接的写入速度成为瓶颈。而多连接下的brpc则达到了测试中最高的2.3GB/s。注意: 虽然使用连接池的brpc在发送大包时吞吐更高，但也会耗费更多的CPU（UB和thrift也是这样）。下图中的单连接brpc已经可以提供800多兆的吞吐，足以打满万兆网卡，而使用的CPU可能只有多链接下的1/2(写出过程是[wait-free的](#rpc-in-depth-io--sending-messages))，真实系统中请优先使用单链接。
...
<a id="benchmark--client-server-qps-2"></a>
...
<a id="benchmark--client-server-qps-cdf"></a>
...
<a id="benchmark--client-server-qps-3"></a>
...
<a id="benchmark--client-server-qps-cdfbuiltin-servicesvars-2"></a>
<a id="benchmark--client-server-qps-cdf-2"></a>
...
<a id="benchmark--client-server-qps-cdfbuiltin-servicesvars-3"></a>
<a id="benchmark--client-server-qps-cdf-3"></a>
...
<a id="benchmark--client-server-server-qps-cdf"></a>
...
<a id="blogs-releases"></a>

<!-- source_url: https://brpc.apache.org/docs/blogs/releases/ -->

<!-- page_index: 5 -->

# Release

[Edit this page](https://github.com/apache/brpc-website/edit/master/content/en/docs/blogs/releases/_index.md)
 [Create documentation issue](https://github.com/apache/brpc-website/issues/new?title=Release)
 [Create project issue](https://github.com/apache/brpc/issues/new)

<a id="blogs-releases--release"></a>

# Release

---

<a id="blogs-releases--brpc-1.5.0"></a>

##### [bRPC 1.5.0](https://brpc.apache.org/docs/blogs/releases/1.5.0/)

Apache bRPC 1.5.0 Release Version.

<a id="blogs-releases--brpc-1.4.0"></a>

##### [bRPC 1.4.0](https://brpc.apache.org/docs/blogs/releases/1.4.0/)

Apache bRPC 1.4.0 Release Version.

<a id="blogs-releases--brpc-1.3.0"></a>

##### [bRPC 1.3.0](https://brpc.apache.org/docs/blogs/releases/1.3.0/)

Apache bRPC 1.3.0 Release Version.

<a id="blogs-releases--brpc-1.2.0"></a>

##### [bRPC 1.2.0](https://brpc.apache.org/docs/blogs/releases/1.2.0/)

Apache bRPC 1.2.0 Release Version.

<a id="blogs-releases--brpc-1.1.0"></a>

##### [bRPC 1.1.0](https://brpc.apache.org/docs/blogs/releases/1.1.0/)

Apache bRPC 1.1.0 Release Version.

<a id="blogs-releases--brpc-1.0.0"></a>
...
<a id="bthread-bthread-or-not--bthread-or-not"></a>
...
另外当你有类似线程池的需求时，像执行一类job的线程池时，也可以用bthread代替。如果对job的执行顺序有要求，你可以使用基于bthread的[ExecutionQueue](#bthread-execution-queue)。
...
<a id="bthread-bthread--bthread"></a>
...
<a id="bthread-bthread--q-bthread-coroutine"></a>
...
不应该。除非你需要在一次RPC过程中[让一些代码并发运行](#bthread-bthread-or-not)，你不应该直接调用bthread函数，把这些留给brpc做更好。

<a id="bthread-bthread--qbthread-pthread-worker"></a>
<a id="bthread-bthread--q-bthread-pthread-worker"></a>
...
<a id="bthread-bthread--q-bthread-pthread"></a>
...
<a id="bthread-bthread--q-pthread-bthread-api"></a>
...
<a id="bthread-bthread--q-bthread-channel"></a>
...
我们需要的往往是buffered channel，扮演的是队列和有序执行的作用，bthread提供了[ExecutionQueue](#bthread-execution-queue)，可以完成这个目的。
...
<a id="bthread-execution-queue--execution-queue"></a>
...
<a id="bthread-execution-queue--section-2"></a>
...
<a id="bthread-execution-queue--section-3"></a>

# 使用方式

<a id="bthread-execution-queue--section-4"></a>
...
<a id="bthread-execution-queue--executionqueue-2"></a>
<a id="bthread-execution-queue--executionqueue:-2"></a>
...
<a id="bthread-execution-queue--section-5"></a>
...
<a id="bthread-execution-queue--section-6"></a>
...
<a id="bthread-thread-local--thread-local-2"></a>
...
<a id="builtin-services-buildin_services--builtin-services-2"></a>
...
<a id="builtin-services-connections--connections"></a>
...
<a id="builtin-services-contention_profiler--contention-profiler"></a>
...
<a id="builtin-services-contention_profiler--section-2"></a>
...
<a id="builtin-services-cpu_profiler--cpu-profiler"></a>
...
2. 定义宏BRPC\_ENABLE\_CPU\_PROFILER, 一般加入编译参数-DBRPC\_ENABLE\_CPU\_PROFILER。
...
<a id="builtin-services-cpu_profiler--section-2"></a>
...
<a id="builtin-services-cpu_profiler--section-3"></a>
...
<a id="builtin-services-cpu_profiler--section-4"></a>

# 控制采样时间

url加上?seconds=秒数，如/hotspots/cpu?seconds=5

<a id="builtin-services-cpu_profiler--section-5"></a>
...
<a id="builtin-services-cpu_profiler--section-6"></a>
...
<a id="builtin-services-flags--flags"></a>
...
<a id="builtin-services-heap_profiler--heap-profiler"></a>
...
```
$ tools/pprof --text localhost:9002/pprof/heap Fetching /pprof/heap profile from http://localhost:9002/pprof/heap to
...
<a id="builtin-services-heap_profiler--section-2"></a>
...
<a id="builtin-services--builtin-services"></a>
...
<a id="builtin-services--builtin-services-2"></a>
...
<a id="builtin-services--cpu-profiler"></a>

##### [cpu profiler](#builtin-services-cpu_profiler)

Learn about cpu profiler service.

<a id="builtin-services--heap-profiler"></a>

##### [heap profiler](#builtin-services-heap_profiler)

Learn about heap profiler service.

<a id="builtin-services--contention-profiler"></a>
...
- [数据展现](#builtin-services-rpcz--section)
  - [第一层](#builtin-services-rpcz--section)
  - [第二层](#builtin-services-rpcz--section)
- [Annotation](#builtin-services-rpcz--annotation)

<a id="builtin-services-rpcz--rpcz"></a>
...
<a id="builtin-services-rpcz--section-2"></a>

## 数据展现

/rpcz展现的数据分为两层。

<a id="builtin-services-rpcz--section-3"></a>
...
<a id="builtin-services-rpcz--section-4"></a>
...
<a id="builtin-services-status--status"></a>
...
<a id="builtin-services-vars--vars"></a>
...
```c++
#include <bvar/bvar.h>
...bvar::LatencyRecorder g_latency_recorder("client");  // expose this recorder ...void foo() {...g_latency_recorder << my_latency; ...}
```
...
- - [Best Practice for Naming:](#bvar-bvar-c--best-practice-for-naming)
...
<a id="bvar-bvar-c--bvar::variable:"></a>
...
```C++
#include <gflags/gflags.h> ...int main(int argc, char* argv[]) {if (google::SetCommandLineOption("bvar_dump", "true").empty()) {LOG(FATAL) << "Fail to enable bvar dump";} ...}
```
...
<a id="bvar-bvar-c--bvar::reducer"></a>
...
<a id="bvar-bvar-c--bvar::intrecorder"></a>
...
<a id="bvar-bvar-c--bvar::latencyrecoder"></a>
...
<a id="bvar-bvar-c--bvar::window"></a>
...
<a id="bvar-bvar-c--bvar::persecond"></a>
...
<a id="bvar-bvar-c--bvar::status"></a>
...
<a id="c-base-brpc-training-materials--brpc-training-materials"></a>
...
<a id="c-base-flatmap--section-2"></a>
...
<a id="c-base-iobuf--what-iobuf-can-t:"></a>
...
- [LogSink](#c-base-streaming-log--logsink)
  - [StringSink](#c-base-streaming-log--stringsink)

<a id="c-base-streaming-log--streaming-log"></a>
...
```c++
struct B {int value; }; std::ostream& operator<<(std::ostream& os, const B& b) {return os << "B{value=" << b.value << "}";}
struct C {string name; }; std::ostream& operator<<(std::ostream& os, const C& c) {return os << "C{name=" << c.name << "}";}
```
...
<a id="client-access-grpc--access-grpc"></a>
...
<a id="client-access-grpc--http:json-h2:json"></a>
...
<a id="client-access-grpc--http:proto-h2:proto"></a>
...
<a id="client-access-grpc--h2:grpc"></a>
...
<a id="client-access-grpc--h2:grpc-json"></a>
...
<a id="client-access-httph2--access-http:h2"></a>
...
<a id="client-access-memcached--access-memcached"></a>
...
- Support synchronous, asynchronous, semi-synchronous accesses etc. Support [ParallelChannel etc](#client-combo-channels) to define access patterns declaratively.
...
```c++
#include <brpc/memcache.h> #include <brpc/channel.h>
brpc::ChannelOptions options; options.protocol = brpc::PROTOCOL_MEMCACHE; if (channel.Init("0.0.0.0:11211", &options) != 0) {  // 11211 is the default port for memcached LOG(FATAL) << "Fail to init channel to memcached"; return -1;} ...
```
...
<a id="client-access-redis--access-redis"></a>
...
<a id="client-access-thrift--access-thrift"></a>
...
```c++
#include <brpc/channel.h> #include <brpc/thrift_message.h>         // Defines ThriftStub ...
DEFINE_string(server, "0.0.0.0:8019", "IP Address of thrift server"); DEFINE_string(load_balancer, "", "The algorithm for load balancing"); ...
brpc::ChannelOptions options; options.protocol = brpc::PROTOCOL_THRIFT; brpc::Channel thrift_channel; if (thrift_channel.Init(Flags_server.c_str(), FLAGS_load_balancer.c_str(), &options) != 0) {LOG(ERROR) << "Fail to initialize thrift channel"; return -1;}
brpc::ThriftStub stub(&thrift_channel); ...
// example::[EchoRequest/EchoResponse] are types generated by thrift example::EchoRequest req; example::EchoResponse res; req.data = "hello";
stub.CallMethod("Echo", &cntl, &req, &res, NULL); if (cntl.Failed()) {LOG(ERROR) << "Fail to send thrift request, " << cntl.ErrorText(); return -1;}
```
...
<a id="client-access-ub--access-ub"></a>
...
```idl
struct EchoRequest {string message; };
struct EchoResponse {string message; };
service EchoService {void Echo(EchoRequest req, out EchoResponse res); uint32_t EchoWithMultiArgs(EchoRequest req1, EchoRequest req2, out EchoResponse res1, out EchoResponse res2); };
```
...
<a id="client-access-ub--nshead-idl"></a>
...
<a id="client-access-ub--nshead-mcpack-idl"></a>
...
<a id="client-access-ub--nshead-blob"></a>
...
<a id="client-backup-request--backup-request"></a>
...
<a id="client-basics--bns:-bns-name"></a>
...
<a id="client-basics--file:-path"></a>
...
<a id="client-basics--list:-addr1-addr2"></a>
...
<a id="client-basics--https:-url"></a>
...
<a id="client-basics--consul:-service-name"></a>
...
```c++
class MyNamingServiceFilter : public brpc::NamingServiceFilter {public:bool Accept(const brpc::ServerNode& server) const {return server.tag == "main";} };
...
<a id="client-basics--inherit-google::protobuf::closure"></a>
...
```c++
const brpc::CallId cid1 = controller1->call_id(); const brpc::CallId cid2 = controller2->call_id(); ...stub.method1(controller1, request1, response1, done1); stub.method2(controller2, request2, response2, done2); ...brpc::Join(cid1); brpc::Join(cid2);
```
...
<a id="client-basics--should-brpc::controller-be-reused"></a>
...
```c++
// snippet1 for (int i = 0; i < n; ++i) {brpc::Controller controller; ...stub.CallSomething(..., &controller);}
// snippet2 brpc::Controller controller; for (int i = 0; i < n; ++i) {controller.Reset(); ...stub.CallSomething(..., &controller);}
```
...
class MyRetryPolicy : public brpc::RetryPolicy {public:bool DoRetry(const brpc::Controller* cntl) const {if (cntl->ErrorCode() == brpc::EHTTP && // http/h2 error cntl->http_response().status_code() == brpc::HTTP_STATUS_FORBIDDEN) {return true;} // Leave other cases to brpc.return brpc::DefaultRetryPolicy()->DoRetry(cntl);} }; ...
// Assign the instance to ChannelOptions.retry_policy.// NOTE: retry_policy must be kept valid during lifetime of Channel, and Channel does not retry_policy, so in most cases RetryPolicy should be created by singleton..brpc::ChannelOptions options; static MyRetryPolicy g_my_retry_policy; options.retry_policy = &g_my_retry_policy; ...
```
...
- PROTOCOL\_BAIDU\_STD or “baidu\_std”, which is [the standard binary protocol inside Baidu](https://github.com/apache/brpc/blob/master/docs/cn/baidu_std.md), using single connection by default.
- PROTOCOL\_HTTP or “http”, which is http/1.0 or http/1.1, using pooled connection by default (Keep-Alive).
  - Methods for accessing ordinary http services are listed in [Access http/h2](#client-access-httph2).
...
- PROTOCOL\_H2 or ”h2", which is http/2, using single connection by default.
  - Methods for accessing ordinary h2 services are listed in [Access http/h2](#client-access-httph2).
...
- PROTOCOL\_THRIFT or “thrift”, which is the protocol of [apache thrift](https://thrift.apache.org), using pooled connection by default, check out [Access thrift](#client-access-thrift) for details.
- PROTOCOL\_MEMCACHE or “memcache”, which is binary protocol of memcached, using **single connection** by default. Check out [Access memcached](#client-access-memcached) for details.
- PROTOCOL\_REDIS or “redis”, which is protocol of redis 1.2+ (the one supported by hiredis), using **single connection** by default. Check out [Access Redis](#client-access-redis) for details.
- PROTOCOL\_HULU\_PBRPC or “hulu\_pbrpc”, which is protocol of hulu-pbrpc, using single connection by default.
- PROTOCOL\_NOVA\_PBRPC or “nova\_pbrpc”, which is protocol of Baidu ads union, using pooled connection by default.
- PROTOCOL\_SOFA\_PBRPC or “sofa\_pbrpc”, which is protocol of sofa-pbrpc, using single connection by default.
- PROTOCOL\_PUBLIC\_PBRPC or “public\_pbrpc”, which is protocol of public\_pbrpc, using pooled connection by default.
- PROTOCOL\_UBRPC\_COMPACK or “ubrpc\_compack”, which is protocol of public/ubrpc, packing with compack, using pooled connection by default. check out [ubrpc (by protobuf)](#client-access-ub) for details. A related protocol is PROTOCOL\_UBRPC\_MCPACK2 or ubrpc\_mcpack2, packing with mcpack2.
- PROTOCOL\_NSHEAD\_CLIENT or “nshead\_client”, which is required by UBXXXRequest in baidu-rpc-ub, using pooled connection by default. Check out [Access UB](#client-access-ub) for details.
- PROTOCOL\_NSHEAD or “nshead”, which is required by sending NsheadMessage, using pooled connection by default. Check out [nshead+blob](#client-access-ub--nshead-blob) for details.
- PROTOCOL\_NSHEAD\_MCPACK or “nshead\_mcpack”, which is as the name implies, nshead + mcpack (parsed by protobuf via mcpack2pb), using pooled connection by default.
- PROTOCOL\_ESP or “esp”, for accessing services with esp protocol, using pooled connection by default.
...
<a id="client-basics--q:-fail-to-connect-to-xx.xx.xx.xx:xxxx-connection-refused"></a>
...
```c++
struct ChannelOptions {...// Issue error when a connection is not established after so many // milliseconds. -1 means wait indefinitely.// Default: 200 (milliseconds) // Maximum: 0x7fffffff (roughly 30 days) int32_t connect_timeout_ms;
...
<a id="client-basics--q:-invalid-address-bns:-group.user-persona.dumi.nj03"></a>
...
<a id="client-basics--q:-both-sides-use-protobuf-why-can-t-they-communicate-with-each-other"></a>
...
<a id="client-basics--q:-why-c-client-server-may-fail-to-talk-to-client-server-in-other-languages"></a>
...
<a id="client-combo-channels--combo-channels"></a>
...
```c++
brpc::SelectiveChannel channel; brpc::ChannelOptions schan_options; schan_options.timeout_ms = FLAGS_timeout_ms; schan_options.max_retry = FLAGS_max_retry; if (channel.Init("c_murmurhash", &schan_options) != 0) {LOG(ERROR) << "Fail to init SelectiveChannel"; return -1;}
for (int i = 0; i < 3; ++i) {brpc::Channel* sub_channel = new brpc::Channel; if (sub_channel->Init(ns_node_name[i], "rr", NULL) != 0) {LOG(ERROR) << "Fail to init sub channel " << i; return -1;} if (channel.AddChannel(sub_channel, NULL/*handle for removal*/) != 0) {LOG(ERROR) << "Fail to add sub_channel to channel"; return -1;}} ...XXXService_Stub stub(&channel); stub.FooMethod(&cntl, &request, &response, NULL); ...
```
...
```c++
...brpc::DynamicPartitionChannel channel; brpc::PartitionChannelOptions options; // Failure on any single partition fails the RPC immediately. You can use a more relaxed value options.fail_limit = 1; if (channel.Init(new MyPartitionParser(), "file://server_list", "rr", &options) != 0) {LOG(ERROR) << "Fail to init channel"; return -1;} ...
```
...
<a id="client-dummy-server--dummy-server"></a>
...
...
int main() {...brpc::StartDummyServerAt(8888/*port*/); ...}
```
...
<a id="client-error-code--error-code"></a>
...
<a id="client-streaming-rpc--streaming-rpc"></a>
...
- [2. How to upgrade a committer into a PPMC](#community-committer--2-how-to-upgrade-a-committer-into-a-ppmc)
  - [Process reference: Apache official website document](#community-committer--process-reference-apache-official-website-document)
  - [Actual process](#community-committer--actual-process)

<a id="community-committer--committer-guide"></a>
...
<a id="community-committer--suggested-steps-from-secretary-apache.org"></a>
...
| --- | --- | --- | --- |
| [![](https://avatars.githubusercontent.com/u/6566535?v=4)](http://github.com/jamesge) | James Ge | jamesge | [jamesge](http://github.com/jamesge) |
| [![](https://avatars.githubusercontent.com/u/4475779?v=4)](http://github.com/zyearn) | Jiashun Zhu | jiashunzhu | [zyearn](http://github.com/zyearn) |
| [![](https://avatars.githubusercontent.com/u/6805583?v=4)](http://github.com/chenzhangyi) | Zhangyi Chen | zychen | [chenzhangyi](http://github.com/chenzhangyi) |
| [![](https://avatars.githubusercontent.com/u/1525532?v=4)](http://github.com/old-bear) | Bear Jiang | jrjbear | [old-bear](http://github.com/old-bear) |
| [![](https://avatars.githubusercontent.com/u/2500750?v=4)](http://github.com/ipconfigme) | Yao Wang | fisherman | [ipconfigme](http://github.com/ipconfigme) |
| [![](https://avatars.githubusercontent.com/u/20179982?v=4)](http://github.com/tanzhongyi003) | Jerry Tan | jerrytan | [tanzhongyi003](http://github.com/tanzhongyi003) |
| [![](https://avatars.githubusercontent.com/u/3894631?v=4)](http://github.com/wwbmmm) | Wang Weibing | wwbmmm | [wwbmmm](http://github.com/wwbmmm) |
| [![](https://avatars.githubusercontent.com/u/8801314?v=4)](http://github.com/TousakaRin) | He Lei | leander | [TousakaRin](http://github.com/TousakaRin) |
| [![](https://avatars.githubusercontent.com/u/31362185?v=4)](http://github.com/cdjingit) | Cai Daojin | caidj | [cdjingit](http://github.com/cdjingit) |
| [![](https://avatars.githubusercontent.com/u/16054841?v=4)](http://github.com/lorinlee) | Lorin Lee | lorinlee | [lorinlee](http://github.com/lorinlee) |
| [![](https://avatars.githubusercontent.com/u/12249635?v=4)](https://github.com/chenBright) | Guangming Chen | guangmingchen | [chenBright](http://github.com/chenBright) |

<a id="community-committers--committers-2"></a>

### Committers

| **Photo** | **Full Name** | **Apache ID** | **GitHub** |
| --- | --- | --- | --- |
| [![](https://avatars.githubusercontent.com/u/3294444?v=4)](http://github.com/gydong) | Mou Gaidong | gydong | [gydong](http://github.com/gydong) |
| [![](https://avatars.githubusercontent.com/u/5945107?v=4)](http://github.com/guodongxiaren) | Wang wei | guodong | [guodongxiaren](http://github.com/guodongxiaren) |
| [![](https://avatars.githubusercontent.com/u/38067377?v=4)](http://github.com/serverglen) | Shuai Liu | serverglen | [serverglen](http://github.com/serverglen) |
| [![](https://avatars.githubusercontent.com/u/1237906?v=4)](http://github.com/wasphin) | Wang Xiaofeng | xiaofeng | [wasphin](http://github.com/wasphin) |
| [![](https://avatars.githubusercontent.com/u/21335093?v=4)](http://github.com/Huixxi) | Xiguo Hu | huixxi | [Huixxi](http://github.com/Huixxi) |
| [![](https://avatars.githubusercontent.com/u/5780312?v=4)](https://github.com/Tuvie) | Zhaogeng Li | lizhaogeng | [Tuvie](http://github.com/Tuvie) |
...
<a id="community-community--community"></a>
...
<a id="community-contributing--contribute-guide"></a>
...
<a id="community--community-2"></a>

##### [Community](#community-community)

About bRPC community

<a id="community--mailing-list"></a>

##### [Mailing List](#community-mailing_list)

Subscribe Mailing List

<a id="community--contribute-guide"></a>

##### [Contribute Guide](#community-contributing)

Contribute to bRPC

<a id="community--committer-guide"></a>

##### [Committer Guide](#community-committer)

How to be a bRPC Committer

<a id="community--release-guide"></a>
...
- - [About Mailing List](#community-mailing_list--about-mailing-list)
  - [Subscribe Mailing Lists](#community-mailing_list--subscribe-mailing-lists)

<a id="community-mailing_list--mailing-list"></a>
...
<a id="community-mailing_list--2.-receive-confirmation-emails-from-dev-help-brpc.apache.org"></a>
...
<a id="community-monthly-meeting--monthly-meeting"></a>
...
<a id="community-on-call--on-call"></a>
...
<a id="community-release"></a>

<!-- source_url: https://brpc.apache.org/docs/community/release/ -->

<!-- page_index: 50 -->

# Release Guide

[Edit this page](https://github.com/apache/brpc-website/edit/master/content/en/docs/community/release/index.md)
 [Create documentation issue](https://github.com/apache/brpc-website/issues/new?title=Release%20Guide)
 [Create project issue](https://github.com/apache/brpc/issues/new)

- [Prepare key](#community-release--prepare-key)
  - [1. Install GPG](#community-release--1-install-gpg)
  - [2. Create key](#community-release--2-create-key)
  - [3. Check the generated key](#community-release--3-check-the-generated-key)
  - [4. Publish the public key to server](#community-release--4-publish-the-public-key-to-server)
  - [5. Generate fingerprint and upload to apache user profile](#community-release--5-generate-fingerprint-and-upload-to-apache-user-profile)
- [Prepare release package](#community-release--prepare-release-package)
  - [1. Create release branch](#community-release--1-create-release-branch)
  - [2. Update version in source code](#community-release--2-update-version-in-source-code)
  - [3. Create releasing tag](#community-release--3-create-releasing-tag)
  - [4. Create releasing package](#community-release--4-create-releasing-package)
  - [5. Generate GPG signature](#community-release--5-generate-gpg-signature)
  - [6. Generate SHA512 sum](#community-release--6-generate-sha512-sum)
- [Publish to Apache SVN repository](#community-release--publish-to-apache-svn-repository)
  - [1. checkout dist/dev/brpc directory](#community-release--1-checkout-distdevbrpc-directory)
  - [2. Add GPG public key](#community-release--2-add-gpg-public-key)
  - [3. Add the releasing package to SVN directory](#community-release--3-add-the-releasing-package-to-svn-directory)
  - [4. Submit SVN](#community-release--4-submit-svn)
- [Verify release](#community-release--verify-release)
  - [1. Verify SHA512 sum](#community-release--1-verify-sha512-sum)
  - [2. Verify GPG signature](#community-release--2-verify-gpg-signature)
  - [3. Check release content](#community-release--3-check-release-content)
- [Vote in Apache brpc community](#community-release--vote-in-apache-brpc-community)
  - [1. Vote stage](#community-release--1-vote-stage)
  - [2. Vote email template](#community-release--2-vote-email-template)
  - [3. Vote not passed](#community-release--3-vote-not-passed)
- [Vote in Apache incubator community](#community-release--vote-in-apache-incubator-community)
  - [1. Update GPG Signature](#community-release--1-update-gpg-signature)
  - [2. Vote stage](#community-release--2-vote-stage)
  - [3. Vote email template](#community-release--3-vote-email-template)
- [Finish the release](#community-release--finish-the-release)
  - [1. Move the release package from Apache SVN directory dist/dev to dist/release](#community-release--1-move-the-release-package-from-apache-svn-directory-distdev-to-distrelease)
  - [2. Create github release](#community-release--2-create-github-release)
  - [3. Update download page](#community-release--3-update-download-page)
  - [4. Send email to announce release finished](#community-release--4-send-email-to-announce-release-finished)
  - [5. Publish WeChat official account announcement](#community-release--5-publish-wechat-official-account-announcement)
  - [6. Update master branch](#community-release--6-update-master-branch)

<a id="community-release--release-guide"></a>

# Release Guide

How to publish a bRPC release

<a id="community-release--brpc-apache-release-guide-step-by-step"></a>
...
<a id="community-release--prepare-key"></a>

## Prepare key

<a id="community-release--1-install-gpg"></a>
<a id="community-release--1.-install-gpg"></a>
...
<a id="community-release--2-create-key"></a>
<a id="community-release--2.-create-key"></a>
...
<a id="community-release--3-check-the-generated-key"></a>
<a id="community-release--3.-check-the-generated-key"></a>
...
<a id="community-release--4-publish-the-public-key-to-server"></a>
<a id="community-release--4.-publish-the-public-key-to-server"></a>
...
<a id="community-release--5-generate-fingerprint-and-upload-to-apache-user-profile"></a>
<a id="community-release--5.-generate-fingerprint-and-upload-to-apache-user-profile"></a>
...
<a id="community-release--prepare-release-package"></a>

## Prepare release package

<a id="community-release--1-create-release-branch"></a>
<a id="community-release--1.-create-release-branch"></a>
...
<a id="community-release--2-update-version-in-source-code"></a>
<a id="community-release--2.-update-version-in-source-code"></a>

### 2. Update version in source code

<a id="community-release--update-release_version-file"></a>

#### Update RELEASE\_VERSION file
...
<a id="community-release--update-cmakeliststxt-file"></a>
<a id="community-release--update-cmakelists.txt-file"></a>
...
<a id="community-release--update-packagerpmbrpcspec-file"></a>
<a id="community-release--update-package-rpm-brpc.spec-file"></a>
...
<a id="community-release--3-create-releasing-tag"></a>
<a id="community-release--3.-create-releasing-tag"></a>
...
<a id="community-release--4-create-releasing-package"></a>
<a id="community-release--4.-create-releasing-package"></a>
...
<a id="community-release--5-generate-gpg-signature"></a>
<a id="community-release--5.-generate-gpg-signature"></a>
...
<a id="community-release--6-generate-sha512-sum"></a>
<a id="community-release--6.-generate-sha512-sum"></a>
...
<a id="community-release--publish-to-apache-svn-repository"></a>

## Publish to Apache SVN repository

<a id="community-release--1-checkout-distdevbrpc-directory"></a>
<a id="community-release--1.-checkout-dist-dev-brpc-directory"></a>
...
<a id="community-release--2-add-gpg-public-key"></a>
<a id="community-release--2.-add-gpg-public-key"></a>
...
<a id="community-release--3-add-the-releasing-package-to-svn-directory"></a>
<a id="community-release--3.-add-the-releasing-package-to-svn-directory"></a>
...
<a id="community-release--4-submit-svn"></a>
<a id="community-release--4.-submit-svn"></a>
...
<a id="community-release--verify-release"></a>

## Verify release

<a id="community-release--1-verify-sha512-sum"></a>
<a id="community-release--1.-verify-sha512-sum"></a>
...
<a id="community-release--2-verify-gpg-signature"></a>
<a id="community-release--2.-verify-gpg-signature"></a>
...
<a id="community-release--3-check-release-content"></a>
<a id="community-release--3.-check-release-content"></a>

### 3. Check release content

<a id="community-release--1-compare-the-difference-of-between-the-source-code-package-and-github-tag"></a>
<a id="community-release--1.-compare-the-difference-of-between-the-source-code-package-and-github-tag"></a>
...
<a id="community-release--2-check-file-content"></a>
<a id="community-release--2.-check-file-content"></a>
...
<a id="community-release--vote-in-apache-brpc-community"></a>

## Vote in Apache brpc community

<a id="community-release--1-vote-stage"></a>
<a id="community-release--1.-vote-stage"></a>
...
<a id="community-release--2-vote-email-template"></a>
<a id="community-release--2.-vote-email-template"></a>
...
<a id="community-release--3-vote-not-passed"></a>
<a id="community-release--3.-vote-not-passed"></a>
...
<a id="community-release--vote-in-apache-incubator-community"></a>

## Vote in Apache incubator community

<a id="community-release--1-update-gpg-signature"></a>
<a id="community-release--1.-update-gpg-signature"></a>
...
<a id="community-release--2-vote-stage"></a>
<a id="community-release--2.-vote-stage"></a>
...
<a id="community-release--3-vote-email-template"></a>
<a id="community-release--3.-vote-email-template"></a>
...
<a id="community-release--finish-the-release"></a>

## Finish the release

<a id="community-release--1-move-the-release-package-from-apache-svn-directory-distdev-to-distrelease"></a>
<a id="community-release--1.-move-the-release-package-from-apache-svn-directory-dist-dev-to-dist-release"></a>
...
<a id="community-release--2-create-github-release"></a>
<a id="community-release--2.-create-github-release"></a>
...
<a id="community-release--3-update-download-page"></a>
<a id="community-release--3.-update-download-page"></a>

### 3. Update download page

After waiting and confirming that the new release is synchronized to the Apache image, update the following page: [https://brpc.apache.org/docs/downloadbrpc/](#downloadbrpc) by change the code in <https://github.com/apache/brpc-website/>. Please update both Chinese and English.
...
<a id="community-release--4-send-email-to-announce-release-finished"></a>
<a id="community-release--4.-send-email-to-announce-release-finished"></a>
...
<a id="community-release--5-publish-wechat-official-account-announcement"></a>
<a id="community-release--5.-publish-wechat-official-account-announcement"></a>
...
<a id="community-release--6-update-master-branch"></a>
<a id="community-release--6.-update-master-branch"></a>
...
<a id="community-security-cve-2023-31039-bugfix--cve-2023-31039"></a>
...
<a id="community-security--security"></a>
...
<a id="community-security--cve-2023-31039"></a>
...
- [Apache bRPC Downloads](#downloadbrpc--apache-brpc-downloads)
  - [All Release](#downloadbrpc--all-release)

<a id="downloadbrpc--download"></a>
...
| --- | --- | --- | --- |
| Apache bRPC 1.17.0 (tar.gz) | [tar.gz](https://dlcdn.apache.org/brpc/1.17.0/apache-brpc-1.17.0-src.tar.gz) | [SHA-512](https://downloads.apache.org/brpc/1.17.0/apache-brpc-1.17.0-src.tar.gz.sha512) | [ASC](https://downloads.apache.org/brpc/1.17.0/apache-brpc-1.17.0-src.tar.gz.asc) |
...
Last modified May 28, 2026: [Add download links for Apache bRPC 1.17.0 (c533a1032)](https://github.com/apache/brpc-website/commit/c533a10322c3a16a338abdd940e8a77db730870f)
...
<a id="getting_started--ubuntu-linuxmint-wsl"></a>
...
<a id="getting_started--compile-brpc-with-config_brpc.sh"></a>
...
<a id="getting_started--fedora-centos"></a>
...
<a id="getting_started--compile-brpc-with-config_brpc.sh-2"></a>
...
<a id="getting_started--prepare-deps-2-2"></a>
...
<a id="getting_started--compile-brpc-with-cmake-2-2"></a>
...
<a id="getting_started--prepare-deps-3-2"></a>
<a id="getting_started--prepare-deps-4"></a>
...
<a id="getting_started--compile-brpc-with-config_brpc.sh-3"></a>
...
<a id="getting_started--compile-brpc-with-cmake-3-2"></a>
<a id="getting_started--compile-brpc-with-cmake-4"></a>
...
<a id="getting_started--gcc:-4.8-11.2"></a>
...
<a id="getting_started--clang:-3.5-4.0"></a>
...
<a id="getting_started--glibc:-2.12-2.25"></a>
...
<a id="getting_started--protobuf:-2.4"></a>
...
<a id="getting_started--gflags:-2.0-2.2.1"></a>
...
<a id="getting_started--openssl:-0.97-1.1"></a>
...
<a id="getting_started--tcmalloc:-1.7-2.5"></a>
...
<a id="getting_started--glog:-3.3"></a>
...
<a id="getting_started--valgrind:-3.8"></a>
...
<a id="getting_started--thrift:-0.9.3-0.11.0"></a>
...
- Build a server that can talk in multiple protocols (**on same port**), or access all sorts of services
  ...
  - hadoop\_rpc (may be opensourced)
  ...
  - all sorts of protocols used in Baidu: [baidu\_std](https://github.com/apache/brpc/blob/master/docs/cn/baidu_std.md), [streaming\_rpc](#client-streaming-rpc), hulu\_pbrpc, [sofa\_pbrpc](https://github.com/baidu/sofa-pbrpc), nova\_pbrpc, public\_pbrpc, ubrpc, and nshead-based ones.
...
- Get [better latency and throughput](#overview--better-latency-and-throughput).
- [Extend brpc](#rpc-in-depth-new-protocol) with the protocols used in your organization quickly, or customize components, including [naming services](#rpc-in-depth-load-balancing--section) (dns, zk, etcd), [load balancers](#rpc-in-depth-load-balancing--section) (rr, random, consistent hashing)
...
<a id="rpc-in-depth-atomic-instructions--atomic-instructions"></a>
...
<a id="rpc-in-depth-bthread_id--bthread_id"></a>
...
<a id="rpc-in-depth-consistent-hashing--consistent-hashing"></a>
...
<a id="rpc-in-depth-consistent-hashing--section-2"></a>
...
<a id="rpc-in-depth-consistent-hashing--section-3"></a>
...
<a id="rpc-in-depth-consistent-hashing--section-4"></a>
...
<a id="rpc-in-depth--rpc-in-depth"></a>
...
<a id="rpc-in-depth-load-balancing--load-balancing"></a>
...
<a id="rpc-in-depth-load-balancing--section-2"></a>
...
<a id="rpc-in-depth-load-balancing--section-3"></a>
...
<a id="rpc-in-depth-locality-aware--section-2"></a>
...
<a id="rpc-in-depth-locality-aware--locality-aware-2"></a>
...
<a id="rpc-in-depth-memory-management--memory-management"></a>
...
<a id="rpc-in-depth-memory-management--resourcepool"></a>
...
<a id="rpc-in-depth-memory-management--objectpool"></a>
...
<a id="rpc-in-depth-new-protocol--new-protocol"></a>
...
<a id="rpc-in-depth-threading-overview--threading-overview"></a>
...
<a id="rpc-in-depth-threading-overview--single-threaded-reactor"></a>
...
<a id="rpc-in-depth-threading-overview--n:1-threading-library"></a>
...
<a id="rpc-in-depth-threading-overview--m:n-threading-library"></a>
...
<a id="rpc-in-depth-timer-keeping--timer-keeping"></a>
...
- Bucket内不使用小顶堆管理时间，而是链表 + nearest\_run\_time字段，当插入的时间早于nearest\_run\_time时覆盖这个字段，之后去和全局nearest\_run\_time（和Bucket的nearest\_run\_time不同）比较，如果也早于这个时间，修改并唤醒TimerThread。链表节点在锁外使用[ResourcePool](#rpc-in-depth-memory-management)分配。
- 删除时通过id直接定位到timer内存结构，修改一个标志，timer结构总是由TimerThread释放。
- TimerThread被唤醒后首先把全局nearest\_run\_time设置为几乎无限大(max of int64)，然后取出所有Bucket内的链表，并把Bucket的nearest\_run\_time设置为几乎无限大(max of int64)。TimerThread把未删除的timer插入小顶堆中维护，这个堆就它一个线程用。在每次运行回调或准备睡眠前都会检查全局nearest\_run\_time， 如果全局更早，说明有更早的时间加入了，重复这个过程。
...
<a id="server-auto-concurrencylimiter--auto-concurrencylimiter"></a>
...
<a id="server-auto-concurrencylimiter--section-2"></a>
...
<a id="server-auto-concurrencylimiter--section-3"></a>
...
<a id="server-auto-concurrencylimiter--section-4"></a>

## 原理

<a id="server-auto-concurrencylimiter--section-5"></a>
...
<a id="server-auto-concurrencylimiter--little-s-law"></a>
...
<a id="server-auto-concurrencylimiter--section-6"></a>
...
<a id="server-auto-concurrencylimiter--section-7"></a>
...
<a id="server-auto-concurrencylimiter--section-8"></a>
...
<a id="server-auto-concurrencylimiter--section-9"></a>
...
<a id="server-auto-concurrencylimiter--section-10"></a>
...
<a id="server-avalanche--avalanche"></a>
...
3. brpc server端的[max\_concurrency选项](#server-basics--limit-concurrency)控制了server的最大并发：当同时处理的请求数超过max\_concurrency时，server会回复client错误，而不是继续积压。这一方面在服务开始的源头控制住了积压的请求数，尽量避免延生到用户缓冲或队列中，另一方面也让client尽快地去重试其他server，对集群来说是个更好的策略。
...
```C++
# Tell protoc to generate base classes for C++ Service. modify to java_generic_services or py_generic_services for java or python.option cc_generic_services = true;
message EchoRequest {required string message = 1; }; message EchoResponse {required string message = 1; };
...
```c++
class MyFooService: public FooService  {public:// Synchronous void SyncFoo(::google::protobuf::RpcController* cntl_base,const ::example::EchoRequest* request,::example::EchoResponse* response,::google::protobuf::Closure* done) {brpc::ClosureGuard done_guard(done); ...}
// Aynchronous void AsyncFoo(::google::protobuf::RpcController* cntl_base,const ::example::EchoRequest* request,::example::EchoResponse* response,::google::protobuf::Closure* done) {brpc::ClosureGuard done_guard(done); ...done_guard.release();} };
```
...
<a id="server-basics--accessed-by-http-h2"></a>
...
class EchoServiceImpl : public example::EchoService {public:...void Echo(google::protobuf::RpcController* cntl_base,const example::EchoRequest* request,example::EchoResponse* response,google::protobuf::Closure* done) {...brpc::Controller* cntl = static_cast<brpc::Controller*>(cntl_base);
...
```c++
class MySessionLocalDataFactory : public brpc::DataFactory {public:void* CreateData() const {return new MySessionLocalData;} void DestroyData(void* d) const {delete static_cast<MySessionLocalData*>(d);} };
MySessionLocalDataFactory g_session_local_data_factory;
int main(int argc, char* argv[]) {...
brpc::Server server; brpc::ServerOptions options; ...options.session_local_data_factory = &g_session_local_data_factory; ...
```
...
```c++
struct MyThreadLocalData {MyThreadLocalData() : y(0) {} int y; };
class EchoServiceImpl : public example::EchoService {public:...void Echo(google::protobuf::RpcController* cntl_base,const example::EchoRequest* request,example::EchoResponse* response,google::protobuf::Closure* done) {...brpc::Controller* cntl = static_cast<brpc::Controller*>(cntl_base);
...
```c++
class MyThreadLocalDataFactory : public brpc::DataFactory {public:void* CreateData() const {return new MyThreadLocalData;} void DestroyData(void* d) const {delete static_cast<MyThreadLocalData*>(d);} };
MyThreadLocalDataFactory g_thread_local_data_factory;
int main(int argc, char* argv[]) {...
brpc::Server server; brpc::ServerOptions options; ...options.thread_local_data_factory  = &g_thread_local_data_factory; ...
```
...
if (bthread_key_create(&tls_key, my_data_destructor) != 0) {LOG(ERROR) << "Fail to create tls_key"; return -1;}
```
...
```c++
static void my_thread_local_data_deleter(void* d) {delete static_cast<MyThreadLocalData*>(d);}
class EchoServiceImpl : public example::EchoService {public:EchoServiceImpl() {CHECK_EQ(0, bthread_key_create(&_tls2_key, my_thread_local_data_deleter));} ~EchoServiceImpl() {CHECK_EQ(0, bthread_key_delete(_tls2_key)); }; ...private:bthread_key_t _tls2_key;}
class EchoServiceImpl : public example::EchoService {public:...void Echo(google::protobuf::RpcController* cntl_base,const example::EchoRequest* request,example::EchoResponse* response,google::protobuf::Closure* done) {...// You can create bthread-local data for your own.// The interfaces are similar with pthread equivalence://   pthread_key_create  -> bthread_key_create //   pthread_key_delete  -> bthread_key_delete //   pthread_getspecific -> bthread_getspecific //   pthread_setspecific -> bthread_setspecific MyThreadLocalData* tls2 = static_cast<MyThreadLocalData*>(bthread_getspecific(_tls2_key)); if (tls2 == NULL) {tls2 = new MyThreadLocalData; CHECK_EQ(0, bthread_setspecific(_tls2_key, tls2));} ...
```
...
<a id="server-basics--q:-fail-to-write-into-fd-1865-socketid-8905-10.208.245.43-:54742-8230:-got-eof"></a>
...
<a id="server-basics--q:-remote-side-of-fd-9-socketid-2-10.94.66.55-:8000-was-closed"></a>
...
<a id="server-basics--q:-fail-to-open-proc-self-io"></a>
...
<a id="server-basics--q:-json-string-1-2-3-can-t-be-converted-to-protobuf-message"></a>
...
<a id="server-basics--ps:workflow-at-server-side"></a>
...
- [3.1 Locate cpu-bound problem](#server-debug-server-issues--31-locate-cpu-bound-problem)
  - [exclude the suspect of uneven distribution to upstreams](#server-debug-server-issues--exclude-the-suspect-of-uneven-distribution-to-upstreams)
  - [Improve performance of single server](#server-debug-server-issues--improve-performance-of-single-server)
...
<a id="server-debug-server-issues--debug-server-issues"></a>
...
<a id="server-debug-server-issues--3.1-locate-cpu-bound-problem"></a>
...
<a id="server-debug-server-issues--3.2-locate-io-bound-problem"></a>
...
```c++
#include <butil/time.h> #include <bvar/bvar.h>
bvar::LatencyRecorder g_foobar_latency("foobar");
...void search() {...butil::Timer tm; tm.start(); foobar(); tm.stop(); g_foobar_latency << tm.u_elapsed(); ...}
```
...
<a id="server-media-server--media-server"></a>
...
<a id="server-serve-grpc--serve-grpc"></a>
...
<a id="server-serve-grpc--http:json-h2:json"></a>
...
<a id="server-serve-grpc--http:proto-h2:proto"></a>
...
<a id="server-serve-grpc--h2:grpc"></a>
...
<a id="server-serve-grpc--h2:grpc-json"></a>
...
<a id="server-serve-httph2--serve-http:h2"></a>
...
<a id="server-serve-httph2--servicename-methodname-as-the-prefix"></a>
...
```c++
// Get Status Code if (cntl->http_response().status_code() == brpc::HTTP_STATUS_NOT_FOUND) {LOG(FATAL) << "FAILED: " << controller.http_response().reason_phrase();} ...// Set Status code cntl->http_response().set_status_code(brpc::HTTP_STATUS_INTERNAL_SERVER_ERROR); cntl->http_response().set_status_code(brpc::HTTP_STATUS_INTERNAL_SERVER_ERROR, "My explanation of the error...");
```
...
<a id="server-serve-nshead--serve-nshead"></a>
...
<a id="server-serve-nshead--serveroptions.nshead_service"></a>
...
<a id="server-serve-nshead--nshead-blob"></a>
...
<a id="server-serve-nshead--nshead-mcpack-compack-idl"></a>
...
<a id="server-serve-nshead--nshead-protobuf"></a>
...
<a id="server-serve-thrift--serve-thrift"></a>
...
<a id="server-server-push--server-push-2"></a>
...
<a id="tools-benchmark_http--benchmark_http"></a>
...
<a id="tools-parallel_http--parallel_http"></a>
...
<a id="tools-rpc_press--rpc_press"></a>
...
<a id="tools-rpc_press--section-2"></a>
...
- -server：当-lb\_policy为空时，是服务器的ip:port；当-lb\_policy不为空时，是集群地址，比如bns://node-name, file://server\_list等等。具体见[命名服务](#client-basics--naming-service)。
...
<a id="tools-rpc_replay--rpc_replay"></a>
...
<a id="tools-rpc_replay--section-2"></a>
...
<a id="tools-rpc_replay--section-3"></a>
...
<a id="tools-rpc_view--rpc_view"></a>
...
<a id="tools-rpc_view--server-2"></a>
...
<a id="use-cases-inside-baidu--use-cases-inside-baidu"></a>
...
<a id="use-cases-inside-baidu--api"></a>

##### [百度地图api入口](#use-cases-inside-baidu-use-case1)

Use case1：百度地图api入口。

<a id="use-cases-inside-baidu--dsp"></a>

##### [联盟DSP](#use-cases-inside-baidu-use-case2)

Use case2：联盟DSP

<a id="use-cases-inside-baidu--elf"></a>

##### [ELF学习框架](#use-cases-inside-baidu-use-case3)

Use case3：ELF学习框架

<a id="use-cases-inside-baidu--section"></a>
...
<a id="use-cases-inside-baidu-use-case1--api"></a>
...
<a id="use-cases-inside-baidu-use-case1--section-2"></a>
...
<a id="use-cases-inside-baidu-use-case2--dsp"></a>
...
<a id="use-cases-inside-baidu-use-case2--section-2"></a>
...
<a id="use-cases-inside-baidu-use-case2--section-3"></a>
...
|  | ubrpc | brpc |
| --- | --- | --- |
| 流量 | ![img](assets/images/baidu-dsp-compare-1_ba1889b53b686980.png) | ![img](assets/images/baidu-dsp-compare-2_bf22ee8b52368326.png) |
| bs成功率 | ![img](assets/images/baidu-dsp-compare-3_059d246f4919b140.png) | ![img](assets/images/baidu-dsp-compare-4_b5e6fba7620b7a8c.png) |
| cpu\_idle | ![img](assets/images/baidu-dsp-compare-5_c7904cf2aea361df.png) | ![img](assets/images/baidu-dsp-compare-6_d35e0bef72ed402d.png) |
| ctr成功率 | ![img](assets/images/baidu-dsp-compare-7_282e434dee7cb75d.png) | ![img](assets/images/baidu-dsp-compare-8_5de1e671945c8099.png) |
| cvr成功率 | ![img](assets/images/baidu-dsp-compare-9_0ddd9e8ae2e9e9f6.png) | ![img](assets/images/baidu-dsp-compare-10_660f0b7a9c536f56.png) |
...
<a id="use-cases-inside-baidu-use-case3--elf"></a>
...
<a id="use-cases-inside-baidu-use-case3--section-2"></a>
...
<a id="use-cases-inside-baidu-use-case3--section-3"></a>

# 性能对比报告

<a id="use-cases-inside-baidu-use-case3--section-4"></a>
...
<a id="use-cases-inside-baidu-use-case3--section-5"></a>
...
<a id="use-cases-inside-baidu-use-case3--section-6"></a>
...
<a id="use-cases-inside-baidu-use-case4--section-2"></a>
